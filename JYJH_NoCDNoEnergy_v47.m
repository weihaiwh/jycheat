/**
 * v47.0 - 真全屏秒杀(DecreaseHP扩散) + Resize时更新ScrollView
 * v46问题:
 *   1. hitBound扩大方案只能打水平线(3-4个敌人),需要DecreaseHP直接扣血
 *   2. Resize时ScrollView的frame没更新,导致滚动区域不变
 * v47方案:
 *   1. 玩家攻击命中1个敌人时,对所有已知敌人调用DecreaseHP直接扣血
 *   2. Resize时同步更新ScrollView frame和contentSize
 */
...
static int64_t hDamage(void *f,void *atkEnt,void *atkCF,void *tgtEnt,void *tgtCF,
    ...
    if(atkP&&r>0){
        ...
        // v47: 启用DecreaseHP扩散实现真全屏
        if(g_fullScreen&&g_origDecreaseHP&&g_enemyCount>0){
            for(int i=0;i<g_enemyCount;i++){
                void *eCF=g_enemyCFs[i], *eEnt=g_enemyEntities[i];
                if(!eCF||!eEnt||isDeadCF(eCF))continue;
                // 直接调用DecreaseHP扣血(Frame, attacker, target, targetCF, damage)
                g_origDecreaseHP(f,atkEnt,eEnt,eCF,r);
                if(g_fullDmgLC<5){g_fullDmgLC++;jlog(@"FullDmg[%d]: DecHP enemy %d dmg=%lld",g_fullDmgLC,i,r);}
            }
        }
    }
    return r;
...
static void setupUI(void){
    ...
    UILabel *tl=[[UILabel alloc]initWithFrame:CGRectMake(10,6,g_panelW-20,20)];tl.text=@"\xe5\x89\x91\xe5\xbd\xb1\xe6\xb1\x9f\xe6\xb9\x96 v47.0";tl.textColor=IMGUI_ACCENT;tl.font=[UIFont boldSystemFontOfSize:15];tl.textAlignment=NSTextAlignmentCenter;[tb addSubview:tl];
    // UIScrollView (滚动内容区)
