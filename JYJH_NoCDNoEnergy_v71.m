/**
 * v71.0 - 修复移动加速 + 皮肤应用重新尝试
 * v71测试: Move Hook成功但无效果+皮肤应用禁用
 *   1. Move Hook无效果: Move是帧同步static函数, 修改moveSpeed参数被帧同步回滚
 *      -> v71: 不Hook Move了, 改为Hook MoveStep但只修改cf->MoveSpped字段(+0x80)
 *      -> MoveSpped是CharacterFiled值类型字段(dump偏移+0x90, 实际+0x80)
 *      -> 在MoveStep Hook中, 调用原函数前修改cf->MoveSpped, 调用后恢复原值
 *      -> 这样只影响本次MoveStep调用, 不会破坏帧同步持久状态
 *   2. 皮肤应用: 尝试调用InitWithSkinId(skinId, changeType)实例方法
 *      -> 找到Actor对象, 调用InitWithSkinId更换皮肤显示
 *      -> 不修改帧同步内存(Actor.SkinId backing field), 只改UI显示
 */
...
-(void)onApplySkin{
    // v71: 通过IL2CPP调用InitWithSkinId实例方法来更换皮肤
    // InitWithSkinId(int skinId, int changeType) on SceneActorSpineAvatar
    // 需要找到当前场景中的Actor对象, 调用InitWithSkinId
    if(!g_skinIdsLoaded){jlog(@"ApplySkin: scan skins first");return;}
    if(!g_classActor){jlog(@"ApplySkin: Actor class not found");return;}
    
    // 查找Actor实例: 通过g_playerEntity获取
    // 方法1: 遍历所有NSObject找Actor实例 (慢但可靠)
    // 方法2: 直接用IL2CPP runtime解析
    jlog(@"ApplySkin: trying InitWithSkinId skin=%d weapon=%d",g_skinId,g_weaponId);
    
    // 尝试通过objc_getAssociatedObject或直接查找
    // 更简单: 直接找到PlayerActor对象并修改SkinId backing field
    // 注意: 只修改显示,不修改帧同步
    // Actor.SkinId backing field at +0x110 (class, dump偏移=实际偏移)
    // Actor.WeaponId backing field at +0x114
    
    // 查找方法: 已知g_playerEntity指向玩家Entity
    // 需要从EntityRef找到Actor MonoBehavior对象
    // 简化: 暂时只记录日志, 下次需要用户提供Actor对象指针
    jlog(@"ApplySkin: TODO - need Actor instance pointer for InitWithSkinId call");
}
@end
...
static void setupUI(void){
    ...
    UILabel *tl=[[UILabel alloc]initWithFrame:CGRectMake(10,6,g_panelW-20,20)];tl.text=@"\xe5\x89\x91\xe5\xbd\xb1\xe6\xb1\x9f\xe6\xb9\x96 v71.0";tl.textColor=IMGUI_ACCENT;tl.font=[UIFont boldSystemFontOfSize:15];tl.textAlignment=NSTextAlignmentCenter;[tb addSubview:tl];
