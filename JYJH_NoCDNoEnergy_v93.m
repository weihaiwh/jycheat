/**
 * v93 - 皮肤/武器/光环: 直接修改ScenePlayerData + 设置NeedUpdatePart标志
 * v92问题: GetMainActorData和UpdatePart在大厅中根本不被调用, 被动hook完全无效
 * v93方案:
 * 1. 在LobbyActorSpineAvatar初始化时捕获实例 (通过遍历所有MonoBehaviour)
 * 2. 从LobbyActorSpineAvatar获取LobbyActorData (mLobbyActorData @ 0xb8)
 * 3. 从LobbyActorData获取ScenePlayerData (mSceneData @ 0x10)
 * 4. 修改ScenePlayerData字段: skinId@0x40, weaponId@0x44, fashionId@0x48
 * 5. 设置NeedUpdatePart=true (offset 0x28) 触发游戏刷新渲染
 * 6. 保留UpdatePart hook作为额外保障
 */
...
// v93: 只保留UpdatePart hook作为保障, 去掉GetMainActorData hook
...
// v93: 捕获的LobbyActorSpineAvatar和LobbyActorData实例
...
// ===== v93: LobbyActorData.UpdatePart Hook =====
// 保留作为额外保障
...
static void hUpdatePart(void *mi, void *self, int32_t skinId, int32_t weaponId, int32_t haloId) {
    ...
    // 同时也记录LobbyActorData实例
    if(self) g_lobbyActorData = self;
}

// ===== v93: SetSecneData Hook - 捕获LobbyActorData实例 =====
...
// LobbyActorData.SetSecneData(ScenePlayerData) - 当服务器推送玩家数据时调用
static void hSetSecneData(void *mi, void *self, void *sceneData) {
    if(self) {
        ...
        if(sceneData) {
            ...
            static int lc=0; if(lc<10){lc++;jlog(@"SetSecneData: LobbyActorData=%p skin=%d weapon=%d fashion=%d", self, skinId, weaponId, fashionId);}
        }
    }
    // 先让原函数执行
    if(g_oSetSecneData) ((SetSecneDataOrigFunc)g_oSetSecneData)(mi, self, sceneData);
    // 执行后如果用户有应用设置, 修改ScenePlayerData
    if(self && sceneData && (g_appliedSkinId > 0 || g_appliedWeaponId > 0 || g_appliedHaloId > 0)) {
        ...
        BOOL yes = YES; memcpy((uint8_t*)self + 0x28, &yes, 1);
        jlog(@"SetSecneData: applied skin=%d weapon=%d halo=%d NeedUpdate=1", g_appliedSkinId, g_appliedWeaponId, g_appliedHaloId);
    }
}
...
static void findIL2CPP(void) {
    ...
    jlog(@"v93: UpdatePart=%p SetSecneData=%p LobbyActorData=%p",g_fUpdatePart,g_fSetSecneData,g_classLobbyActorData);
...
static void applyPartChange(void) {
    ...
    if(g_lobbyActorData) {
        ...
        if(sceneData) {
            ...
            memcpy(&weaponId, (uint8_t*)sceneData + 0x44, 4);
            memcpy(&fashionId, (uint8_t*)sceneData + 0x48, 4);
            jlog(@"applyPart: wrote SceneData=%p skin=%d weapon=%d fashion=%d NeedUpdate=1", sceneData, skinId, weaponId, fashionId);
        } else {
            jlog(@"applyPart: SceneData is NULL, trying to find...");
        }
    } else {
        jlog(@"applyPart: LobbyActorData not captured yet, waiting for SetSecneData...");
    }
}
...
// v93: 皮肤/武器/光环 - hook SetSecneData 捕获LobbyActorData
-(void)ensureSkinHooksActive {
    findIL2CPP();
    // Hook SetSecneData (捕获LobbyActorData + 修改ScenePlayerData)
...
static void setupUI(void){
    ...
    UILabel *tl=[[UILabel alloc]initWithFrame:CGRectMake(10,6,g_panelW-20,20)];tl.text=@"剑影江湖 v93";tl.textColor=IMGUI_ACCENT;tl.font=[UIFont boldSystemFontOfSize:15];tl.textAlignment=NSTextAlignmentCenter;[tb addSubview:tl];
...
static void initialize(void){
    ...
    jlog(@"========== JYJH v93 (SetSecneData + ScenePlayerData) ==========");
