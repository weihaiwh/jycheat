/**
 * 剑影江湖 v40.0 - 真全屏秒杀(IsFirstEnemyHit) + 时装皮肤修改 + ImGui风格UI
 *
 * v39问题: CheckPlayerHitCollider只检查"玩家是否被命中",
 *   但非玩家目标(怪物)的命中判定走IsFirstEnemyHit,
 *   IsFirstEnemyHit内部也做XY+Z碰撞判定→不在一条直线打不到
 *
 * v40方案: Hook IsFirstEnemyHit返回非空SpineAnimationInfo
 *   - IsFirstEnemyHit(f, cf, skillButton, states) → SpineAnimationInfo
 *   - 返回null=没命中, 返回SpineAnimationInfo=命中
 *   - Hook后始终返回当前SpineAnimationInfo→强制命中
 *
 * 时装皮肤修改:
 *   - ScenePlayerData: skinId(+0x40), weaponId(+0x44), fashionId(+0x48)
 *   - 通过IL2CPP搜索SetSecneData方法, Hook后修改skinId/weaponId
 *   - 滑块选择皮肤ID
 */
...
// IsFirstEnemyHit(Frame f, CharacterFiled* cf, Int32 skillButton, CharacterStatesAsset states) -> SpineAnimationInfo
typedef void* (*IsFirstEnemyHitFunc)(void *frame, void *cf, int32_t skillButton, void *states);
// GetCurSpineAnimationInfo(Frame f, CharacterFiled* cf) -> SpineAnimationInfo
typedef void* (*GetCurSpineInfoFunc)(void *frame, void *cf);
...
static void *g_funcIsFirstEnemyHit = NULL;  static IsFirstEnemyHitFunc g_origIsFirstEnemyHit = NULL; static BOOL g_isFirstHitHooked = NO;
static void *g_funcGetCurSpineInfo = NULL;  static GetCurSpineInfoFunc g_origGetCurSpineInfo = NULL;
...
// v40: Hook IsFirstEnemyHit - 真全屏命中(覆盖怪物)
// SpineAnimationInfo IsFirstEnemyHit(Frame f, CharacterFiled* cf, Int32 skillButton, CharacterStatesAsset states)
// 返回null=不命中, 返回SpineAnimationInfo对象=命中
static int g_firstHitLogCount = 0;
static void* hookIsFirstEnemyHit(void *frame, void *cf, int32_t skillButton, void *states) {
    void *result = g_origIsFirstEnemyHit ? g_origIsFirstEnemyHit(frame, cf, skillButton, states) : NULL;
    if (g_fullScreen && cf && !isPlayerCF(cf)) {
        // 对非玩家目标(怪物), 强制返回当前SpineAnimationInfo
        if (!result && g_funcGetCurSpineInfo && g_origGetCurSpineInfo) {
            void *spineInfo = g_origGetCurSpineInfo(frame, cf);
            if (spineInfo) {
                if (g_firstHitLogCount < 15) { g_firstHitLogCount++; jlog(@"FirstHit[%d]: forced hit sb=%d spineInfo=%p", g_firstHitLogCount, skillButton, spineInfo); }
                return spineInfo;
            }
        }
    }
    ...
    for (size_t a = 0; a < assemCount; a++) {
        ...
        for (size_t c = 0; c < cnt; c++) {
            ...
            while ((m = get_methods(klass, &iter)) != NULL) {
                ...
                else if (strcmp(n, "IsFirstEnemyHit") == 0 && pc == 4 && !g_funcIsFirstEnemyHit) { g_funcIsFirstEnemyHit=funcAddr; found++; jlog(@"FOUND %s.%s p=%u %p", cn?:"?",n,pc,funcAddr); }
                else if (strcmp(n, "GetCurSpineAnimationInfo") == 0 && pc == 2 && !g_funcGetCurSpineInfo) { g_funcGetCurSpineInfo=funcAddr; found++; jlog(@"FOUND %s.%s p=%u %p", cn?:"?",n,pc,funcAddr); }
            }
        }
    }
...
#define IMGUI_SKIN_BG    [UIColor colorWithRed:0.15 green:0.25 blue:0.45 alpha:0.95]
...
- (void)onFullScreen {
    ...
    if (g_fullScreen) {
        ...
        if (!g_isFirstHitHooked) {
            // Hook IsFirstEnemyHit for true full-screen hit on monsters
            if (g_funcIsFirstEnemyHit) {
                hookOneFunc(g_funcIsFirstEnemyHit, hookIsFirstEnemyHit, (void**)&g_origIsFirstEnemyHit, &g_isFirstHitHooked, "IsFirstEnemyHit");
            } else {
                jlog(@"IsFirstEnemyHit: not found, skip");
            }
        }
    }
...
- (void)skinSliderChanged:(UISlider *)s {
    ...
    // Apply skin change to ScenePlayerData
    if (g_skinId > 0) {
        jlog(@"SkinID changed to: %d (visual only, may need re-enter scene)", g_skinId);
    }
}
@end
...
static void setupUI(void) {
    ...
    title.text=@"  \xe5\x89\x91\xe5\xbd\xb1\xe6\xb1\x9f\xe6\xb9\x96 v40.0"; title.textColor=IMGUI_ACCENT;
    ...
    secTitle.text=@"\xe7\x9a\xae\xe8\x82\xa4" "\xe4\xbf\xae\xe6\x94\xb9 (\xe4\xbb\x85\xe8\xa7\x86\xe8\xa7\x89)";
    secTitle.textColor=IMGUI_ACCENT;
    secTitle.font=[UIFont boldSystemFontOfSize:11];
    [g_panel addSubview:secTitle];

    g_skinLabel=[[UILabel alloc]initWithFrame:CGRectMake(bx,ssy+18,bw,16)];
    g_skinLabel.text=[NSString stringWithFormat:@"\xe7\x9a\xae\xe8\x82\xa4" "ID: %d", g_skinId];
    g_skinLabel.textColor=IMGUI_DIMTEXT;
    g_skinLabel.font=[UIFont systemFontOfSize:11];
    [g_panel addSubview:g_skinLabel];

    g_skinSlider=[[UISlider alloc]initWithFrame:CGRectMake(bx,ssy+36,bw,24)];
