/**
 * v75 - 游戏速度加速 (Time.timeScale + GameSpeed)
 * 基于v74纯功能版, 新增:
 *   - Hook Time.set_timeScale → 控制Unity全局时间缩放(动画/技能/移动/CD全部加速)
 *   - Hook BaseComponent.set_GameSpeed → 游戏自身的速度控制
 *   - 移除无效的MoveStep hook (FrameSync帧同步下改位移≠加速)
 *   - 速度滑块改为"游戏速度", 直接控制timeScale (1.0~5.0x)
 */
    ...
    for(size_t a=0;a<assemCount;a++){
        ...
        for(size_t c=0;c<cnt;c++){
            ...
            while((m=get_methods(klass,&iter))!=NULL){
                ...
                // v71: MoveStep - 保留搜索但v75不用于加速(改用timeScale)
                else if(strcmp(n,"MoveStep")==0&&pc>=7&&!g_fMoveStep){g_fMoveStep=fa;found++;jlog(@"FOUND %s.%s p=%u %p",cn?:"?",n,pc,fa);}
                // v75: Time.timeScale - 控制Unity全局时间(动画/技能/移动/CD全部加速)
                else if(strcmp(n,"set_timeScale")==0&&pc==1&&cn&&strcmp(cn,"Time")==0&&!g_fSetTimeScale){g_fSetTimeScale=fa;found++;jlog(@"FOUND %s.%s p=%u %p",cn,n,pc,fa);}
                else if(strcmp(n,"get_timeScale")==0&&pc==0&&cn&&strcmp(cn,"Time")==0&&!g_fGetTimeScale){g_fGetTimeScale=fa;found++;jlog(@"FOUND %s.%s p=%u %p",cn,n,pc,fa);}
                // v75: BaseComponent.GameSpeed - 游戏自身速度控制
                else if(strcmp(n,"set_GameSpeed")==0&&pc==1&&cn&&strcmp(cn,"BaseComponent")==0&&!g_fSetGameSpeed){g_fSetGameSpeed=fa;found++;jlog(@"FOUND %s.%s p=%u %p",cn,n,pc,fa);}
                else if(strcmp(n,"get_GameSpeed")==0&&pc==0&&cn&&strcmp(cn,"BaseComponent")==0&&!g_fGetGameSpeed){g_fGetGameSpeed=fa;found++;jlog(@"FOUND %s.%s p=%u %p",cn,n,pc,fa);}
            }
        }
    }
...
static void setupUI(void){
    ...
    UILabel *tl=[[UILabel alloc]initWithFrame:CGRectMake(10,6,g_panelW-20,20)];tl.text=@"\xe5\x89\x91\xe5\xbd\xb1\xe6\xb1\x9f\xe6\xb9\x96 v74";tl.textColor=IMGUI_ACCENT;tl.font=[UIFont boldSystemFontOfSize:15];tl.textAlignment=NSTextAlignmentCenter;[tb addSubview:tl];
    ...
    CGFloat spy=s3Y+4;
    g_speedLabel=[[UILabel alloc]initWithFrame:CGRectMake(bx,spy,bw,18)];g_speedLabel.text=[NSString stringWithFormat:@"游戏速度: %.1fx",g_speedMul];g_speedLabel.textColor=IMGUI_ACCENT;g_speedLabel.font=[UIFont boldSystemFontOfSize:12];[sv addSubview:g_speedLabel];
...
static void initialize(void){
    ...
    jlog(@"========== JYJH v75 (game speed via timeScale) ==========");
    jlog(@"iOS %@",[[UIDevice currentDevice] systemVersion]);
    // v75: 无反检测, Shadow处理越狱隐藏
    // 直接启动: 2s后findIL2CPP+applyAllHooks, 5s后setupUI
