/**
 * v84 - 修复皮肤场景切换卡住 + 大招卡住
 * v83测试: MSHookFunction成功, 所有hook OK, 功能可用
 * 问题1: 皮肤应用后切换场景卡住 - g_playerActorObj指向已销毁对象, InitWithSkinId调用在无效对象上
 * 问题2: 某角色放大招卡住 - IL2CPP方法有隐藏MethodInfo*参数, hook签名需包含
 * 修复:
 *   1. 皮肤: 检查Actor对象有效性, 场景切换时重置, 不在无效对象上调用InitWithSkinId
 *   2. 所有IL2CPP hook函数增加第一个void*参数(MethodInfo*), 防止调用栈错位
 *   3. InitWithSkinId调用前验证对象klass指针
 */
...
// v84: 保持v71的hook签名(无MethodInfo*), v71用DobbyHook稳定说明签名正确
// MSHookFunction的trampoline和DobbyHook一样, 不需要MethodInfo*
...
// v84: 验证Actor对象有效性 - 场景切换后旧对象被销毁
static BOOL isValidActorObj(void *obj) {
    if(!obj) return NO;
    // 验证klass指针: IL2CPP对象头部是klass指针
    void *klass=NULL;
    memcpy(&klass,(uint8_t*)obj,8);
    if(!klass) return NO;
    // 验证klass的klass (Il2CppClass**): 应该指向有效的runtime type
    void *klassKlass=NULL;
    memcpy(&klassKlass,(uint8_t*)klass,8);
    if(!klassKlass) return NO;
    return YES;
}

// v84: 场景切换检测 - 当get_SkinId被新对象调用时, 更新playerActorObj
static int32_t hGetSkinId(void *self) {
    // v84: 验证缓存的Actor对象有效性, 无效则重置
    if(g_playerActorObj && !isValidActorObj(g_playerActorObj)) {
        jlog(@"get_SkinId: playerActorObj became invalid, resetting");
        g_playerActorObj=NULL;
        g_appliedSkinId=0; // v84: 场景切换时重置皮肤, 避免加载不存在的资源
    }
    if(self && isValidActorObj(self)) {
        // 检查是否是玩家Actor (isAI==0 at cf+0x44, 但Actor对象需要不同方式判断)
        // 简单策略: 第一个调用get_SkinId的对象就是玩家
        if(!g_playerActorObj) {
    ...
    // v84: 只对当前有效的玩家Actor且已应用皮肤时才返回假ID
    // 场景切换后g_appliedSkinId被重置为0, 不会返回无效ID
    if(self==g_playerActorObj && g_appliedSkinId>0 && isValidActorObj(self)) {
...
static void findIL2CPP(void) {
    ...
    for(size_t a=0;a<assemCount;a++){
        ...
        for(size_t c=0;c<cnt;c++){
            ...
            while((m=get_methods(klass,&iter))!=NULL){
                ...
                else if(strcmp(n,"get_SkinId")==0&&pc==0&&cn&&strcmp(cn,"Actor")==0&&!g_fGetSkinId){g_fGetSkinId=fa;found++;}
                else if(strcmp(n,"InitWithSkinId")==0&&pc==2&&!g_fInitWithSkinId){g_fInitWithSkinId=fa;found++;}
    ...
    g_il2cppDone=YES;
    jlog(@"v84: found %d targets, scanned %d methods",found,totalMethods);
    jlog(@"v84: limitDmg=%p IsReady=%p AttackCanUse=%p CanBeAttack=%p Damage=%p",
         g_fLimitDmg,g_fIsReady,g_fAttackCanUse,g_fCanBeAttack,g_fDamage);
    jlog(@"v84: Intersects=%p CheckHit=%p HitSysUpd=%p MoveStep=%p",
         g_fIntersects,g_fCheckHit,g_fHitSystemUpdate,g_fMoveStep);
    jlog(@"v84: UseSkill=%p UpdateSkillCD=%p HandleSkillRange=%p Unlock=%p",
         g_fUseSkill,g_fUpdateSkillCD,g_fHandleSkillRange,g_fUnlock);
    jlog(@"v84: getSkinId=%p InitWithSkinId=%p Actor=%p",
...
// v84: 不自动hook! applyAllHooks为空 (用MSHookFunction替代DobbyHook)
static void applyAllHooks(void){jlog(@"v84: applyAllHooks skipped (lazy hook mode)");}
...
static void refreshBtns(void){
    ...
    if(g_btnApplySkin){
        if(g_appliedSkinId>0){
            [g_btnApplySkin setTitle:[NSString stringWithFormat:@"\xe7\x9a\xae\xe8\x82\xa4:%d",g_appliedSkinId] forState:UIControlStateNormal];
            g_btnApplySkin.backgroundColor=IMGUI_BTN_ON;g_btnApplySkin.layer.borderColor=IMGUI_GREEN.CGColor;
        } else if(g_skinIdsLoaded){
            [g_btnApplySkin setTitle:@"\xe5\xba\x94\xe7\x94\xa8\xe7\x9a\xae\xe8\x82\xa4" forState:UIControlStateNormal];
            g_btnApplySkin.backgroundColor=[UIColor colorWithRed:0.18 green:0.35 blue:0.55 alpha:0.95];g_btnApplySkin.layer.borderColor=IMGUI_ACCENT.CGColor;
        } else {
            [g_btnApplySkin setTitle:@"\xe5\x85\x88\xe6\x89\xab\xe6\x8f\x8f\xe7\x9a\xae\xe8\x82\xa4" forState:UIControlStateNormal];
...
-(void)onApplySkin{
    ...
    // v84: 只通过hook get_SkinId返回假ID来切换皮肤
    // 不调用InitWithSkinId - 它可能导致场景切换卡住和技能动画卡住
    // 原因: InitWithSkinId会异步加载Spine资源, 可能在特定场景/角色下造成死锁
    // get_SkinId hook已经足够让游戏加载新皮肤外观
    jlog(@"ApplySkin: skin=%d weapon=%d (via get_SkinId hook only)",g_skinId,g_weaponId);
}
@end
...
static void setupUI(void){
    ...
    UILabel *tl=[[UILabel alloc]initWithFrame:CGRectMake(10,6,g_panelW-20,20)];tl.text=@"\xe5\x89\x91\xe5\xbd\xb1\xe6\xb1\x9f\xe6\xb9\x96 v82";tl.textColor=IMGUI_ACCENT;tl.font=[UIFont boldSystemFontOfSize:15];tl.textAlignment=NSTextAlignmentCenter;[tb addSubview:tl];
    ...
    CGFloat spy=s3Y+4;
    g_speedLabel=[[UILabel alloc]initWithFrame:CGRectMake(bx,spy,bw,18)];g_speedLabel.text=@"\xe7\xa7\xbb\xe5\x8a\xa8\xe9\x80\x9f\xe5\xba\xa6: 1.0x";g_speedLabel.textColor=IMGUI_ACCENT;g_speedLabel.font=[UIFont boldSystemFontOfSize:12];[sv addSubview:g_speedLabel];
    g_speedSlider=[[UISlider alloc]initWithFrame:CGRectMake(bx,spy+20,bw,28)];g_speedSlider.minimumValue=1.0;g_speedSlider.maximumValue=5.0;g_speedSlider.value=1.0;[g_speedSlider addTarget:[JYJHActionHandler shared] action:@selector(speedSliderChanged:) forControlEvents:UIControlEventValueChanged];[sv addSubview:g_speedSlider];
...
// v84: 启动时只findIL2CPP, 不自动hook任何函数!
...
static void initialize(void){
    ...
    jlog(@"========== JYJH v84 (MSHookFunction, skin fix) ==========");
