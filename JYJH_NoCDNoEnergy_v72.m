/**
 * v72.1 - 反检测: hook dyld/getenv/NSProcessInfo隐藏注入dylib
 * 参考Shadow(jjolano/shadow)的反检测思路:
 *   - hook _dyld_image_count/_dyld_get_image_name 过滤掉注入的dylib
 *   - hook getenv 隐藏 DYLD_INSERT_LIBRARIES 环境变量
 *   - hook NSProcessInfo.environment 过滤敏感键
 *   - hook task_info 限制 dyld_all_image_infos 数量
 */
...
// 要隐藏的dylib路径关键字列表(只要路径包含这些关键字就从dyld列表中过滤掉)
...
static void setupUI(void){
    ...
    UILabel *tl=[[UILabel alloc]initWithFrame:CGRectMake(10,6,g_panelW-20,20)];tl.text=@"\xe5\x89\x91\xe5\xbd\xb1\xe6\xb1\x9f\xe6\xb9\x96 v72.1";tl.textColor=IMGUI_ACCENT;tl.font=[UIFont boldSystemFontOfSize:15];tl.textAlignment=NSTextAlignmentCenter;[tb addSubview:tl];
...
static void initialize(void){
    ...
    // v72.2: 延迟安装反检测hook - constructor阶段dyld还在初始化,
