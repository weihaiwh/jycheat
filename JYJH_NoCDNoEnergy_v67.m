/**
 * v67.0 - 大招替换(UseSkill仅替换玩家技能) + 皮肤UI交互完善
 * v66测试:
 *   1. 技能无CD全部生效(14-19都返回YES+跳过CD更新) ✓
 *   2. 但大招有怒气判断,无CD没用,没怒气释放不了
 *      -> v67: 回归Hook UseSkill,只替换玩家(通过isPlayerCF判断cf参数)的skillStateType
 *      -> 关键: v65卡住是因为替换了所有人的技能(含AI),导致帧同步不一致
 *      -> v67: 仅替换玩家cf的14-18->19, AI/其他玩家不变
 *   3. 皮肤扫描成功(206皮肤+78武器) ✓
 *      -> v67: 完善皮肤应用逻辑, slider范围改为扫描到的ID列表索引
 *      -> 皮肤应用: Hook get_SkinId返回目标ID + 写Actor backing field
 */
...
static BOOL g_skillReplace=NO; // v64: 技能替换开关
...
static BOOL hIsReady(void *f,int st,void *cf,void *st2){
    ...
    // v66: 技能替换时, 所有技能(14-19)都返回YES; 普通无CD时, 只对大招(17-19)返回YES
    if(g_skillReplace&&st>=14&&st<=19){if(g_isReadyLC<30){g_isReadyLC++;jlog(@"IsReady: st=%d->YES(replace)",st);}return YES;}
...
static BOOL hAttackCanUse(void *f,int st,void *cf,void *st2){
    ...
    // v66: 同IsReady逻辑
    if(g_skillReplace&&st>=14&&st<=19){if(g_attackLC<30){g_attackLC++;jlog(@"AttackCanUse: st=%d->YES(replace)",st);}return YES;}
...
// v67: UseSkill Hook - 大招替换核心
// UseSkill(Frame, EntityRef, CharacterFiled*, CharacterStateType skillStateType, BOOL isRight, CharacterStatesAsset states, BaseCharacterState state, PlayerInfo* playerInfo)
// v65卡住原因: 替换了所有人的技能(含AI), 导致帧同步不一致
// v67修复: 仅替换玩家(cf==g_playerCF)的skillStateType 14-18->19
...
static void hUseSkill(void *f,void *entity,void *cf,int skillStateType,BOOL isRight,void *states,void *state,void *playerInfo) {
    if(g_skillReplace && skillStateType>=14 && skillStateType<=18) {
        // v67: 只替换玩家(非AI)的技能
        // 方法1: 通过cf判断是否为玩家(IsAI==0)
        // 方法2: 更安全 - 只替换攻击时st>=14且cf是玩家控制的
        BOOL isPlayer = NO;
        if(cf) {
            // CharacterFiled.IsAI at +0x44 (dump偏移0x54-0x10=0x44)
        ...
        if(isPlayer) {
            ...
            if(g_useSkillLC<10){g_useSkillLC++;jlog(@"UseSkill: AI st=%d (no replace)",skillStateType);}
        }
    }
    ...
    if(!g_classHotfixGameEntry){
        for(int i=0;i<g_gameEntryCount;i++){
            if(g_allGameEntryNS[i]==NULL||strcmp(g_allGameEntryNS[i],"")==0){
                ...
                jlog(@"v67: assume GameEntry[%d] ns=empty is HotfixGameEntry=%p",i,g_allGameEntries[i]);
                break;
            }
        }
    }
    jlog(@"v67: UnityGameEntry=%p HotfixGameEntry=%p",g_classUnityGameEntry,g_classHotfixGameEntry);
...
+(instancetype)shared;
-(void)onIgnoreUnlock;-(void)onExSkillNoCD;-(void)onGodMode;-(void)onFullScreen;-(void)onSkillReplace;-(void)onApplySkin;
...
-(void)onApplySkin{
    ...
    if(g_playerActorObj){
        ...
        // 写Actor backing field (SkinId at +0x110, WeaponId at +0x114)
        memcpy(p+0x110,&g_skinId,4); memcpy(p+0x114,&g_weaponId,4);
        // v67: Hook get_SkinId会在下次读取时返回g_appliedSkinId
        // 同时需要触发UI刷新: 设置NeedUpdatePart标志
        // Actor.IsLocal at +0x10d (bool) - 不再依赖, 直接设置
        jlog(@"ApplySkin: written to Actor fields + get_SkinId hook active");
    } else {
        jlog(@"ApplySkin: Actor not found yet, hook active - will apply on next get_SkinId call");
    }
}
@end
...
static void setupUI(void){
    ...
    UILabel *tl=[[UILabel alloc]initWithFrame:CGRectMake(10,6,g_panelW-20,20)];tl.text=@"\xe5\x89\x91\xe5\xbd\xb1\xe6\xb1\x9f\xe6\xb9\x96 v67.0";tl.textColor=IMGUI_ACCENT;tl.font=[UIFont boldSystemFontOfSize:15];tl.textAlignment=NSTextAlignmentCenter;[tb addSubview:tl];
