/**
 * v95 - 皮肤/武器/光环: 通过mActorData直接修改+ReInitWithActorData刷新
 * v94问题: SetSecneData路线skinId=0, ScenePlayerData不是渲染数据源
 * v95方案:
 *   1. Hook LobbyActorSpineAvatar.SetData 捕获avatar实例
 *   2. 修改avatar->mActorData(0x78)的ActorData字段: SkinId@0x14 WeaponId@0x18 HaloId@0x1c
 *   3. 直接调用ReInitWithActorData(函数指针,不走trampoline)刷新
 *   4. SetSecneData只做调试dump,不修改
 */
...
static void findIL2CPP(void) {
    ...
    for(size_t a=0;a<assemCount;a++){
        ...
        for(size_t c=0;c<cnt;c++){
            ...
            while((m=get_methods(klass,&iter))!=NULL){
                ...
                // v95: 搜索LobbyActorSpineAvatar.SetData 1参数
                else if(strcmp(n,"SetData")==0&&pc==1&&!g_fSetData){g_fSetData=fa;found++;jlog(@"FOUND %s.%s p=%u %p",cn?:"?",n,pc,fa);}
                // v95: 搜索SceneActorSpineAvatar.ReInitWithActorData 1参数
                else if(strcmp(n,"ReInitWithActorData")==0&&pc==1&&!g_fReInitWithActorData){g_fReInitWithActorData=fa;found++;jlog(@"FOUND %s.%s p=%u %p",cn?:"?",n,pc,fa);}
                // v95: 搜索SceneActorSpineAvatar.InitWithSkinId 2参数
                else if(strcmp(n,"InitWithSkinId")==0&&pc==2&&!g_fInitWithSkinId){g_fInitWithSkinId=fa;found++;jlog(@"FOUND %s.%s p=%u %p",cn?:"?",n,pc,fa);}
                // v95: SetSecneData 1参数 (调试用)
                else if(strcmp(n,"SetSecneData")==0&&pc==1&&!g_fSetSecneData){g_fSetSecneData=fa;found++;jlog(@"FOUND %s.%s p=%u %p",cn?:"?",n,pc,fa);}
            }
        }
    }
    ...
    jlog(@"v95: SetData=%p ReInit=%p InitSkin=%p SetSecneData=%p",g_fSetData,g_fReInitWithActorData,g_fInitWithSkinId,g_fSetSecneData);
...
// v95: 应用外观 - 修改avatar->mActorData字段+ReInitWithActorData刷新
static void applyPartChange(void) {
    jlog(@"applyPart: skin=%d weapon=%d halo=%d", g_appliedSkinId, g_appliedWeaponId, g_appliedHaloId);
    
    // 路径1: 通过avatar->mActorData修改 + ReInitWithActorData刷新
    if(g_lobbyAvatarObj && g_fReInitWithActorData) {
        void *actorData = NULL;
        memcpy(&actorData, (uint8_t*)g_lobbyAvatarObj + 0x78, 8);
        if(isValidPtr(actorData)) {
            // 读取原始值
            int32_t origSkin=0, origWeapon=0, origHalo=0;
            memcpy(&origSkin, (uint8_t*)actorData + 0x14, 4);
            memcpy(&origWeapon, (uint8_t*)actorData + 0x18, 4);
            memcpy(&origHalo, (uint8_t*)actorData + 0x1c, 4);
            jlog(@"applyPart: actorData=%p before: skin=%d weapon=%d halo=%d", actorData, origSkin, origWeapon, origHalo);
            
            // 修改字段
            if(g_appliedSkinId > 0) memcpy((uint8_t*)actorData + 0x14, &g_appliedSkinId, 4);
            if(g_appliedWeaponId > 0) memcpy((uint8_t*)actorData + 0x18, &g_appliedWeaponId, 4);
            if(g_appliedHaloId > 0) memcpy((uint8_t*)actorData + 0x1c, &g_appliedHaloId, 4);
            
            // 验证写入
            int32_t newSkin=0, newWeapon=0, newHalo=0;
            memcpy(&newSkin, (uint8_t*)actorData + 0x14, 4);
            memcpy(&newWeapon, (uint8_t*)actorData + 0x18, 4);
            memcpy(&newHalo, (uint8_t*)actorData + 0x1c, 4);
            jlog(@"applyPart: after write: skin=%d weapon=%d halo=%d", newSkin, newWeapon, newHalo);
            
            // 直接调用ReInitWithActorData(不走trampoline)
            // IL2CPP实例方法签名: (MethodInfo*, self, ActorData*)
            typedef void (*ReInitFunc)(void*, void*, void*);
            ReInitFunc fn = (ReInitFunc)g_fReInitWithActorData;
            jlog(@"applyPart: calling ReInitWithActorData(%p, %p, %p)", NULL, g_lobbyAvatarObj, actorData);
            fn(NULL, g_lobbyAvatarObj, actorData);
            jlog(@"applyPart: ReInitWithActorData done!");
            
            // 不清除applied值, 允许反复应用
            return;
        } else {
            jlog(@"applyPart: avatar->mActorData is NULL/invalid, trying fallback");
        }
    }
    
    // 路径2(备选): 通过InitWithSkinId刷新(只支持皮肤)
    if(g_lobbyAvatarObj && g_fInitWithSkinId && g_appliedSkinId > 0) {
        // InitWithSkinId(skinId, changeType) 签名: (MethodInfo*, self, int32_t, int32_t)
        typedef void (*InitSkinFunc)(void*, void*, int32_t, int32_t);
        InitSkinFunc fn = (InitSkinFunc)g_fInitWithSkinId;
        jlog(@"applyPart: calling InitWithSkinId(%p, %p, %d, 1)", NULL, g_lobbyAvatarObj, g_appliedSkinId);
        fn(NULL, g_lobbyAvatarObj, g_appliedSkinId, 1);
        jlog(@"applyPart: InitWithSkinId done!");
        return;
    }
    
    // 路径3(兜底): ScenePlayerData+NeedUpdatePart
    if(g_lobbyActorData) {
        BOOL isSelf = NO;
        ...
        if(sceneData) {
            ...
            if(g_appliedHaloId > 0) memcpy((uint8_t*)sceneData + 0x48, &g_appliedHaloId, 4);
            BOOL yes = YES; memcpy((uint8_t*)g_lobbyActorData + 0x28, &yes, 1);
            jlog(@"applyPart: fallback SceneData path, NeedUpdate=1");
        }
    } else {
        jlog(@"applyPart: no avatar/data captured yet");
    }
}
...
static void setupUI(void){
    ...
    UILabel *tl=[[UILabel alloc]initWithFrame:CGRectMake(10,6,g_panelW-20,20)];tl.text=@"剑影江湖 v95";tl.textColor=IMGUI_ACCENT;tl.font=[UIFont boldSystemFontOfSize:15];tl.textAlignment=NSTextAlignmentCenter;[tb addSubview:tl];
...
static void initialize(void){
    ...
    jlog(@"========== JYJH v95 (mActorData+ReInit) ==========");
