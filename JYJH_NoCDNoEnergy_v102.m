/**
 * v102 - 禁用皮肤hook(参数错位卡死) + 新增SSO登录检测跳过
 * v101问题:
 *   1. InitWithSkinId hook参数错位! self=0xbb8 → MSHookFunction参数栈错位
 *   2. UpdatePart从未被游戏调用
 *   3. 皮肤hook导致游戏卡死
 * v102方案:
 *   1. 完全禁用皮肤/武器/光环hook (待找到正确方案再恢复)
 *   2. 新增SSO登录检测跳过: hook SSOData.get_code()返回200
 *      服务器返回code=502+banTime → 游戏判定账号异常
 *      hook get_code返回200 → 绕过检测
 *   3. 保留所有其他已验证可用的功能(技能/伤害/无敌等)
 */
...
static int32_t hGetCode(void *mi, void *self) {
    ...
    if(g_bypassSSO && origCode != 200) {
        jlog(@"SSO: get_code=%d → bypassed to 200", origCode);
        return 200;
    }
    return origCode;
}
...
static void initialize(void){
    ...
    jlog(@"========== JYJH v102 (skin disabled + SSO bypass) ==========");
