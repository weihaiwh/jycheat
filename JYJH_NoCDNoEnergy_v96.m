/**
 * v96 - 修复SetData搜索匹配错误类 + 直接修改Actor字段+ChangePart刷新
 * v95问题: SetData搜索pc==1匹配到了GraphicsBuffer.SetData, LobbyActorSpineAvatar.SetData从未被hook
 *   导致g_lobbyAvatarObj始终为NULL, applyPart走兜底路径也失败(IsSelf=NO)
 * v96修复:
 *   1. SetData搜索必须限定类名LobbyActorSpineAvatar
 *   2. SetSecneData搜索必须限定类名LobbyActorData
 *   3. ReInitWithActorData搜索限定类名SceneActorSpineAvatar
 *   4. InitWithSkinId搜索限定类名SceneActorSpineAvatar
 * v96方案:
 *   1. Hook LobbyActorSpineAvatar.SetData 捕获avatar实例
 *   2. 修改Actor字段: <SkinId>k__BackingField@0x110, <WeaponId>@0x114, <HaloId>@0x118
 *   3. 调用ChangePart(AvatarPartType, id, NULL)刷新渲染
 *   4. 备选: 修改mActorData+ReInitWithActorData
 */
...
// v96: 直接调用函数指针(不走trampoline)
static void *g_fReInitWithActorData=NULL; // SceneActorSpineAvatar.ReInitWithActorData(ActorData)
static void *g_fInitWithSkinId=NULL;      // SceneActorSpineAvatar.InitWithSkinId(skinId, changeType)
static void *g_fChangePart=NULL;          // SceneActorSpineAvatar.ChangePart(AvatarPartType, id, UIC_SpineView)

// v96: Hook LobbyActorSpineAvatar.SetData 捕获avatar
static void *g_fSetData=NULL; static void *g_oSetData=NULL; static BOOL g_hSetData=NO;
typedef void (*SetDataOrigFunc)(void*,void*,void*);

// v96: SetSecneData hook只做调试dump
...
static void *g_lobbyAvatarObj=NULL; // LobbyActorSpineAvatar MonoBehaviour实例
static void *g_lobbyActorData=NULL; // LobbyActorData 数据实例
...
// ===== v95: Hook LobbyActorSpineAvatar.SetData 捕获avatar =====
static int g_setDataLC=0;
static void hSetData(void *mi, void *self, void *lobbyActorData) {
    if(g_oSetData) ((SetDataOrigFunc)g_oSetData)(mi, self, lobbyActorData);
    if(!self) return;
    g_lobbyAvatarObj = self;
    if(lobbyActorData) g_lobbyActorData = lobbyActorData;
    if(g_setDataLC < 30) {
        g_setDataLC++;
        // dump avatar->mActorData (0x78)
        void *actorData = NULL;
        memcpy(&actorData, (uint8_t*)self + 0x78, 8);
        if(isValidPtr(actorData)) {
            int32_t skinId=0, weaponId=0, haloId=0;
            memcpy(&skinId, (uint8_t*)actorData + 0x14, 4);
            memcpy(&weaponId, (uint8_t*)actorData + 0x18, 4);
            memcpy(&haloId, (uint8_t*)actorData + 0x1c, 4);
            jlog(@"SetData: avatar=%p actorData=%p skin=%d weapon=%d halo=%d", self, actorData, skinId, weaponId, haloId);
        } else {
            jlog(@"SetData: avatar=%p actorData=NULL/invalid", self);
        }
        // dump LobbyActorData
        if(lobbyActorData) {
            BOOL isSelf = NO;
            memcpy(&isSelf, (uint8_t*)lobbyActorData + 0x2b, 1);
            void *sceneData = NULL;
            memcpy(&sceneData, (uint8_t*)lobbyActorData + 0x10, 8);
            if(isValidPtr(sceneData) && isSelf) {
                int32_t skin2=0, weapon2=0, fashion=0;
                memcpy(&skin2, (uint8_t*)sceneData + 0x40, 4);
                memcpy(&weapon2, (uint8_t*)sceneData + 0x44, 4);
                memcpy(&fashion, (uint8_t*)sceneData + 0x48, 4);
                jlog(@"  IsSelf! sceneData=%p skin=%d weapon=%d fashion=%d", sceneData, skin2, weapon2, fashion);
            }
        }
    }
}

// ===== v95: SetSecneData hook - 只做调试dump =====
...
static void hSetSecneData(void *mi, void *self, void *sceneData) {
    ...
    if(isSelf && sceneData && g_setSecneDataLC < 20) {
        g_setSecneDataLC++;
        ...
        jlog(@"SetSecneData[IsSelf]: skin=%d weapon=%d fashion=%d", skinId, weaponId, fashionId);
    }
}
...
static void findIL2CPP(void) {
    ...
    for(size_t a=0;a<assemCount;a++){
        ...
        for(size_t c=0;c<cnt;c++){
            ...
            while((m=get_methods(klass,&iter))!=NULL){
                ...
                // v96: 搜索LobbyActorSpineAvatar.SetData 1参数 (必须限定类名!)
                else if(strcmp(n,"SetData")==0&&pc==1&&cn&&strcmp(cn,"LobbyActorSpineAvatar")==0&&!g_fSetData){g_fSetData=fa;found++;jlog(@"FOUND %s.%s p=%u %p",cn,n,pc,fa);}
                // v96: 搜索SceneActorSpineAvatar.ReInitWithActorData 1参数
                else if(strcmp(n,"ReInitWithActorData")==0&&pc==1&&cn&&strcmp(cn,"SceneActorSpineAvatar")==0&&!g_fReInitWithActorData){g_fReInitWithActorData=fa;found++;jlog(@"FOUND %s.%s p=%u %p",cn,n,pc,fa);}
                // v96: 搜索SceneActorSpineAvatar.InitWithSkinId 2参数
                else if(strcmp(n,"InitWithSkinId")==0&&pc==2&&cn&&strcmp(cn,"SceneActorSpineAvatar")==0&&!g_fInitWithSkinId){g_fInitWithSkinId=fa;found++;jlog(@"FOUND %s.%s p=%u %p",cn,n,pc,fa);}
                // v96: 搜索SceneActorSpineAvatar.ChangePart 3参数
                else if(strcmp(n,"ChangePart")==0&&pc==3&&cn&&strcmp(cn,"SceneActorSpineAvatar")==0&&!g_fChangePart){g_fChangePart=fa;found++;jlog(@"FOUND %s.%s p=%u %p",cn,n,pc,fa);}
                // v96: SetSecneData 1参数 限定LobbyActorData类
    ...
    jlog(@"v96: SetData=%p ReInit=%p InitSkin=%p ChangePart=%p SetSecneData=%p",g_fSetData,g_fReInitWithActorData,g_fInitWithSkinId,g_fChangePart,g_fSetSecneData);
...
// v96: 应用外观 - 修改Actor字段+ChangePart刷新
// Actor字段: <SkinId>@0x110, <WeaponId>@0x114, <HaloId>@0x118
// AvatarPartType: Skin=2, Weapon=3, Halo=4
static void applyPartChange(void) {
    jlog(@"applyPart: skin=%d weapon=%d halo=%d avatar=%p", g_appliedSkinId, g_appliedWeaponId, g_appliedHaloId, g_lobbyAvatarObj);
    
    if(!g_lobbyAvatarObj) {
        jlog(@"applyPart: no avatar captured yet!");
        return;
    }
    
    // dump当前状态
    void *actorData = NULL;
    memcpy(&actorData, (uint8_t*)g_lobbyAvatarObj + 0x78, 8);
    int32_t aSkin=0, aWeapon=0, aHalo=0;
    int32_t fSkin=0, fWeapon=0, fHalo=0;
    if(isValidPtr(actorData)) {
        memcpy(&fSkin, (uint8_t*)actorData + 0x14, 4);
        memcpy(&fWeapon, (uint8_t*)actorData + 0x18, 4);
        memcpy(&fHalo, (uint8_t*)actorData + 0x1c, 4);
    }
    memcpy(&aSkin, (uint8_t*)g_lobbyAvatarObj + 0x110, 4);
    memcpy(&aWeapon, (uint8_t*)g_lobbyAvatarObj + 0x114, 4);
    memcpy(&aHalo, (uint8_t*)g_lobbyAvatarObj + 0x118, 4);
    jlog(@"applyPart: before: Actor skin=%d weapon=%d halo=%d | ActorData skin=%d weapon=%d halo=%d", aSkin, aWeapon, aHalo, fSkin, fWeapon, fHalo);
    
    // 修改Actor字段 + ActorData字段
    if(g_appliedSkinId > 0) {
        memcpy((uint8_t*)g_lobbyAvatarObj + 0x110, &g_appliedSkinId, 4);
        if(isValidPtr(actorData)) memcpy((uint8_t*)actorData + 0x14, &g_appliedSkinId, 4);
    }
    if(g_appliedWeaponId > 0) {
        memcpy((uint8_t*)g_lobbyAvatarObj + 0x114, &g_appliedWeaponId, 4);
        if(isValidPtr(actorData)) memcpy((uint8_t*)actorData + 0x18, &g_appliedWeaponId, 4);
    }
    if(g_appliedHaloId > 0) {
        memcpy((uint8_t*)g_lobbyAvatarObj + 0x118, &g_appliedHaloId, 4);
        if(isValidPtr(actorData)) memcpy((uint8_t*)actorData + 0x1c, &g_appliedHaloId, 4);
    }
    ...
    memcpy(&aWeapon, (uint8_t*)g_lobbyAvatarObj + 0x114, 4);
    memcpy(&aHalo, (uint8_t*)g_lobbyAvatarObj + 0x118, 4);
    jlog(@"applyPart: after write: Actor skin=%d weapon=%d halo=%d", aSkin, aWeapon, aHalo);
    
    // ChangePart刷新: (MethodInfo*, self, int32_t partType, int32_t id, void* spineView)
    if(g_fChangePart) {
        typedef void (*ChangePartFunc)(void*, void*, int32_t, int32_t, void*);
        ChangePartFunc fn = (ChangePartFunc)g_fChangePart;
        if(g_appliedSkinId > 0) { jlog(@"applyPart: ChangePart(Skin=2, id=%d)", g_appliedSkinId); fn(NULL, g_lobbyAvatarObj, 2, g_appliedSkinId, NULL); jlog(@"applyPart: ChangePart Skin done"); }
        if(g_appliedWeaponId > 0) { jlog(@"applyPart: ChangePart(Weapon=3, id=%d)", g_appliedWeaponId); fn(NULL, g_lobbyAvatarObj, 3, g_appliedWeaponId, NULL); jlog(@"applyPart: ChangePart Weapon done"); }
        if(g_appliedHaloId > 0) { jlog(@"applyPart: ChangePart(Halo=4, id=%d)", g_appliedHaloId); fn(NULL, g_lobbyAvatarObj, 4, g_appliedHaloId, NULL); jlog(@"applyPart: ChangePart Halo done"); }
    } else if(g_fReInitWithActorData && isValidPtr(actorData)) {
        typedef void (*ReInitFunc)(void*, void*, void*);
        ReInitFunc fn = (ReInitFunc)g_fReInitWithActorData;
        jlog(@"applyPart: calling ReInitWithActorData"); fn(NULL, g_lobbyAvatarObj, actorData); jlog(@"applyPart: ReInit done!");
    } else if(g_fInitWithSkinId && g_appliedSkinId > 0) {
        typedef void (*InitSkinFunc)(void*, void*, int32_t, int32_t);
        InitSkinFunc fn = (InitSkinFunc)g_fInitWithSkinId;
        jlog(@"applyPart: calling InitWithSkinId"); fn(NULL, g_lobbyAvatarObj, g_appliedSkinId, 1); jlog(@"applyPart: InitSkin done!");
    } else {
        jlog(@"applyPart: no refresh func, only fields modified");
    }
}
...
// v95: 皮肤/武器/光环 - hook SetData捕获avatar + SetSecneData调试
-(void)ensureSkinHooksActive {
    findIL2CPP();
    // Hook LobbyActorSpineAvatar.SetData 捕获avatar实例
    if(g_fSetData && !g_hSetData) hookOneFunc(g_fSetData, hSetData, (void**)&g_oSetData, &g_hSetData, "SetData");
    // Hook SetSecneData (只做调试dump)
...
static void setupUI(void){
    ...
    UILabel *tl=[[UILabel alloc]initWithFrame:CGRectMake(10,6,g_panelW-20,20)];tl.text=@"剑影江湖 v96";tl.textColor=IMGUI_ACCENT;tl.font=[UIFont boldSystemFontOfSize:15];tl.textAlignment=NSTextAlignmentCenter;[tb addSubview:tl];
...
static void initialize(void){
    ...
    jlog(@"========== JYJH v96 (fix SetData search + ChangePart) ==========");
