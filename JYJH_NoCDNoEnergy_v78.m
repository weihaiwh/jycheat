/**
 * v78 - 基于稳定v74 + Time.timeScale内存直写(安全版)
 * v75 DobbyHook timeScale闪退, v77 initTimeScale在applyAllHooks后crash
 * v78方案: 
 *   1. initTimeScale延迟到8s(游戏完全加载后), 用@try/@catch保护
 *   2. il2cpp_class_get_static_field_data可能返回NULL或crash, 加保护
 *   3. 如果initTimeScale失败, v74功能照常工作
 *   4. 去掉MoveStep hook, 改用timeScale
 */
...
static float g_gameSpeed=1.0f; // v78: 游戏速度倍率(timeScale) 1.0=正常

// ===== v78: Time.timeScale 内存直写 =====
static void *g_timeClassSD=NULL; // Time类静态数据区指针
static int g_timeScaleOff=-1;    // s_timeScale在SD中的偏移
static void *g_timeKlass=NULL;   // UnityEngine.Time klass指针
static dispatch_source_t g_speedTimer=NULL;

// 通过IL2CPP field API查找s_timeScale偏移
static void findTimeScaleFieldOffset(void *timeKlass) {
    if(!timeKlass) return;
    void *h=dlopen(NULL,RTLD_LAZY);
    if(!h) return;
    typedef void* (*FnGetFields)(void*,void**);
    typedef const char* (*FnFieldName)(void*);
    typedef int (*FnFieldOffset)(void*);
    FnGetFields get_fields=dlsym(h,"il2cpp_class_get_fields");
    FnFieldName field_name=dlsym(h,"il2cpp_field_get_name");
    FnFieldOffset field_offset=dlsym(h,"il2cpp_field_get_offset");
    if(!get_fields||!field_name||!field_offset) {
        jlog(@"findTimeScaleField: IL2CPP field API not found");
        return;
    }
    void *iter=NULL,*field=NULL;
    while((field=get_fields(timeKlass,&iter))!=NULL){
        const char *fn=field_name(field);
        int foff=field_offset(field);
        if(fn) {
            jlog(@"  Time.%s offset=%d",fn,foff);
            if(strcmp(fn,"s_timeScale")==0 && foff>=0) {
                g_timeScaleOff=foff;
                jlog(@"findTimeScaleField: FOUND s_timeScale offset=%d",foff);
            }
        }
    }
}

// 初始化Time类静态数据区和timeScale偏移
static void initTimeScale(void) {
    if(g_timeClassSD && g_timeScaleOff>=0) {
        jlog(@"initTimeScale: already initialized (SD=%p off=%d)",g_timeClassSD,g_timeScaleOff);
        return;
    }
    void *h=dlopen(NULL,RTLD_LAZY);
    if(!h) { jlog(@"initTimeScale: dlopen failed"); return; }
    typedef void* (*FnStaticData)(void*);
    FnStaticData get_sd=dlsym(h,"il2cpp_class_get_static_field_data");
    if(!get_sd) { jlog(@"initTimeScale: il2cpp_class_get_static_field_data not found"); return; }
    if(!g_timeKlass) {
        jlog(@"initTimeScale: Time klass not found, cannot init");
        return;
    }
    
    jlog(@"initTimeScale: calling get_sd(TimeKlass=%p)...", g_timeKlass);
    g_timeClassSD=get_sd(g_timeKlass);
    jlog(@"initTimeScale: Time SD=%p",g_timeClassSD);
    if(!g_timeClassSD) { jlog(@"initTimeScale: SD is NULL, cannot write timeScale"); return; }

    // dump前64字节诊断
    jlog(@"initTimeScale: dumping Time SD (first 64 bytes):");
    for(int i=0;i<16;i++){
        float fv=0; memcpy(&fv,(uint8_t*)g_timeClassSD+i*4,4);
        int32_t iv=0; memcpy(&iv,(uint8_t*)g_timeClassSD+i*4,4);
        jlog(@"  SD[+0x%02x] float=%.6f int=%d (0x%08x)",i*4,fv,iv,iv);
    }

    // 先通过field API找偏移
    findTimeScaleFieldOffset(g_timeKlass);

    // 如果field API没找到, 暴力搜索float==1.0
    if(g_timeScaleOff<0) {
        jlog(@"initTimeScale: field API failed, brute-force searching float==1.0...");
        for(int off=0;off<128;off+=4){
            float fv=0; memcpy(&fv,(uint8_t*)g_timeClassSD+off,4);
            if(fv==1.0f) {
                jlog(@"  found float 1.0 at SD+0x%x",off);
                if(g_timeScaleOff<0) g_timeScaleOff=off;
            }
        }
        if(g_timeScaleOff>=0) {
            jlog(@"initTimeScale: using brute-force offset +0x%x",g_timeScaleOff);
        }
    }
    if(g_timeScaleOff>=0) {
        jlog(@"initTimeScale: timeScale offset=%d, ready to write",g_timeScaleOff);
    } else {
        jlog(@"initTimeScale: FAILED to find timeScale offset, speed feature disabled");
    }
}

// 写入timeScale值
static void writeTimeScale(float val) {
    if(!g_timeClassSD || g_timeScaleOff<0) return;
    memcpy((uint8_t*)g_timeClassSD+g_timeScaleOff, &val, 4);
}

// 读取当前timeScale值
static float readTimeScale(void) {
    if(!g_timeClassSD || g_timeScaleOff<0) return 1.0f;
    float val=1.0f;
    memcpy(&val, (uint8_t*)g_timeClassSD+g_timeScaleOff, 4);
    return val;
}

// 启动速度定时器
static void startSpeedTimer(void) {
    if(g_speedTimer) return;
    if(!g_timeClassSD || g_timeScaleOff<0) return;
    g_speedTimer=dispatch_source_create(DISPATCH_SOURCE_TYPE_TIMER,0,0,dispatch_get_main_queue());
    dispatch_source_set_timer(g_speedTimer,dispatch_time(DISPATCH_TIME_NOW,0),500*NSEC_PER_MSEC,100*NSEC_PER_MSEC);
    dispatch_source_set_event_handler(g_speedTimer,^{
        if(g_gameSpeed>1.0f && g_timeClassSD && g_timeScaleOff>=0) {
            float cur=readTimeScale();
            if(cur!=g_gameSpeed) {
                writeTimeScale(g_gameSpeed);
            }
        }
    });
    dispatch_resume(g_speedTimer);
    jlog(@"speedTimer: started (0.5s interval)");
}

// 停止速度定时器
static void stopSpeedTimer(void) {
    if(g_speedTimer) {
        dispatch_source_cancel(g_speedTimer);
        g_speedTimer=NULL;
        jlog(@"speedTimer: stopped");
    }
}
...
// v78: MoveStep hook removed (帧同步游戏效果有限, 改用timeScale)
...
// v78: hMoveStep removed, using timeScale instead
...
static void findIL2CPP(void) {
    ...
    for(size_t a=0;a<assemCount;a++){
        ...
        for(size_t c=0;c<cnt;c++){
            ...
            // v78: 搜索UnityEngine.Time类
            if(cn&&strcmp(cn,"Time")==0&&ns&&strcmp(ns,"UnityEngine")==0&&!g_timeKlass){
                g_timeKlass=klass; jlog(@"FOUND UnityEngine.Time=%p",klass);
            }
            ...
            while((m=get_methods(klass,&iter))!=NULL){
                ...
                // v78: MoveStep removed, using timeScale instead
            }
        }
    }
    ...
    if(!g_classHotfixGameEntry){
        for(int i=0;i<g_gameEntryCount;i++){
            if(g_allGameEntryNS[i]==NULL||strcmp(g_allGameEntryNS[i],"")==0){
                ...
                jlog(@"v78: assume GameEntry[%d] ns=empty is HotfixGameEntry=%p",i,g_allGameEntries[i]);
                break;
            }
        }
    }
    jlog(@"v78: UnityGameEntry=%p HotfixGameEntry=%p TimeKlass=%p",g_classUnityGameEntry,g_classHotfixGameEntry,g_timeKlass);
...
-(void)speedSliderChanged:(UISlider*)s{
    ...
    if(g_gameSpeed>1.0f) {
        if(g_timeClassSD && g_timeScaleOff>=0) {
            ...
            jlog(@"speedSlider: timeScale not initialized yet, will apply when ready");
        }
    } else {
        stopSpeedTimer();
        if(g_timeClassSD && g_timeScaleOff>=0) writeTimeScale(1.0f);
    }
    g_speedLabel.text=[NSString stringWithFormat:@"游戏速度: %.1fx",g_gameSpeed];
    jlog(@"GameSpeed: %.1f (timeScale memory write)",g_gameSpeed);
}
-(void)onFullScreen{
...
static void setupUI(void){
    ...
    UILabel *tl=[[UILabel alloc]initWithFrame:CGRectMake(10,6,g_panelW-20,20)];tl.text=@"\xe5\x89\x91\xe5\xbd\xb1\xe6\xb1\x9f\xe6\xb9\x96 v78";tl.textColor=IMGUI_ACCENT;tl.font=[UIFont boldSystemFontOfSize:15];tl.textAlignment=NSTextAlignmentCenter;[tb addSubview:tl];
    ...
    CGFloat spy=s3Y+4;
    g_speedLabel=[[UILabel alloc]initWithFrame:CGRectMake(bx,spy,bw,18)];g_speedLabel.text=[NSString stringWithFormat:@"游戏速度: %.1fx",g_gameSpeed];g_speedLabel.textColor=IMGUI_ACCENT;g_speedLabel.font=[UIFont boldSystemFontOfSize:12];[sv addSubview:g_speedLabel];
    g_speedSlider=[[UISlider alloc]initWithFrame:CGRectMake(bx,spy+20,bw,28)];g_speedSlider.minimumValue=1.0;g_speedSlider.maximumValue=5.0;g_speedSlider.value=g_gameSpeed;[g_speedSlider addTarget:[JYJHActionHandler shared] action:@selector(speedSliderChanged:) forControlEvents:UIControlEventValueChanged];[sv addSubview:g_speedSlider];
...
static void initialize(void){
    ...
    jlog(@"========== JYJH v78 (safe timeScale) ==========");
    jlog(@"iOS %@",[[UIDevice currentDevice] systemVersion]);
    // v78: 无反检测, Shadow处理越狱隐藏; timeScale延迟8s安全初始化
    // 2s: findIL2CPP+applyAllHooks, 5s: setupUI, 8s: initTimeScale
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW,(int64_t)(2.0*NSEC_PER_SEC)),dispatch_get_main_queue(),^{
        jlog(@"2s: finding IL2CPP methods...");
        findIL2CPP();
        applyAllHooks();
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW,(int64_t)(3.0*NSEC_PER_SEC)),dispatch_get_main_queue(),^{
            jlog(@"5s: setting up UI...");
            setupUI();
            // v78: 延迟到8s初始化timeScale (游戏完全加载后)
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW,(int64_t)(3.0*NSEC_PER_SEC)),dispatch_get_main_queue(),^{
                jlog(@"8s: initializing timeScale...");
                @try {
                    initTimeScale();
                } @catch(NSException *e) {
                    jlog(@"8s: initTimeScale EXCEPTION: %@",e);
                }
            });
        });
    });
