/**
 * v62.0 - 皮肤扫描换用UnityGameFramework.GameEntry.GetComponent + 大招放弃帧同步Hook + HitSystem偏移修正
 * v61测试:
 *   1. 皮肤扫描: HotfixFramework.GameEntry._config静态字段在0xa8位置为0, 说明该类延迟初始化
 *      → 发现真正的GameEntry是UnityGameFramework.Runtime.GameEntry, 有GetComponent(string)方法
 *      → v62: 直接调用GetComponent("ConfigComponent")获取实例(用il2cpp_runtime_invoke)
 *   2. 大招增强: TryTriggerExSkill+IsExSkillInCD Hook成功但从未触发(0调用)
 *      → 帧同步游戏中ExSkill由服务端帧数据驱动, 本地Hook无法干预
 *      → v62: 移除大招Hook(无效), 按钮改为"大招无CD(仅本地CD重置)"
 *      → 换Hook AttackSystem.UpdatePlayerCD → 让大招CD永远为0(本地逻辑)
 *   3. HitSystem: dump终于有数据!
 *      [+0x38]=-296167 = FP Center.X (负值,场景坐标)
 *      [+0x40]=148003  = FP Center.Y
 *      [+0x48]=18994   = FP Extents.X
 *      [+0x50]=65500   = FP Extents.Y
 *      → collBound确实在0x38, Extents在0x48/0x50! v61代码写对了但没生效因为只在非0时修改
 *      → v62: 去掉非0检查(这些值本来就是有效的FP小数), 直接修改
 */
...
static BOOL g_ignoreUnlock=NO, g_exSkillNoCD=NO, g_godMode=NO, g_fullScreen=NO, g_exSkillLocalCD=NO;
...
// v62: 大招CD本地重置 - Hook AttackSystem.UpdatePlayerCD
// AttackSystem.UpdatePlayerCD(Frame, EntityRef, CharacterFiled*, PlayerInfo*)
typedef void (*UpdatePlayerCDFunc)(void*,void*,void*,void*);
static void *g_fUpdatePlayerCD=NULL; static UpdatePlayerCDFunc g_oUpdatePlayerCD=NULL; static BOOL g_hUpdatePlayerCD=NO;
static int g_updateCDLC=0;
static void hUpdatePlayerCD(void *f,void *entRef,void *cf,void *pi) {
    if(g_exSkillLocalCD && g_updateCDLC<30) {
        g_updateCDLC++;
...
// v62: 皮肤扫描 - 用il2cpp_runtime_invoke调用UnityGameFramework.GameEntry.GetComponent(string)获取ConfigComponent实例
// UnityGameFramework.Runtime.GameEntry.GetComponent(System.String typeName) at 0x6e6158c
static void *g_classUnityGameEntry=NULL;
static void *g_methodGetComponentString=NULL;
...
// v62: 通过runtime_invoke调用GetComponent获取ConfigComponent
static void *getConfigCompViaRuntimeInvoke(void) {
    if(!g_classUnityGameEntry || !g_methodGetComponentString) {
        jlog(@"getConfigComp: class=%p method=%p (missing)", g_classUnityGameEntry, g_methodGetComponentString);
        return NULL;
    }
    ...
    // 创建IL2CPP字符串 "ConfigComponent"
    typedef void* (*Il2CppStringNew)(const char*);
    Il2CppStringNew string_new=dlsym(h,"il2cpp_string_new");
    if(!string_new) {
        jlog(@"il2cpp_string_new not found");
        return NULL;
    }
    void *strParam=string_new("ConfigComponent");
    jlog(@"Created IL2CPP string 'ConfigComponent' = %p", strParam);

    // il2cpp_runtime_invoke(methodInfo, obj, params, exception)
    // GetComponent是static方法, obj=NULL
    typedef void* (*Il2CppRuntimeInvoke)(void*,void*,void**,void**);
    Il2CppRuntimeInvoke runtime_invoke=dlsym(h,"il2cpp_runtime_invoke");
    if(!runtime_invoke) {
        jlog(@"il2cpp_runtime_invoke not found");
        return NULL;
    }

    void *params[1]={strParam};
    void *exception=NULL;
    void *result=runtime_invoke(g_methodGetComponentString, NULL, params, &exception);
    if(exception) {
        jlog(@"GetComponent runtime_invoke exception: %p", exception);
        return NULL;
    }
    jlog(@"GetComponent('ConfigComponent') result=%p", result);
    return result;
}
...
static void scanSkinIds(void) {
    ...
    void *cc=getConfigCompViaRuntimeInvoke();
    if(!isValidPtr(cc)){
        jlog(@"ScanSkin: ConfigComponent still NULL after runtime_invoke");
        g_skinIdsLoaded=NO; // allow retry
        return;
    }
...
static void findIL2CPP(void) {
    ...
    for(size_t a=0;a<assemCount;a++){
        ...
        for(size_t c=0;c<cnt;c++){
            ...
            // v62: 搜索UnityGameFramework.Runtime.GameEntry (有GetComponent)
            ...
            while((m=get_methods(klass,&iter))!=NULL){
                ...
                // v62: 搜索UnityGameEntry.GetComponent(string) - 1参
                else if(strcmp(n,"GetComponent")==0&&pc==1&&cn&&strcmp(cn,"GameEntry")==0&&ns&&strcmp(ns,"UnityGameFramework.Runtime")==0&&!g_methodGetComponentString){
                    g_methodGetComponentString=fa;found++;jlog(@"FOUND %s.%s p=%u %p [皮肤扫描关键]",cn,n,pc,fa);
                }
                // v62: 搜索AttackSystem.UpdatePlayerCD (4参)
                else if(strcmp(n,"UpdatePlayerCD")==0&&pc==4&&cn&&strcmp(cn,"AttackSystem")==0&&!g_fUpdatePlayerCD){
                    g_fUpdatePlayerCD=fa;found++;jlog(@"FOUND %s.%s p=%u %p [大招CD]",cn,n,pc,fa);
                }
            }
        }
    }
...
static UIButton *g_btnIgnoreUnlock=nil,*g_btnExSkillNoCD=nil,*g_btnGodMode=nil,*g_btnFullScreen=nil,*g_btnApplySkin=nil,*g_btnScanSkin=nil,*g_btnExSkillLocalCD=nil;
...
-(void)onDumpSkinIds;-(void)onExSkillLocalCD;
@end
...
static void refreshBtns(void){
    ...
    // v62: 大招CD本地重置按钮
    if(g_exSkillLocalCD){[g_btnExSkillLocalCD setTitle:@"ON  \xe5\xa4\xa7\xe6\x8b\x9b\xe6\x97" "\xa0" "CD" forState:UIControlStateNormal];g_btnExSkillLocalCD.backgroundColor=IMGUI_BTN_ON;g_btnExSkillLocalCD.layer.borderColor=IMGUI_GREEN.CGColor;}
    else{[g_btnExSkillLocalCD setTitle:@"OFF \xe5\xa4\xa7\xe6\x8b\x9b\xe6\x97" "\xa0" "CD" forState:UIControlStateNormal];g_btnExSkillLocalCD.backgroundColor=IMGUI_BTN_OFF;g_btnExSkillLocalCD.layer.borderColor=IMGUI_RED.CGColor;}
...
-(void)onExSkillLocalCD{
    g_exSkillLocalCD=!g_exSkillLocalCD;
    if(g_exSkillLocalCD){findIL2CPP();
        if(!g_hUpdatePlayerCD && g_fUpdatePlayerCD)
            hookOneFunc(g_fUpdatePlayerCD,hUpdatePlayerCD,(void**)&g_oUpdatePlayerCD,&g_hUpdatePlayerCD,"UpdatePlayerCD");
    }
    refreshBtns();jlog(@"Toggle ExSkillLocalCD: %d func=%p hooked=%d",g_exSkillLocalCD,g_fUpdatePlayerCD,g_hUpdatePlayerCD);
}
...
-(void)onDumpSkinIds{
    if(!g_classUnityGameEntry||!g_classConfigComponent){jlog(@"DumpSkin: classes not found, running findIL2CPP...");findIL2CPP();}
    if(g_skinIdsLoaded){jlog(@"DumpSkin: already scanned, role=%d weapon=%d",g_roleSkinCount,g_weaponSkinCount);refreshBtns();return;}
    jlog(@"DumpSkin: starting (via runtime_invoke GetComponent)...");
    scanSkinIds();
    refreshBtns();
}
@end
...
static void setupUI(void){
    ...
    UILabel *tl=[[UILabel alloc]initWithFrame:CGRectMake(10,6,g_panelW-20,20)];tl.text=@"\xe5\x89\x91\xe5\xbd\xb1\xe6\xb1\x9f\xe6\xb9\x96 v62.0";tl.textColor=IMGUI_ACCENT;tl.font=[UIFont boldSystemFontOfSize:15];tl.textAlignment=NSTextAlignmentCenter;[tb addSubview:tl];
    ...
    g_btnExSkillLocalCD=mkBtn(CGRectMake(bx,by0+bdy*4,bw,bh),@selector(onExSkillLocalCD));[sv addSubview:g_btnExSkillLocalCD];
