/**
 * v54.0 - 修复HitSystem.Update Hook卡死 + 皮肤修改
 * v53问题:
 *   1. 开启秒杀进入战斗场景游戏卡死 - 遍历ComponentDataBuffer逻辑有bug
 *      根因: _componentBuffers是**指针数组,但代码按单指针偏移访问;Block也是指针数组
 *      非HitBox组件的数据被错误解读导致越界/死循环
 *   2. 皮肤修改无效 - 只改了Actor的backing field,没触发UpdatePart刷新模型
 * v54方案:
 *   1. HitSystem.Update Hook简化: 不遍历组件,只在调用原始前后修改HitSystem.collBound
 *      collBound是HitSystem的私有FPBounds2字段(+0x38),Update内部用于碰撞检测
 *      扩大collBound.Extents让碰撞范围覆盖全屏
 *   2. 皮肤修改: Hook Actor.set_SkinId/set_WeaponId, 在setter中写入新值
 *      Actor.SkinId backing field=+0x110, WeaponId=+0x114
 *      同时调用UpdatePart(skinId,weaponId,haloId)刷新模型显示
 *   3. 缩放handle修复沿用v53(window上)
 */
...
// v54: Hook Actor.get_SkinId (0x2ae94bc) 来拦截Actor对象
...
// FPVector2: x(FP,+0x10) + y(FP,+0x18), FP.RawValue is Int64
// So collBound.Extents.x.RawValue at self+0x38+0x20+0x10 = self+0x68
// collBound.Extents.y.RawValue at self+0x38+0x20+0x18 = self+0x70
#define HITSYS_COLLBOUND_OFF 0x38
// FPBounds2内部: 0x10=class ptr, 0x10+Center, 0x10+0x20=Extents
// Center: x.RawValue at +0x10+0x10=+0x20, y.RawValue at +0x10+0x18+0x10=+0x38
// Extents: x.RawValue at +0x20+0x10=+0x30, y.RawValue at +0x20+0x18+0x10=+0x48
// 所以HitSystem.collBound从+0x38开始:
//   collBound本身是struct(FPBounds2),大小=0x10(header)+0x20(center)+0x20(extents)=0x50
//   实际有效偏移: collBound+0x10=Center(FPVector2), collBound+0x30=Extents(FPVector2)
//   Center.x.RawValue = self+0x38+0x10+0x10 = self+0x58
//   Center.y.RawValue = self+0x38+0x10+0x18+0x10 = self+0x70
//   Extents.x.RawValue = self+0x38+0x30+0x10 = self+0x78
//   Extents.y.RawValue = self+0x38+0x30+0x18+0x10 = self+0x90
// 但dump中HitSystem只有一个字段collBound at 0x38, 且HitSystem继承自SystemMainThread
// SystemMainThread继承自SystemBase, 有自己的字段
// 按IL2CPP dump偏移规则, 0x38需要减去0x10才是实际偏移=0x28
// FPBounds2是struct无header,直接嵌套在HitSystem中
// FPBounds2 layout: Center.x(RawValue=Int64), Center.y(RawValue=Int64), 
//                    Extents.x(RawValue=Int64), Extents.y(RawValue=Int64)
// 每个FP是8字节(RawValue), FPVector2是16字节, FPBounds2是32字节
// collBound实际偏移=0x28:
//   Center.x.RawValue = self+0x28
//   Center.y.RawValue = self+0x30
//   Extents.x.RawValue = self+0x38
//   Extents.y.RawValue = self+0x40
// 用dump偏移0x38-0x10=0x28作为collBound起始
#define HITSYS_COLLBOUND_REAL_OFF 0x28
// FP Bounds2: Center(FPVector2=16B) + Extents(FPVector2=16B) = 32B
// Center.x at collBound+0, Center.y at collBound+8
// Extents.x at collBound+16, Extents.y at collBound+24
#define HITSYS_EXTENTS_X_OFF (HITSYS_COLLBOUND_REAL_OFF+16)
#define HITSYS_EXTENTS_Y_OFF (HITSYS_COLLBOUND_REAL_OFF+24)
...
static UIButton *g_btnIgnoreUnlock=nil,*g_btnExSkillNoCD=nil,*g_btnGodMode=nil,*g_btnFullScreen=nil,*g_btnApplySkin=nil;
...
// v54: 应用皮肤修改 - 通过Hook get_SkinId修改返回值 + 直接写backing field
-(void)onApplySkin{
    findIL2CPP();
    // Hook get_SkinId来拦截Actor对象
    ...
    if(g_playerActorObj){
        ...
        jlog(@"ApplySkin: Actor not found yet, getter hook will return new values");
    }
}
-(void)onDumpSkinIds{
    findIL2CPP();
...
static void setupUI(void){
    ...
    UILabel *tl=[[UILabel alloc]initWithFrame:CGRectMake(10,6,g_panelW-20,20)];tl.text=@"\xe5\x89\x91\xe5\xbd\xb1\xe6\xb1\x9f\xe6\xb9\x96 v54.0";tl.textColor=IMGUI_ACCENT;tl.font=[UIFont boldSystemFontOfSize:15];tl.textAlignment=NSTextAlignmentCenter;[tb addSubview:tl];
    // UIScrollView (滚动内容区)
    ...
    CGFloat contentH=by0+bdy*4+60+160; // v54: 增加高度以容纳apply button
