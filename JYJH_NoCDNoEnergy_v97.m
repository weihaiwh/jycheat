/**
 * v97 - 修复mActorData偏移: dump内存找正确偏移 + mLobbyActorData路径
 * v96问题: 
 *   1. mActorData@0x78大多数NULL，有值读到skin=7536705等垃圾 → 偏移错误
 *   2. ScenePlayerData skinId=0 → 服务器不发skinId/weaponId，只有fashionId
 *   3. 大厅里mActorData可能还没初始化(set_SkinId后才有)
 * v97方案:
 *   1. SetData hook中dump avatar实例内存(0x50-0x120)，扫描找mActorData/mLobbyActorData
 *   2. 通过mLobbyActorData(@0xb8)读取LobbyActorData，再读mSceneData
 *   3. 重点: 用LobbyActorData.UpdatePart(0x2bfb050)来应用 - 这是游戏自己的API
 *   4. 不再依赖mActorData偏移，改用mLobbyActorData偏移(0xb8更可靠)
 */
...
static void *g_fUpdatePart=NULL;          // LobbyActorData.UpdatePart(skinId, weaponId, haloId) - 游戏自己的API!
...
// ===== v97: Hook LobbyActorSpineAvatar.SetData 捕获avatar =====
...
static void hSetData(void *mi, void *self, void *lobbyActorData) {
    ...
    // v97: 只对第一个IsSelf的avatar做完整dump
    if(lobbyActorData) {
        BOOL isSelf = NO;
        memcpy(&isSelf, (uint8_t*)lobbyActorData + 0x2b, 1);
        if(isSelf && g_setDataLC < 3) {
            g_setDataLC++;
            jlog(@"=== SetData[IsSelf] DUMP avatar=%p data=%p ===", self, lobbyActorData);
            
            // Dump avatar实例内存 0x50-0xC0 (找mActorData偏移)
            jlog(@"  avatar memory dump (0x50-0xC0):");
            for(int off=0x50; off<0xC0; off+=8) {
                uint64_t v=0; memcpy(&v, (uint8_t*)self+off, 8);
                jlog(@"    +0x%02x = 0x%llx (%lld)", off, v, (int64_t)v);
            }
            
            // Dump mLobbyActorData偏移(0xb8)的数据
            void *mLD = NULL;
            memcpy(&mLD, (uint8_t*)self + 0xb8, 8);
            jlog(@"  self+0xb8(mLobbyActorData?) = %p", mLD);
            
            // Dump lobbyActorData内存 0x10-0x30
            jlog(@"  lobbyActorData memory dump (0x00-0x30):");
            for(int off=0x00; off<0x30; off+=8) {
                uint64_t v=0; memcpy(&v, (uint8_t*)lobbyActorData+off, 8);
                jlog(@"    +0x%02x = 0x%llx", off, v);
            }
            
            // 通过mLobbyActorData(0xb8)验证
            if(isValidPtr(mLD)) {
                jlog(@"  mLobbyActorData memory (0x00-0x30):");
                for(int off=0x00; off<0x30; off+=8) {
                    uint64_t v=0; memcpy(&v, (uint8_t*)mLD+off, 8);
                    jlog(@"    +0x%02x = 0x%llx", off, v);
                }
                // 读mLobbyActorData的mSceneData(0x10)
                void *sd2=NULL; memcpy(&sd2, (uint8_t*)mLD+0x10, 8);
                if(isValidPtr(sd2)) {
                    int32_t s2=0,w2=0,f2=0;
                    memcpy(&s2,(uint8_t*)sd2+0x40,4);
                    memcpy(&w2,(uint8_t*)sd2+0x44,4);
                    memcpy(&f2,(uint8_t*)sd2+0x48,4);
                    jlog(@"  mLobbyActorData->mSceneData: skin=%d weapon=%d fashion=%d", s2, w2, f2);
                }
            }
        }
    }
...
static void findIL2CPP(void) {
    ...
    jlog(@"v97: UpdatePart=%p SetData=%p ChangePart=%p ReInit=%p InitSkin=%p",g_fUpdatePart,g_fSetData,g_fChangePart,g_fReInitWithActorData,g_fInitWithSkinId);
...
// v97: 应用外观 - 用LobbyActorData.UpdatePart(游戏自己的API)
// LobbyActorData.UpdatePart(skinId, weaponId, haloId) @ 0x2bfb050
// 这是游戏用来更新外观的标准方法，应该最可靠
// 同时修改avatar字段作为辅助
static void applyPartChange(void) {
    jlog(@"applyPart: skin=%d weapon=%d halo=%d data=%p avatar=%p", g_appliedSkinId, g_appliedWeaponId, g_appliedHaloId, g_lobbyActorData, g_lobbyAvatarObj);
    
    // 路径1: 用LobbyActorData.UpdatePart - 这是游戏自己的API
    if(g_lobbyActorData && g_fUpdatePart) {
        // 先读当前值
        void *sceneData = NULL;
        memcpy(&sceneData, (uint8_t*)g_lobbyActorData + 0x10, 8);
        int32_t curSkin=0, curWeapon=0, curHalo=0;
        if(isValidPtr(sceneData)) {
            memcpy(&curSkin, (uint8_t*)sceneData + 0x40, 4);
            memcpy(&curWeapon, (uint8_t*)sceneData + 0x44, 4);
            // haloId不在ScenePlayerData标准字段中，但可以尝试其他偏移
        }
        // 从mLobbyActorData(avatar+0xb8)的mSceneData读取
        if(g_lobbyAvatarObj) {
            void *mLD=NULL; memcpy(&mLD, (uint8_t*)g_lobbyAvatarObj+0xb8, 8);
            if(isValidPtr(mLD)) {
                void *sd=NULL; memcpy(&sd, (uint8_t*)mLD+0x10, 8);
                if(isValidPtr(sd)) {
                    memcpy(&curSkin,(uint8_t*)sd+0x40,4);
                    memcpy(&curWeapon,(uint8_t*)sd+0x44,4);
                    jlog(@"applyPart: current skin=%d weapon=%d (from mLobbyActorData)", curSkin, curWeapon);
                }
            }
        }
        
        int32_t useSkin = (g_appliedSkinId > 0) ? g_appliedSkinId : curSkin;
        int32_t useWeapon = (g_appliedWeaponId > 0) ? g_appliedWeaponId : curWeapon;
        int32_t useHalo = g_appliedHaloId; // halo没有当前值可读
        
        // 直接调用UpdatePart - (MethodInfo*, self, skinId, weaponId, haloId)
        typedef void (*UpdatePartFunc)(void*, void*, int32_t, int32_t, int32_t);
        UpdatePartFunc fn = (UpdatePartFunc)g_fUpdatePart;
        jlog(@"applyPart: calling UpdatePart(data=%p, skin=%d, weapon=%d, halo=%d)", g_lobbyActorData, useSkin, useWeapon, useHalo);
        fn(NULL, g_lobbyActorData, useSkin, useWeapon, useHalo);
        jlog(@"applyPart: UpdatePart done!");
        return;
    }
    
    // 路径2: 通过avatar修改+ChangePart
    if(g_lobbyAvatarObj) {
        // 修改Actor字段
        if(g_appliedSkinId > 0) memcpy((uint8_t*)g_lobbyAvatarObj + 0x110, &g_appliedSkinId, 4);
        if(g_appliedWeaponId > 0) memcpy((uint8_t*)g_lobbyAvatarObj + 0x114, &g_appliedWeaponId, 4);
        if(g_appliedHaloId > 0) memcpy((uint8_t*)g_lobbyAvatarObj + 0x118, &g_appliedHaloId, 4);
        jlog(@"applyPart: wrote ActorFields skin=%d weapon=%d halo=%d", g_appliedSkinId, g_appliedWeaponId, g_appliedHaloId);
        
        if(g_fChangePart) {
            typedef void (*ChangePartFunc)(void*, void*, int32_t, int32_t, void*);
            ChangePartFunc fn = (ChangePartFunc)g_fChangePart;
            if(g_appliedSkinId > 0) { fn(NULL, g_lobbyAvatarObj, 2, g_appliedSkinId, NULL); jlog(@"ChangePart Skin done"); }
            if(g_appliedWeaponId > 0) { fn(NULL, g_lobbyAvatarObj, 3, g_appliedWeaponId, NULL); jlog(@"ChangePart Weapon done"); }
            if(g_appliedHaloId > 0) { fn(NULL, g_lobbyAvatarObj, 4, g_appliedHaloId, NULL); jlog(@"ChangePart Halo done"); }
        }
        return;
    }
    
    jlog(@"applyPart: no data/avatar captured yet!");
}
...
static void setupUI(void){
    ...
    UILabel *tl=[[UILabel alloc]initWithFrame:CGRectMake(10,6,g_panelW-20,20)];tl.text=@"剑影江湖 v97";tl.textColor=IMGUI_ACCENT;tl.font=[UIFont boldSystemFontOfSize:15];tl.textAlignment=NSTextAlignmentCenter;[tb addSubview:tl];
...
static void initialize(void){
    ...
    jlog(@"========== JYJH v97 (UpdatePart API + memory dump) ==========");
