/**
 * v46.0 - UIScrollView滚动面板 + Tables实例直接读 + 禁用Damage扩散防卡顿
 * v43问题:
...
static void *g_fGetSpineAni=NULL; // GetCurSpineAnimationInfo (Frame, CharacterFiled*) -> SpineAnimationInfo*
...
// v46: Hook GetCurSpineAnimationInfo to expand hitBound for true full-screen
// SpineAnimationInfo layout: hitBound(FPBounds2,+0x40), hitBounds(List<FPBounds2>,+0x60), HitBoundZ(FP,+0x70)
// FPBounds2 layout: Center(FPVector2,+0x10) + Extents(FPVector2,+0x20)
// FPVector2 layout: x(FP,+0x10) + y(FP,+0x18), FP = Int64 raw value
// To make hitBound cover entire screen: set Extents to very large values
static int64_t g_bigExtent = 500000000LL; // ~7629 in game units (500000000/65536), covers ~15 screen widths
static int g_spineAniLC=0;
typedef void* (*GetSpineAniFunc)(void*,void*);
static GetSpineAniFunc g_oGetSpineAni=NULL;
static void* hGetSpineAni(void *f,void *cf){
    void *result = g_oGetSpineAni ? g_oGetSpineAni(f,cf) : NULL;
    if(g_fullScreen && result){
        // Expand hitBound: SpineAnimationInfo+0x40 = hitBound(FPBounds2)
        // FPBounds2+0x20 = Extents(FPVector2)
        // FPVector2+0x10 = x(FP), +0x18 = y(FP)
        void *hitBound = (uint8_t*)result + 0x40;
        void *extents = (uint8_t*)hitBound + 0x20;
        // Write large extent values (FP raw = value * 65536)
        int64_t bigVal = g_bigExtent;
        memcpy((uint8_t*)extents + 0x10, &bigVal, 8); // x extent
        memcpy((uint8_t*)extents + 0x18, &bigVal, 8); // y extent
        // Also expand HitBoundZ: SpineAnimationInfo+0x70 = HitBoundZ(FP)
        int64_t bigZ = 300000000LL; // ~4578 game units, covers Z axis
        memcpy((uint8_t*)result + 0x70, &bigZ, 8);
        if(g_spineAniLC<20){g_spineAniLC++;jlog(@"SpineAni: expanded hitBound+Z");}
    }
    return result;
}
    ...
    for(size_t a=0;a<assemCount;a++){
        ...
        for(size_t c=0;c<cnt;c++){
            ...
            while((m=get_methods(klass,&iter))!=NULL){
                ...
                else if(strcmp(n,"GetCurSpineAnimationInfo")==0&&pc==2&&!g_fGetSpineAni){g_fGetSpineAni=fa;found++;jlog(@"FOUND %s.%s p=%u %p",cn?:"?",n,pc,fa);}
            }
        }
    }
...
-(void)onFullScreen{
    ...
    if(g_fullScreen){findIL2CPP();
        ...
        // v46: Hook GetCurSpineAnimationInfo to expand hitBound for true full-screen
        if(g_fGetSpineAni&&!g_oGetSpineAni){DobbyHook(g_fGetSpineAni,hGetSpineAni,&g_oGetSpineAni);jlog(@"GetSpineAni: OK at %p orig=%p",g_fGetSpineAni,g_oGetSpineAni);}
        if(g_fullScreen&&!g_oGetSpineAni)jlog(@"WARNING: GetSpineAni not found, hitBound expand not active");
        if(!g_hDamage)hookOneFunc(g_fDamage,hDamage,(void**)&g_oDamage,&g_hDamage,"Damage");
        ...
        jlog(@"FullScreen ON: hitBound expand, enemies=0");
...
static void setupUI(void){
    ...
    // 大面板 - 屏幕居中
    UIView *outer=[[UIView alloc]initWithFrame:CGRectMake(0,0,g_panelW,g_panelH)];outer.backgroundColor=IMGUI_BG;outer.layer.cornerRadius=10;outer.layer.borderWidth=1;outer.layer.borderColor=IMGUI_BORDER.CGColor;outer.hidden=YES;outer.clipsToBounds=YES;g_panel=outer;[win addSubview:outer];
    layoutPanelCenter();
    // Title bar (固定在顶部)
    UIView *tb=[[UIView alloc]initWithFrame:CGRectMake(0,0,g_panelW,32)];tb.backgroundColor=IMGUI_TITLE_BG;[outer addSubview:tb];
    UILabel *tl=[[UILabel alloc]initWithFrame:CGRectMake(10,6,g_panelW-20,20)];tl.text=@"\xe5\x89\x91\xe5\xbd\xb1\xe6\xb1\x9f\xe6\xb9\x96 v46.0";tl.textColor=IMGUI_ACCENT;tl.font=[UIFont boldSystemFontOfSize:15];tl.textAlignment=NSTextAlignmentCenter;[tb addSubview:tl];
    // UIScrollView (滚动内容区)
    UIScrollView *sv=[[UIScrollView alloc]initWithFrame:CGRectMake(0,32,g_panelW,g_panelH-32)];sv.showsVerticalScrollIndicator=YES;sv.delaysContentTouches=NO;sv.canCancelContentTouches=YES;[outer addSubview:sv];
    CGFloat contentH=420;
    sv.contentSize=CGSizeMake(g_panelW,contentH);
    // Buttons
    CGFloat bx=12,bw=g_panelW-24,bh=30,by0=4,bdy=34;
