/**
 * v66.0 - 修复技能替换(不Hook UseSkill,改为大招随时可用) + 皮肤扫描List偏移修复
 * v65测试:
 *   1. 技能替换卡住: UseSkill Hook替换stateType=19导致帧同步不一致,游戏卡死
 *      -> v66: 移除UseSkill Hook! 改为让大招(Skill6)随时可用+无CD
 *      -> 方案: CheckSkillIsReady/CheckSkillAttackCanUse对st>=14返回YES(原st>=17)
 *      -> 同时Hook UpdateSkillCoolDown让CD归零
 *   2. 皮肤扫描RoleSkin=0: arrayLen=0但listSize=206
 *      -> 原因: List<T>泛型dump偏移全0x0不可靠, _items实际偏移可能不是+0x10
 *      -> v66: dump dataList内存暴力搜索有效数组指针, 修复数组长度偏移(+0x18而非+0x10)
 *      -> IL2CPP数组: +0x00=Il2CppObject(16B), +0x10=klass_ptr, +0x18=max_length, +0x20=data[0]
 */
...
// v65: UseSkill类型(保留搜索,不再Hook - 帧同步会卡住)
...
// v66: UseSkill搜索保留(不再Hook,仅日志诊断)
static void *g_fUseSkill=NULL;
    ...
    for(size_t a=0;a<assemCount;a++){
        ...
        for(size_t c=0;c<cnt;c++){
            ...
            while((m=get_methods(klass,&iter))!=NULL){
                ...
                // v66: UseSkill - 仅搜索不Hook(帧同步会卡住)
                else if(strcmp(n,"UseSkill")==0&&pc>=7&&cn&&strcmp(cn,"AttackSystem")==0&&!g_fUseSkill){g_fUseSkill=fa;found++;jlog(@"FOUND %s.%s p=%u %p (search only)",cn,n,pc,fa);}
    ...
    if(!g_classHotfixGameEntry){
        for(int i=0;i<g_gameEntryCount;i++){
            if(g_allGameEntryNS[i]==NULL||strcmp(g_allGameEntryNS[i],"")==0){
                ...
                jlog(@"v66: assume GameEntry[%d] ns=empty is HotfixGameEntry=%p",i,g_allGameEntries[i]);
                break;
            }
        }
    }
    jlog(@"v66: UnityGameEntry=%p HotfixGameEntry=%p",g_classUnityGameEntry,g_classHotfixGameEntry);
...
-(void)onSkillReplace{
    ...
    if(g_skillReplace){findIL2CPP();
        // v66: 不Hook UseSkill(帧同步会卡住), 改为让大招随时可用
        ...
        // Hook IsReady/AttackCanUse让st>=14返回YES(原st>=17只覆盖大招)
        if(!g_hIsReady)hookOneFunc(g_fIsReady,hIsReady,(void**)&g_oIsReady,&g_hIsReady,"IsReady");
        if(!g_hAttackCanUse)hookOneFunc(g_fAttackCanUse,hAttackCanUse,(void**)&g_oAttackCanUse,&g_hAttackCanUse,"AttackCanUse");
        if(!g_hDamage)hookOneFunc(g_fDamage,hDamage,(void**)&g_oDamage,&g_hDamage,"Damage");
    }
...
static void setupUI(void){
    ...
    UILabel *tl=[[UILabel alloc]initWithFrame:CGRectMake(10,6,g_panelW-20,20)];tl.text=@"\xe5\x89\x91\xe5\xbd\xb1\xe6\xb1\x9f\xe6\xb9\x96 v66.0";tl.textColor=IMGUI_ACCENT;tl.font=[UIFont boldSystemFontOfSize:15];tl.textAlignment=NSTextAlignmentCenter;[tb addSubview:tl];
