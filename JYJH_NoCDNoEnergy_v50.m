/**
 * v50.0 - 调用MonsterDeadSignal触发死亡 + 修复UI(面板拖动+下方空隙+右下角可点)
 * v49问题:
 *   1. DecreaseHP只扣血不触发死亡事件 - 敌人空血但不死
 *   2. 面板拖动限制太紧 - 一动就顶到屏幕上边缘
 *   3. 下半部分无空隙 - 右下角resize把手无法点击
 *   4. 皮肤修改无效 - ID范围可能不对或时机不对
 * v50方案:
 *   1. 扩散后调用MonsterDeadSignal(Frame,entity,atk,DeathEnum=0)触发死亡
 *   2. 面板拖动限制放宽: y∈[50,screenH-100]留出空间
 *   3. ScrollView高度-50留出底部空隙,contentH-50确保右下角可见
 *   4. 暂时禁用皮肤功能(需要在战斗中Hook Actor实例才能生效)
 */
...
static int64_t hDamage(void *f,void *atkEnt,void *atkCF,void *tgtEnt,void *tgtCF,
    ...
    if(atkP&&r>0){
        if(g_dmgLC<20){g_dmgLC++;jlog(@"Dmg[%d]=%lld enemies=%d decHP=%p mDead=%p",g_dmgLC,r,g_enemyCount,g_origDecreaseHP,g_fMonsterDead);}
        // v50: 扩散后调用MonsterDeadSignal触发死亡
        if(g_fullScreen&&g_origDecreaseHP&&g_enemyCount>0){
            ...
            for(int i=0;i<g_enemyCount;i++){
                ...
                // v50: 调用MonsterDeadSignal触发死亡事件
                if(g_fMonsterDead){
                    MonsterDeadSignalFunc deadFunc=(MonsterDeadSignalFunc)g_fMonsterDead;
                    deadFunc(f,eEnt,atkEnt,atkEnt,0); // DeathEnum=0
                    if(g_fullDmgLC<10){jlog(@"FullDmg[%d]: DecHP enemy %d dmg=%lld + DeadSignal",g_fullDmgLC+1,i,spreadDmg);}
                }else{
                    if(g_fullDmgLC<10){jlog(@"FullDmg[%d]: DecHP enemy %d dmg=%lld (no DeadSignal)",g_fullDmgLC+1,i,spreadDmg);}
                }
                spreadCount++;
            }
...
-(void)touchesMoved:(NSSet*)t withEvent:(UIEvent*)e{
    ...
    // v47: 同步更新ScrollView的frame
    if(g_scrollView){
        // v50: 留出底部空隙50px
        g_scrollView.frame=CGRectMake(0,32,nw,nh-82);
        // contentSize宽度也要更新
...
-(void)touchesMoved:(NSSet*)t withEvent:(UIEvent*)e{
    ...
    // v50: 放宽Y限制,留出底部空间
    f.origin.y=MAX(50,MIN(sc.size.height-100,f.origin.y+dy));
    g_panel.frame=f;
    _ts=c;
}
@end
...
static void setupUI(void){
    ...
    UILabel *tl=[[UILabel alloc]initWithFrame:CGRectMake(10,6,g_panelW-20,20)];tl.text=@"\xe5\x89\x91\xe5\xbd\xb1\xe6\xb1\x9f\xe6\xb9\x96 v50.0";tl.textColor=IMGUI_ACCENT;tl.font=[UIFont boldSystemFontOfSize:15];tl.textAlignment=NSTextAlignmentCenter;[tb addSubview:tl];
    // UIScrollView (滚动内容区, v50: 留出底部空隙)
    UIScrollView *sv=[[UIScrollView alloc]initWithFrame:CGRectMake(0,32,g_panelW,g_panelH-82)];
    ...
    [outer addSubview:sv];
    g_scrollView=sv; // v47: 保存引用用于resize
    // v49: 减小按钮大小和间距
    CGFloat bx=12,bw=g_panelW-24,bh=24,by0=4,bdy=28;
    CGFloat contentH=by0+bdy*4+60+120-50; // v50: 减小contentH留出底部空隙
