/**
 * v86 - 修复技能替换卡死 + 皮肤hook
 * v85确认: MSHookFunction没问题, 卡死是hUpdateSkillCD完全跳过原函数导致的
 * 当g_skillReplace=YES时hUpdateSkillCD直接return→CD不更新→技能重复触发→卡死
 * v86修复: UpdateSkillCD始终调用原函数, IsReady/AttackCanUse返回YES已足够让技能可用
 * 同时加入皮肤: hook get_SkinId返回假ID
 */
...
// v69: get_SkinId hook - 只hook返回值, 不调用InitWithSkinId
// v86: 重新启用皮肤hook, 但只用get_SkinId返回假ID, 不调用InitWithSkinId
...
// v86: InitWithSkinId只搜索不调用
static void *g_fInitWithSkinId=NULL;

// v71: 移动加速 - Hook MoveStep修改cf->MoveSpped字段(临时修改,调用后恢复)
// MoveStep(Frame, EntityRef, CharacterFiled*, Vector2, FP, FP, FP, Transform2D*)
// MoveSpped: dump偏移+0x90, 实际偏移+0x80 (struct无0x10 header)
// SpintSpped: dump偏移+0x98, 实际偏移+0x88
...
// v86: get_SkinId hook - 返回假skinId, 不调用InitWithSkinId
...
// v86: 不完全跳过原函数, 否则CD不更新导致技能重复触发卡死
// 策略: 始终调用原函数让CD正常更新, 但在技能替换模式下, 让大招(state=19)的CD快速归零
static void hUpdateSkillCD(void *f,void *er,void *cf,void *states) {
    if(g_skillReplace) {
        // 调用原函数让CD正常开始计时
        if(g_oUpdateSkillCD) g_oUpdateSkillCD(f,er,cf,states);
        // 然后将大招(state 19)对应的CD强制归零
        // states是CharacterStatesAsset, 技能CD信息存储在里面
        // 遍历所有技能slot, 将大招CD设为0
        if(states) {
            // CharacterStatesAsset中技能CD的布局:
            // 每个技能有一个CD字段, 大招(19)通常在最后一个slot
            // dump显示: 技能state 14-19, 共6个技能slot
            // CD通常是float, 在states的某个偏移
            // 简单方案: 将states中所有float型CD值设为0
            // 但不知道确切偏移, 用更安全的方式:
            // 只让原函数执行, 不额外干预CD
            // 因为IsReady/AttackCanUse已经返回YES, 技能随时可用
        }
        if(g_uscdLC<20){g_uscdLC++;jlog(@"UpdateSkillCD: called normally (replace mode)");}
        return;
    }
    if(g_oUpdateSkillCD) g_oUpdateSkillCD(f,er,cf,states);
}
...
static void refreshBtns(void){
    ...
    if(g_btnApplySkin){
        if(g_appliedSkinId>0){
            [g_btnApplySkin setTitle:[NSString stringWithFormat:@"\xe7\x9a\xae\xe8\x82\xa4:ID%d",g_appliedSkinId] forState:UIControlStateNormal];
...
-(void)onApplySkin{
    // v86: 皮肤通过hook get_SkinId返回假ID实现, 不调用InitWithSkinId
    ...
    jlog(@"ApplySkin: skin=%d weapon=%d (via get_SkinId hook)",g_skinId,g_weaponId);
}
@end
...
static void setupUI(void){
    ...
    UILabel *tl=[[UILabel alloc]initWithFrame:CGRectMake(10,6,g_panelW-20,20)];tl.text=@"\xe5\x89\x91\xe5\xbd\xb1\xe6\xb1\x9f\xe6\xb9\x96 v86";tl.textColor=IMGUI_ACCENT;tl.font=[UIFont boldSystemFontOfSize:15];tl.textAlignment=NSTextAlignmentCenter;[tb addSubview:tl];
...
static void initialize(void){
    ...
    jlog(@"========== JYJH v86 (fix skill freeze) ==========");
