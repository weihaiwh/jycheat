/**
 * 剑影江湖 v35.0 - 修复isPlayerCF误判 + 玩家不死
 *
 * v34致命BUG: isPlayerCF检查0x54==0, 但0x54是padding(全0) → 把怪物当玩家!
 *   → 怪物被设Invincible+99999999血 → 怪物无敌, 玩家被秒杀
 *   → HP乱写导致退图卡住
 *
 * v34日志分析:
 *   CFDump[1] cf=0x169afcc40 isAI44=1 isAI54=0 isAI64=-2147483647
 *   isAI44=1 → 真正的IsAI.Value=1(AI怪物)
 *   isAI54=0 → padding字节, 恰好为0, 被误判为玩家
 *   所有CF都是isAI44=1 → 全是怪物, 玩家CF从未出现
 *
 * 偏移确认(IL2CPP struct: 实际偏移=dump偏移-0x10):
 *   IsAI(QBoolean) dump=0x54 → 实际CF+0x44
 *   QBoolean.Value dump=0x10 → QBoolean内实际+0x00
 *   → IsAI.Value在CF+0x44+0x00=CF+0x44
 *   CF+0x44=1 → AI; CF+0x44=0 → 玩家
 *
 * v35修复:
 *   1. isPlayerCF: 只检查CF+0x44==0(不检查0x54/0x64)
 *   2. limitDamage默认1(玩家受1点伤, 撑到学到CF)
 *   3. 只对IsAI==0的CF设置GodMode(绝不碰怪物)
 *   4. HP偏移精简: 只写RealAttr的cur_hp/hp(dump-0x10)
 */
...
static int g_damageLimit = 1;  // v35: 默认1, 玩家受1点伤撑到学到CF
...
// v35: 只检查CF+0x44==0 (IsAI.Value的真实偏移)
...
static BOOL isPlayerCF(void *cf) {
    ...
    return (isAI == 0);  // v35: 只检查这一个偏移
}

// v35: 精简HP偏移, 只写RealAttr
// RealAttr dump=0x2b0 → 实际CF+0x2a0
// RealAttr.cur_hp dump=0x98 → 实际RealAttr+0x88 → CF+0x328
// RealAttr.hp dump=0xa0 → 实际RealAttr+0x90 → CF+0x330
...
static void makePlayerGodMode(void *cf) {
    ...
    memcpy((uint8_t*)cf + 0x328, &hugeHp, 8);  // RealAttr.cur_hp
    memcpy((uint8_t*)cf + 0x330, &hugeHp, 8);  // RealAttr.hp(max)
}
...
static BOOL hookCheckSkillIsReady(void *frame, int stateType, void *characterField, void *states) {
    if (g_godMode && characterField) {
        ...
        if (isPlayerCF(characterField)) {
            ...
            if (g_godModeLogCount < 10) { g_godModeLogCount++; jlog(@"GodMode[%d] 玩家无敌+满血", g_godModeLogCount); }
        }
    }
...
static void findIL2CPP(void) {
    jlog(@"=== v35.0 IL2CPP Runtime Search ===");
...
- (void)onGodMode {
    ...
    if (g_godMode) {
        ...
        startGodTimer();
        jlog(@"GodMode启动: v35只检查isAI44==0(不检查0x54/0x64)");
    }
...
static void setupUI(void) {
    ...
    title.text=@"\u5251\u5f71\u6c5f\u6e56 v35.0"; title.textColor=[UIColor cyanColor];
...
static void initialize(void) {
    ...
    if (loaded) return;
    loaded = YES;
    jlog(@"========== JYJH v35.0 (修复isPlayerCF误判) ==========");
    jlog(@"iOS %@", [[UIDevice currentDevice] systemVersion]);
    jlog(@"v35修复: isPlayerCF只检查CF+0x44==0(v34误判0x54padding为IsAI)");
    jlog(@"v35: limitDamage默认1(玩家受1点伤撑到学到CF), 只对IsAI==0设GodMode");
