/**
 * v105 - SSO bypass: 直接调用get_Instance native地址
 * v104问题:
 *   1. il2cpp_compile_method找不到get_Instance — 泛型方法不在子类方法列表中
 *   2. LoginModel没有instance静态字段 — Singleton实例在Nested<T>内部类中
 *   3. LoginCtrl.get_Instance同样找不到
 * v105方案:
 *   1. 直接计算Singleton<T>.get_Instance()的native地址并调用
 *      dump中地址0x27ffe7c, 运行时需加ASLR slide
 *   2. 备选: 遍历所有assembly的所有类, 找Nested类读instance字段
 *   3. 保留DoHandShakeCheck hook + 定时内存修补
 *   4. CurSSOResponData运行时offset=240(0xf0)已确认正确
 */
...
// v105: 保留g_fGetCode/g_fGetBanTime用于搜索, 但不再MSHookFunction它们
...
// v105: 直接内存修改SSOResponData (备用方案, 声明在前, 定义在isValidPtr/getObjClassName之后)
...
// v105: 直接内存修改SSOResponData (实现)
// v104问题: get_Instance不在LoginModel方法列表中(泛型继承), field_static_get_value没有instance字段
// v105方案:
//   1. 用ASLR slide计算Singleton<T>.get_Instance()的运行时地址(基址0x27ffe7c), 直接调用
//   2. 备选: 遍历所有Nested类找instance字段
//   3. 备选: 通过LoginCtrl.get_Instance -> mModel
static void patchSSOResponDataMemory(void) {
    ...
    // 方案1: 直接调用Singleton<T>.get_Instance()的native地址
    // IL2CPP dump中 Singleton<T>.get_Instance() @ 0x27ffe7c
    // 运行时地址 = dump_offset + ASLR_slide
    // ASLR slide = runtime_addr - dump_offset (用已知的DoHandShakeCheck计算)
    if(g_fDoHandShakeCheck) {
        intptr_t slide=(intptr_t)g_fDoHandShakeCheck - 0x2b7eb2c;
        jlog(@"SSO: ASLR slide=%td (DoHandShakeCheck=%p dump=0x2b7eb2c)",slide,g_fDoHandShakeCheck);

        void *getInstanceAddr=(void*)(0x27ffe7c + slide);
        jlog(@"SSO: get_Instance calc addr=%p",getInstanceAddr);

        if(isValidPtr(getInstanceAddr)) {
            // 尝试多种调用方式 (IL2CPP静态方法可能需要MethodInfo*参数)
            typedef void* (*GIFn0)(void);
            typedef void* (*GIFn1)(void*);
            loginModelInstance=((GIFn0)getInstanceAddr)();
            jlog(@"SSO: get_Instance() [0 args]=%p",loginModelInstance);
            if(!isValidPtr(loginModelInstance)||!getObjClassName(loginModelInstance)||strcmp(getObjClassName(loginModelInstance),"LoginModel")!=0) {
                loginModelInstance=((GIFn1)getInstanceAddr)(NULL);
                jlog(@"SSO: get_Instance(NULL) [1 arg]=%p",loginModelInstance);
            }
            if(!isValidPtr(loginModelInstance)||!getObjClassName(loginModelInstance)||strcmp(getObjClassName(loginModelInstance),"LoginModel")!=0) {
                loginModelInstance=((GIFn1)getInstanceAddr)(g_classLoginModel);
                jlog(@"SSO: get_Instance(klass) [1 arg]=%p",loginModelInstance);
            }
            if(isValidPtr(loginModelInstance)) {
                ...
                if(cn && strcmp(cn,"LoginModel")==0) {
                    ...
                    jlog(@"SSO: get_Instance returned class=%s (not LoginModel)",cn?cn:"null");
    ...
    if(!isValidPtr(loginModelInstance)) {
        jlog(@"SSO: get_Instance failed, trying Nested class scan...");
    ...
    // 方案3: 通过LoginCtrl.get_Instance -> mModel(offset 0x10)
    if(!isValidPtr(loginModelInstance) && g_classLoginCtrl && g_fDoHandShakeCheck) {
        jlog(@"SSO: Trying LoginCtrl.get_Instance...");
        intptr_t slide=(intptr_t)g_fDoHandShakeCheck - 0x2b7eb2c;
        void *getInstanceAddr=(void*)(0x27ffe7c + slide);
        if(isValidPtr(getInstanceAddr)) {
            typedef void* (*GIFn1)(void*);
            void *loginCtrlInstance=((GIFn1)getInstanceAddr)(NULL);
            jlog(@"SSO: LoginCtrl get_Instance(NULL)=%p",loginCtrlInstance);
            if(!isValidPtr(loginCtrlInstance)) {
                loginCtrlInstance=((GIFn1)getInstanceAddr)(g_classLoginCtrl);
                jlog(@"SSO: LoginCtrl get_Instance(klass)=%p",loginCtrlInstance);
            }
            if(isValidPtr(loginCtrlInstance)) {
                const char *vCn=getObjClassName(loginCtrlInstance);
                jlog(@"SSO: LoginCtrl class=%s",vCn?vCn:"null");
                if(vCn && strcmp(vCn,"LoginCtrl")==0) {
                    memcpy(&loginModelInstance,(uint8_t*)loginCtrlInstance+0x10,8);
    ...
    // LoginModel.CurSSOResponData @ offset 0xf0 (运行时已确认=240)
...
static void findIL2CPP(void) {
    ...
    for(size_t a=0;a<assemCount;a++){
        ...
        for(size_t c=0;c<cnt;c++){
            ...
            // v105: 搜索LoginModel类 (Singleton, 保存CurSSOResponData)
            ...
            // v105: 搜索LoginCtrl类 (有DoHandShakeCheck方法)
            ...
            while((m=get_methods(klass,&iter))!=NULL){
                ...
                // v105: LoginCtrl.DoHandShakeCheck(int32 code) — 唯一地址0x2b7eb2c, 不被泛型共享
                ...
                // v102→v105: 保留get_code/get_banTime搜索仅用于日志确认, 不再MSHookFunction它们
    ...
    // v105: 用il2cpp_class_from_name搜索热更新DLL中的LoginModel/LoginCtrl/DoHandShakeCheck
            ...
            for(size_t a=0;a<assemCount;a++){
                ...
                if(!g_classLoginModel) {
                    ...
                    if(lmClass) {
                        ...
                        jlog(@"v105: Found LoginModel class in assembly %zu: %p", a, lmClass);
                    }
                }
                ...
                if(!g_fDoHandShakeCheck) {
                    ...
                    if(lcClass) {
                        ...
                        jlog(@"v105: Found LoginCtrl class in assembly %zu: %p", a, lcClass);
                ...
                if(!g_fGetCode) {
                    ...
                    if(ssoRspClass) {
                        jlog(@"v105: Found SSOResponData class: %p", ssoRspClass);
    ...
    jlog(@"v105: DoHandShakeCheck=%p LoginModel=%p LoginCtrl=%p",
         g_fDoHandShakeCheck, g_classLoginModel, g_classLoginCtrl);
    if(!g_fDoHandShakeCheck) jlog(@"v105: DoHandShakeCheck NOT FOUND!");
    if(!g_classLoginModel) jlog(@"v105: LoginModel class NOT FOUND!");
}
...
-(void)onBypassSSO{
    ...
    if(g_bypassSSO){findIL2CPP();
        // v105: 优先hook DoHandShakeCheck (唯一地址, 不被泛型共享)
...
static void setupUI(void){
    ...
    UILabel *tl=[[UILabel alloc]initWithFrame:CGRectMake(10,6,g_panelW-20,20)];tl.text=@"\xe5\x89\x91\xe5\xbd\xb1\xe6\xb1\x9f\xe6\xb9\x96 v105";tl.textColor=IMGUI_ACCENT;tl.font=[UIFont boldSystemFontOfSize:15];tl.textAlignment=NSTextAlignmentCenter;[tb addSubview:tl];
...
static void initialize(void){
    ...
    jlog(@"========== JYJH v105 (SSO bypass: ASLR+get_Instance+Nested) ==========");
