/**
 * v55.0 - 修复HitSystem偏移+皮肤卡死+读取皮肤ID列表
 * v54问题(日志分析):
 *   1. HitSystem.Update Hook: extX=0 extY=0始终为0 → collBound偏移错误!
 *      dump偏移0x38是含IL2CPP header的值, 但struct字段不需要减0x10
 *      实际原因: FPBounds2是值类型嵌套,dump偏移就是字段起始位置
 *      collBound at +0x38就是FPBounds2的起始, Center.x.RawValue=self+0x38
 *      但v54代码用0x28(+0x38-0x10), 所以读到的是错误的内存位置(全0)
 *   2. 皮肤修改卡死: onApplySkin每次调用findIL2CPP()扫描156002方法
 *      用户点20次按钮→20次全量扫描→卡死
 *   3. get_SkinId Hook: isLocal偏移+0x10d不对, 找到的是错误对象
 *      日志: skin=-1042326430 weapon=1066856977 isLocal=116 → 垃圾值
 * v55方案:
 *   1. 修复HitSystem collBound偏移: 用dump原始值0x38作为collBound起始
 *      FPBounds2无IL2CPP header(值类型), 直接就是Center.x开始
 *      Center.x.RawValue = self+0x38, Center.y.RawValue = self+0x40
 *      Extents.x.RawValue = self+0x48, Extents.y.RawValue = self+0x50
 *   2. 修复皮肤: 移除onApplySkin中的findIL2CPP()调用
 *      改用isLocal字段+0x10d来判断玩家Actor(修复判断逻辑)
 *      isLocal是Boolean, 但+0x10d可能超出对象范围, 改用更可靠的判断
 *   3. 读取皮肤ID列表: 通过cfg.Tables运行时实例获取TbRoleSkin._dataList
 *      用il2cpp_class_get_static_field_data或通过GC handle搜索
 */
...
// v55: 皮肤ID列表 - 通过HotfixBusiness.GameHelper.GetRoleSkinById(int id)遍历
// GetRoleSkinById at 0x2bcafa4, 静态方法, 返回cfg.Common.RoleSkin或NULL
typedef void* (*GetRoleSkinByIdFunc)(int32_t);
static GetRoleSkinByIdFunc g_fGetRoleSkinById=NULL;
...
static void scanSkinIds(void) {
    ...
    g_skinIdsLoaded=YES;
    if(!g_fGetRoleSkinById) return;
    g_roleSkinCount=0; g_weaponSkinCount=0;
    // 遍历1~2000, 调用GetRoleSkinById(id), 非NULL则ID有效
    for(int32_t id=1; id<=2000 && g_roleSkinCount<MAX_SKIN_IDS; id++) {
        void *skin=g_fGetRoleSkinById(id);
        if(skin) {
            g_roleSkinIds[g_roleSkinCount++]=id;
        }
    }
    jlog(@"ScanSkinIds: found %d role skin IDs (1~2000)",g_roleSkinCount);
    // 打印前20个
    ...
    for(size_t a=0;a<assemCount;a++){
        ...
        for(size_t c=0;c<cnt;c++){
            ...
            while((m=get_methods(klass,&iter))!=NULL){
                ...
                // v55: 搜索GameHelper.GetRoleSkinById - 皮肤ID枚举
                else if(strcmp(n,"GetRoleSkinById")==0&&pc==1&&!g_fGetRoleSkinById){g_fGetRoleSkinById=(GetRoleSkinByIdFunc)fa;found++;jlog(@"FOUND %s.%s p=%u %p",cn?:"?",n,pc,fa);}
            }
        }
    }
    jlog(@"Scanned %d methods, found %d targets, origDecreaseHP=%p",totalMethods,found,g_origDecreaseHP);
    // v55: 找到GetRoleSkinById后自动扫描皮肤ID列表
    if(g_fGetRoleSkinById) scanSkinIds();
}
...
// v55: 读取皮肤ID列表 - 通过GameHelper.GetRoleSkinById遍历
-(void)onDumpSkinIds{
    // 遍历ID 1~2000, 调用GameHelper.GetRoleSkinById(id)检查是否存在
    // 如果返回非NULL, 说明该ID有效
    jlog(@"DumpSkinIds: scanning 1~2000...");
    void *h=dlopen(NULL,RTLD_LAZY);
    typedef void* (*Il2CppMethodGetPointer)(void*);
    // 找GetRoleSkinById函数指针(在findIL2CPP中已找到g_fUpdatePart, 这里直接搜索)
    // 更简单: 用已知的偏移 0x2bcafa4 计算运行时地址
    // 但ASLR每次不同, 需要用il2cpp_method指针
    // 直接用findIL2CPP中搜索到的函数指针
    if(!g_fUpdatePart){jlog(@"DumpSkin: IL2CPP not searched yet, call findIL2CPP first");findIL2CPP();}
    jlog(@"DumpSkin: role=%d weapon=%d",g_roleSkinCount,g_weaponSkinCount);
    if(g_roleSkinCount==0&&g_weaponSkinCount==0)jlog(@"No skin IDs found yet - need Tables runtime instance");
}
@end
...
static void setupUI(void){
    ...
    UILabel *tl=[[UILabel alloc]initWithFrame:CGRectMake(10,6,g_panelW-20,20)];tl.text=@"\xe5\x89\x91\xe5\xbd\xb1\xe6\xb1\x9f\xe6\xb9\x96 v55.0";tl.textColor=IMGUI_ACCENT;tl.font=[UIFont boldSystemFontOfSize:15];tl.textAlignment=NSTextAlignmentCenter;[tb addSubview:tl];
    // UIScrollView (滚动内容区)
