/**
 * v64.0 - 技能替换(1-5->大招6) + 皮肤扫描诊断修复 + HitSystem保持
 * v63测试:
 *   1. 皮肤扫描: HotfixGameEntry类搜索不到, LinkedList偏移错误, ConfigComponent命名空间搞混
 *      -> v64: (a) 放宽GameEntry搜索+dump所有GameEntry类信息
 *             (b) dump listObj前128字节诊断LinkedList偏移
 *             (c) 区分两个ConfigComponent命名空间
 *   2. 新功能: 按1-5技能键直接释放大招6
 *      -> Hook HandleSkillRange: skillButton 14-18->19
 *      -> Hook Damage: skillButton 14-18->19
 */
...
// v64: HandleSkillRange: void HandleSkillRange(Frame, CharacterFiled*, Int32 skillButton, ExSkillsAsset)
...
static void *getConfigViaComponentList(void) {
    ...
    // v64: 尝试两种偏移方案
    void *headNode=NULL; int32_t count=0;
    memcpy(&headNode,(uint8_t*)listObj+LL_HEAD_OFF,8);
    memcpy(&count,(uint8_t*)listObj+LL_COUNT_OFF,4);
    jlog(@"getConfigList: head@0x10=%p count@0x18=%d",headNode,count);
    if(!isValidPtr(headNode)||count<=0||count>200) {
        // 尝试备选偏移
        memcpy(&headNode,(uint8_t*)listObj+LL_HEAD_OFF2,8);
        memcpy(&count,(uint8_t*)listObj+LL_COUNT_OFF2,4);
        jlog(@"getConfigList: try alt head@0x18=%p count@0x20=%d",headNode,count);
    }
    if(!isValidPtr(headNode)||count<=0||count>200) {
        // v64: 暴力搜索 - 遍历listObj找有效指针作为head
        jlog(@"getConfigList: both offsets failed, brute force search...");
        for(int off=0;off<64;off+=8){
            void *p=NULL; memcpy(&p,(uint8_t*)listObj+off,8);
            if(isValidPtr(p)){
                jlog(@"getConfigList: found valid ptr at listObj+0x%x = %p",off,p);
            }
        }
        return NULL;
    }
    // v64: 尝试两种LLN偏移
    int itemOff=LLN_ITEM_OFF, nextOff=LLN_NEXT_OFF;
    void *node=headNode;
    int maxIter=50;
    for(int i=0;i<maxIter&&isValidPtr(node);i++){
        void *item=NULL;
        memcpy(&item,(uint8_t*)node+itemOff,8);
        if(!isValidPtr(item)&&itemOff==LLN_ITEM_OFF){
            // 尝试备选item偏移
            itemOff=LLN_ITEM_OFF2; nextOff=LLN_NEXT_OFF2;
            memcpy(&item,(uint8_t*)node+itemOff,8);
            jlog(@"getConfigList: switched to alt itemOff=0x%x nextOff=0x%x",itemOff,nextOff);
        }
        if(isValidPtr(item)){
            const char *cn=getObjClassName(item);
            const char *ns=getObjClassNamespace(item);
            jlog(@"  comp[%d]=%p class=%s ns=%s",i,item,cn?cn:"?",ns?ns:"?");
            if(cn&&strcmp(cn,"ConfigComponent")==0){
                // v64: 优先返回HotfixFramework命名空间的
                if(ns&&strstr(ns,"HotfixFramework")!=NULL){
                    jlog(@"getConfigList: FOUND HotfixConfigComponent=%p",item);
                    return item;
                }
                // 记录UnityGameFramework的ConfigComponent作为备选
                jlog(@"getConfigList: found Unity ConfigComponent=%p (keep looking)",item);
                if(!g_classConfigComponent) g_classConfigComponent=item;
            }
        }
        void *nextNode=NULL;
        memcpy(&nextNode,(uint8_t*)node+nextOff,8);
        if(nextNode==headNode||!isValidPtr(nextNode)) break;
        node=nextNode;
    }
    // 如果没找到HotfixFramework的,返回找到的第一个ConfigComponent
...
// v64: HandleSkillRange Hook - 技能替换核心: skillButton 14-18->19
static int g_hsrLC=0;
static void hHandleSkillRange(void *f,void *cf,int32_t skillButton,void *exSkill) {
    if(g_skillReplace&&skillButton>=14&&skillButton<=18){
        if(g_hsrLC<20){g_hsrLC++;jlog(@"HSR: skillBtn %d->19",skillButton);}
        skillButton=19;
    }
    ...
    for(size_t a=0;a<assemCount;a++){
        ...
        for(size_t c=0;c<cnt;c++){
            ...
            while((m=get_methods(klass,&iter))!=NULL){
                ...
                // v64: HandleSkillRange - 技能替换核心Hook点
...
-(void)onSkillReplace{
    ...
    if(g_skillReplace){findIL2CPP();
        if(!g_hHandleSkillRange)hookOneFunc(g_fHandleSkillRange,hHandleSkillRange,(void**)&g_oHandleSkillRange,&g_hHandleSkillRange,"HandleSkillRange");
        if(!g_hDamage)hookOneFunc(g_fDamage,hDamage,(void**)&g_oDamage,&g_hDamage,"Damage");
        if(!g_hIsReady)hookOneFunc(g_fIsReady,hIsReady,(void**)&g_oIsReady,&g_hIsReady,"IsReady");
        if(!g_hAttackCanUse)hookOneFunc(g_fAttackCanUse,hAttackCanUse,(void**)&g_oAttackCanUse,&g_hAttackCanUse,"AttackCanUse");
    }
...
static void setupUI(void){
    ...
    UILabel *tl=[[UILabel alloc]initWithFrame:CGRectMake(10,6,g_panelW-20,20)];tl.text=@"\xe5\x89\x91\xe5\xbd\xb1\xe6\xb1\x9f\xe6\xb9\x96 v64.0";tl.textColor=IMGUI_ACCENT;tl.font=[UIFont boldSystemFontOfSize:15];tl.textAlignment=NSTextAlignmentCenter;[tb addSubview:tl];
