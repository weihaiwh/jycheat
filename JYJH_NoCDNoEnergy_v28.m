/**
 * 剑影江湖 v28.0 - Hook BattleGameLogic.SetInputOpt实例方法
 *
 * v27关键发现：
 * 1. OnClickSkill_6被调1次（按钮点击有效！）
 * 2. 但SetInputOpt hook没触发 → 因为OnClickSkill_6内部调的是实例方法
 * 3. dump.cs有两套SetInputOpt:
 *    - BattleInputOptHelper.SetInputOpt(Int32, Int32) - 静态方法 // 0x2bf5314
 *    - BattleGameLogic.SetInputOpt(Int32, Int32) - 实例方法 // 0x2bfaff0
 * 4. OnClickSkill_6内部调的是BattleGameLogic.SetInputOpt（实例方法）
 * 5. 之前hook的是BattleInputOptHelper.SetInputOpt（静态方法），当然抓不到！
 *
 * v28策略：
 * 1. Hook BattleGameLogic.SetInputOpt（实例方法）而不是静态方法
 * 2. 在OnClickSkill_6 hook中调BattleGameLogic.SetInputOpt的原始函数
 * 3. SetInputOpt签名: (void* self, int optType, int optData) 实例方法
 */
...
// BattleGameLogic.SetInputOpt(Int32 optType, Int32 optData) - 实例方法
...
// v28 hook: 全部改为BattleGameLogic实例方法
...
// ============================================================
// v28核心: Hook BattleGameLogic.SetInputOpt（实例方法）+ OnClickSkill_6
// ============================================================

// BattleGameLogic.SetInputOpt(Int32 optType, Int32 optData) - 实例方法
// 监控UseExSkill(20014)输入
static int g_setInputOptCount = 0;
static void hookSetInputOpt(void *self, int optType, int optData) {
    if (g_exSkillNoCD) {
        if (optType == 20014) {
    ...
    if (g_origSetInputOpt) g_origSetInputOpt(self, optType, optData);
}
...
static void findIL2CPP(void) {
    jlog(@"=== v28.0 IL2CPP Runtime Search ===");
    ...
    for (size_t a = 0; a < assemCount; a++) {
        ...
        for (size_t c = 0; c < cnt; c++) {
            ...
            while ((m = get_methods(klass, &iter)) != NULL) {
                ...
                // v28: SetInputOpt (BattleGameLogic实例方法！不是BattleInputOptHelper静态方法)
                else if (strcmp(n, "SetInputOpt") == 0 && !g_funcSetInputOpt && cn && strcmp(cn, "BattleGameLogic") == 0) { jlog(@"FOUND %s.%s params=%u addr=%p", cn,n,pc,funcAddr); g_funcSetInputOpt=funcAddr; found++; }
...
static void setupUI(void) {
    ...
    title.text=@"\u5251\u5f71\u6c5f\u6e56 v28.0"; title.textColor=[UIColor cyanColor];
...
static void initialize(void) {
    ...
    if (loaded) return;
    loaded = YES;
    jlog(@"========== JYJH v28.0 (Hook BattleGameLogic.SetInputOpt实例方法) ==========");
    jlog(@"iOS %@", [[UIDevice currentDevice] systemVersion]);
    jlog(@"v28核心: Hook BattleGameLogic.SetInputOpt实例方法(不是静态方法)");
