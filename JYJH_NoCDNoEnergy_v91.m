/**
 * v91 - 皮肤/武器/光环: hook LobbyActorData.UpdatePart + ActorData.GetMainActorData + SceneActorSpineAvatar.ChangePart
 * v89问题: hook了MainModel.InitSpineSkin, 但应用后游戏不会再次调用InitSpineSkin, 所以修改无效
 * v91方案:
 * 1. Hook LobbyActorData.UpdatePart(skinId,weaponId,haloId) @ 0x2bfb050 - 替换参数
 * 2. Hook ActorData.GetMainActorData() @ 0x2b793c8 - 获取主角色数据, 修改SkinId(+0x14)/WeaponId(+0x18)/HaloId(+0x1c)
 * 3. 捕获SceneActorSpineAvatar实例(通过hook InitWithActorData)
 * 4. 应用时: 修改ActorData字段 + 调用ChangePart(AvatarPartType,id,NULL) @ 0x2bfb330 触发渲染刷新
 * 5. AvatarPartType: Body=1, Skin=2, Weapon=3, Halo=4
 * 6. 保留v88/v89其他所有功能
 */
...
// v91: LobbyActorData.UpdatePart(skinId,weaponId,haloId) 实例方法
typedef void (*UpdatePartFunc)(void*,void*,int32_t,int32_t,int32_t);
// v91: ActorData.GetMainActorData() 静态方法
typedef void* (*GetMainActorDataFunc)(void*);
// v91: SceneActorSpineAvatar.ChangePart(AvatarPartType,id,spine) 实例方法
typedef void (*ChangePartFunc)(void*,void*,int32_t,int32_t,void*);
// v91: SceneActorSpineAvatar.InitWithActorData(ActorData) 实例方法
typedef void (*InitWithActorDataFunc)(void*,void*,void*);
...
// v91: UpdatePart + GetMainActorData + ChangePart + InitWithActorData
static void *g_fUpdatePart=NULL; static UpdatePartFunc g_oUpdatePart=NULL; static BOOL g_hUpdatePart=NO;
static void *g_fGetMainActorData=NULL; static GetMainActorDataFunc g_oGetMainActorData=NULL; static BOOL g_hGetMainActorData=NO;
static void *g_fChangePart=NULL; static ChangePartFunc g_oChangePart=NULL; static BOOL g_hChangePart=NO;
static void *g_fInitWithActorData=NULL; static InitWithActorDataFunc g_oInitWithActorData=NULL; static BOOL g_hInitWithActorData=NO;
static void *g_classLobbyActorData=NULL;

// v91: 捕获的Avatar实例 + 主角色ActorData
static void *g_playerAvatar=NULL; // SceneActorSpineAvatar实例 (LobbyActorSpineAvatar)
static void *g_mainActorData=NULL; // ActorData实例
...
// ===== v91: LobbyActorData.UpdatePart Hook =====
// LobbyActorData.UpdatePart(skinId,weaponId,haloId) - 游戏换装时调用
static int g_updatePartLC=0;
static void hUpdatePart(void *mi, void *self, int32_t skinId, int32_t weaponId, int32_t haloId) {
    int32_t useSkin = (g_appliedSkinId > 0) ? g_appliedSkinId : skinId;
    int32_t useWeapon = (g_appliedWeaponId > 0) ? g_appliedWeaponId : weaponId;
    int32_t useHalo = (g_appliedHaloId > 0) ? g_appliedHaloId : haloId;
    if(g_appliedSkinId > 0 || g_appliedWeaponId > 0 || g_appliedHaloId > 0) {
        if(g_updatePartLC < 50) { g_updatePartLC++; jlog(@"UpdatePart: %d/%d/%d -> %d/%d/%d", skinId, weaponId, haloId, useSkin, useWeapon, useHalo); }
    }
    if(g_oUpdatePart) g_oUpdatePart(mi, self, useSkin, useWeapon, useHalo);
}

// ===== v91: InitWithActorData Hook - 捕获Avatar实例 =====
static void hInitWithActorData(void *mi, void *self, void *actorData) {
    if(self) {
        // self是SceneActorSpineAvatar (LobbyActorSpineAvatar的父类)
        // 检查IsMyRole (ActorData+0x10) 或通过其他方式判断是主角色
        if(actorData) {
            BOOL isMyRole = NO; memcpy(&isMyRole, (uint8_t*)actorData + 0x10, 1);
            if(isMyRole && !g_playerAvatar) {
                g_playerAvatar = self;
                jlog(@"InitWithActorData: captured Avatar=%p (isMyRole=%d)", self, isMyRole);
            }
        }
    }
    if(g_oInitWithActorData) g_oInitWithActorData(mi, self, actorData);
}

// ===== v91: ActorData.GetMainActorData Hook - 获取主角色数据 =====
static void* hGetMainActorData(void *mi) {
    void *result = NULL;
    if(g_oGetMainActorData) result = g_oGetMainActorData(mi);
    if(result && !g_mainActorData) {
        g_mainActorData = result;
        int32_t skinId=0, weaponId=0, haloId=0;
        memcpy(&skinId, (uint8_t*)result + 0x14, 4);
        memcpy(&weaponId, (uint8_t*)result + 0x18, 4);
        memcpy(&haloId, (uint8_t*)result + 0x1c, 4);
        jlog(@"GetMainActorData: %p skin=%d weapon=%d halo=%d", result, skinId, weaponId, haloId);
    }
    // 应用替换值
    if(result) {
        if(g_appliedSkinId > 0) memcpy((uint8_t*)result + 0x14, &g_appliedSkinId, 4);
        if(g_appliedWeaponId > 0) memcpy((uint8_t*)result + 0x18, &g_appliedWeaponId, 4);
        if(g_appliedHaloId > 0) memcpy((uint8_t*)result + 0x1c, &g_appliedHaloId, 4);
    }
    return result;
}

// ===== v91: SceneActorSpineAvatar.ChangePart Hook - 日志 + 捕获Avatar =====
static int g_changePartLC=0;
static void hChangePart(void *mi, void *self, int32_t partType, int32_t id, void *spine) {
    if(!g_playerAvatar && self) {
        g_playerAvatar = self;
        jlog(@"ChangePart: captured Avatar=%p", self);
    }
    if(g_changePartLC < 30) { g_changePartLC++; jlog(@"ChangePart: type=%d id=%d self=%p", partType, id, self); }
    if(g_oChangePart) g_oChangePart(mi, self, partType, id, spine);
}
...
static void findIL2CPP(void) {
    ...
    for(size_t a=0;a<assemCount;a++){
        ...
        for(size_t c=0;c<cnt;c++){
            ...
            while((m=get_methods(klass,&iter))!=NULL){
                ...
                // v91: 搜索LobbyActorData.UpdatePart(skinId,weaponId,haloId) 3参数实例方法
                else if(strcmp(n,"UpdatePart")==0&&pc==3&&!g_fUpdatePart){g_fUpdatePart=fa;found++;jlog(@"FOUND %s.%s p=%u %p",cn?:"?",n,pc,fa);}
                // v91: 搜索ActorData.GetMainActorData() 静态方法 0参数
                else if(strcmp(n,"GetMainActorData")==0&&pc==0&&!g_fGetMainActorData){g_fGetMainActorData=fa;found++;jlog(@"FOUND %s.%s p=%u %p",cn?:"?",n,pc,fa);}
                // v91: 搜索SceneActorSpineAvatar.ChangePart(AvatarPartType,id,spine) 3参数实例方法
                else if(strcmp(n,"ChangePart")==0&&pc==3&&!g_fChangePart){g_fChangePart=fa;found++;jlog(@"FOUND %s.%s p=%u %p",cn?:"?",n,pc,fa);}
                // v91: 搜索SceneActorSpineAvatar.InitWithActorData(ActorData) 1参数实例方法
                else if(strcmp(n,"InitWithActorData")==0&&pc==1&&!g_fInitWithActorData){g_fInitWithActorData=fa;found++;jlog(@"FOUND %s.%s p=%u %p",cn?:"?",n,pc,fa);}
            }
        }
    }
    ...
    jlog(@"v91: UnityGameEntry=%p HotfixGameEntry=%p UpdatePart=%p GetMainActorData=%p ChangePart=%p InitWithActorData=%p",g_classUnityGameEntry,g_classHotfixGameEntry,g_fUpdatePart,g_fGetMainActorData,g_fChangePart,g_fInitWithActorData);
...
// v91: 应用外观 - 修改ActorData字段 + 调用ChangePart触发渲染
// AvatarPartType: Body=1, Skin=2, Weapon=3, Halo=4
static void applyPartChange(void) {
    // 1. 获取主角色ActorData
    if(!g_mainActorData && g_fGetMainActorData && g_oGetMainActorData) {
        g_mainActorData = g_oGetMainActorData(NULL);
        if(g_mainActorData) jlog(@"applyPart: got MainActorData=%p", g_mainActorData);
    }
    if(!g_mainActorData) {
        jlog(@"applyPart: MainActorData not found yet");
        return;
    }
    // 2. 修改ActorData字段: SkinId(+0x14), WeaponId(+0x18), HaloId(+0x1c)
    if(g_appliedSkinId > 0) { memcpy((uint8_t*)g_mainActorData + 0x14, &g_appliedSkinId, 4); jlog(@"applyPart: SkinId=%d", g_appliedSkinId); }
    if(g_appliedWeaponId > 0) { memcpy((uint8_t*)g_mainActorData + 0x18, &g_appliedWeaponId, 4); jlog(@"applyPart: WeaponId=%d", g_appliedWeaponId); }
    if(g_appliedHaloId > 0) { memcpy((uint8_t*)g_mainActorData + 0x1c, &g_appliedHaloId, 4); jlog(@"applyPart: HaloId=%d", g_appliedHaloId); }
    // 3. 调用ChangePart触发渲染刷新
    if(g_playerAvatar && g_oChangePart) {
        if(g_appliedSkinId > 0) { jlog(@"applyPart: ChangePart(Skin=2, %d)", g_appliedSkinId); g_oChangePart(NULL, g_playerAvatar, 2, g_appliedSkinId, NULL); }
        if(g_appliedWeaponId > 0) { jlog(@"applyPart: ChangePart(Weapon=3, %d)", g_appliedWeaponId); g_oChangePart(NULL, g_playerAvatar, 3, g_appliedWeaponId, NULL); }
        if(g_appliedHaloId > 0) { jlog(@"applyPart: ChangePart(Halo=4, %d)", g_appliedHaloId); g_oChangePart(NULL, g_playerAvatar, 4, g_appliedHaloId, NULL); }
    } else {
        jlog(@"applyPart: Avatar=%p ChangePart=%p - cannot trigger render", g_playerAvatar, g_oChangePart);
    }
}
...
// v91: 皮肤/武器/光环 独立应用 - hook必要函数 + 获取数据
-(void)ensureSkinHooksActive {
    findIL2CPP();
    // Hook UpdatePart (替换参数)
    if(g_fUpdatePart && !g_hUpdatePart) hookOneFunc(g_fUpdatePart, hUpdatePart, (void**)&g_oUpdatePart, &g_hUpdatePart, "UpdatePart");
    // Hook GetMainActorData (获取主角色数据)
    if(g_fGetMainActorData && !g_hGetMainActorData) {
        MSHookFunction(g_fGetMainActorData, (void*)hGetMainActorData, (void**)&g_oGetMainActorData);
        g_hGetMainActorData = YES;
        jlog(@"GetMainActorData: hooked at %p", g_fGetMainActorData);
    }
    // Hook InitWithActorData (捕获Avatar实例)
    if(g_fInitWithActorData && !g_hInitWithActorData) hookOneFunc(g_fInitWithActorData, hInitWithActorData, (void**)&g_oInitWithActorData, &g_hInitWithActorData, "InitWithActorData");
    // Hook ChangePart (用于触发渲染)
    if(g_fChangePart && !g_hChangePart) hookOneFunc(g_fChangePart, hChangePart, (void**)&g_oChangePart, &g_hChangePart, "ChangePart");
}
-(void)onApplySkin{
...
static void setupUI(void){
    ...
    UILabel *tl=[[UILabel alloc]initWithFrame:CGRectMake(10,6,g_panelW-20,20)];tl.text=@"剑影江湖 v91";tl.textColor=IMGUI_ACCENT;tl.font=[UIFont boldSystemFontOfSize:15];tl.textAlignment=NSTextAlignmentCenter;[tb addSubview:tl];
...
static void initialize(void){
    ...
    jlog(@"========== JYJH v91 (UpdatePart + ChangePart + GetMainActorData) ==========");
