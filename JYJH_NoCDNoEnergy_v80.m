/**
 * v80 - 基于稳定v79, 修复皮肤应用
 * 变更: 1) 搜索InitWithSkinId方法, 调用它真正切换皮肤模型
 *       2) 保留get_SkinId hook(返回值) + 调用InitWithSkinId(模型切换)
 *       3) 不加变速功能(帧同步游戏, timeScale只影响动画不影响伤害)
 * v75-v78 timeScale全部闪退, 已放弃
 */
...
// v80: 皮肤Hook - get_SkinId返回值 + InitWithSkinId真正切换模型
...
// v80: InitWithSkinId - SceneActorSpineAvatar.InitWithSkinId(int skinId, int changeType)
// 偏移: 0x2bfaff0 (v1.10.1)
// 这个方法会真正加载Spine模型切换皮肤
typedef void (*InitWithSkinIdFunc)(void*,int32_t,int32_t);
static void *g_fInitWithSkinId=NULL;
static InitWithSkinIdFunc g_oInitWithSkinId=NULL; static BOOL g_hInitWithSkinId=NO;

// v80: 同时搜索ChangePart方法 - Actor.ChangePart(AvatarPartType, int, UIC_SpineView)
// 可以用来单独替换武器/皮肤
typedef void (*ChangePartFunc)(void*,int32_t,int32_t,void*);
static void *g_fChangePart=NULL;

// v80: 同时搜索set_SkinId方法 - Actor.set_SkinId(int value) 偏移 0x2bfae6c
// 虽然v69写backing field导致卡死, 但如果只改self的backing field然后调InitWithSkinId可能可以
typedef void (*SetSkinIdFunc)(void*,int32_t);
static void *g_fSetSkinId=NULL;
...
static void *getConfigViaComponentList(void) {
    ...
    void *headNode=NULL;
    memcpy(&headNode,(uint8_t*)listObj+0x0,8);
    ...
    for(size_t a=0;a<assemCount;a++){
        ...
        for(size_t c=0;c<cnt;c++){
            ...
            while((m=get_methods(klass,&iter))!=NULL){
                ...
                // v80: InitWithSkinId - 真正切换皮肤模型
                // SceneActorSpineAvatar.InitWithSkinId(int skinId, int changeType) - pc=2
                else if(strcmp(n,"InitWithSkinId")==0&&pc==2&&!g_fInitWithSkinId){g_fInitWithSkinId=fa;found++;jlog(@"FOUND %s.%s p=%u %p (skin model switch)",cn?:"?",n,pc,fa);}
                // v80: set_SkinId - Actor私有方法, 写backing field
                else if(strcmp(n,"set_SkinId")==0&&pc==1&&cn&&strcmp(cn,"Actor")==0&&!g_fSetSkinId){g_fSetSkinId=fa;found++;jlog(@"FOUND %s.%s p=%u %p",cn,n,pc,fa);}
                // v80: ChangePart - 替换武器等
    ...
    if(!g_classHotfixGameEntry){
        for(int i=0;i<g_gameEntryCount;i++){
            if(g_allGameEntryNS[i]==NULL||strcmp(g_allGameEntryNS[i],"")==0){
                ...
                jlog(@"v80: assume GameEntry[%d] ns=empty is HotfixGameEntry=%p",i,g_allGameEntries[i]);
                break;
            }
        }
    }
    jlog(@"v80: UnityGameEntry=%p HotfixGameEntry=%p",g_classUnityGameEntry,g_classHotfixGameEntry);
    jlog(@"v80: InitWithSkinId=%p set_SkinId=%p ChangePart=%p get_SkinId=%p",
         g_fInitWithSkinId,g_fSetSkinId,g_fChangePart,g_fGetSkinId);
...
static void refreshBtns(void){
    ...
    if(g_btnApplySkin){
        if(g_appliedSkinId>0){
            [g_btnApplySkin setTitle:[NSString stringWithFormat:@"\xe7\x9a\xae\xe8\x82\xa4:%d(\xe7\x94\x9f\xe6\x95\x88\xe4\xb8\xad)",g_appliedSkinId] forState:UIControlStateNormal];
...
-(void)onApplySkin{
    // v80: 皮肤应用 - 两步走:
    // 1) hook get_SkinId 返回目标skinId (让读取皮肤ID的地方拿到新ID)
    // 2) 调用 InitWithSkinId(skinId, changeType) 真正切换Spine模型
    if(!g_skinIdsLoaded){jlog(@"ApplySkin: scan skins first");return;}
    if(!g_fGetSkinId){jlog(@"ApplySkin: get_SkinId not found");return;}
    
    // Step 1: Hook get_SkinId (如果还没hook)
    if(!g_hGetSkinId) {
    ...
    // Step 2: 调用 InitWithSkinId 真正切换皮肤模型
    if(g_playerActorObj && g_fInitWithSkinId) {
        jlog(@"ApplySkin: calling InitWithSkinId(%d, 2) on Actor=%p", g_skinId, g_playerActorObj);
        // InitWithSkinId(int skinId, int changeType)
        // changeType: 1=Body, 2=Skin, 3=Weapon, 4=Halo (from AvatarPartType enum)
        // 皮肤用 changeType=2
        @try {
            ((InitWithSkinIdFunc)g_fInitWithSkinId)(g_playerActorObj, g_skinId, 2);
            jlog(@"ApplySkin: InitWithSkinId(skinId=%d, type=2) called OK", g_skinId);
        } @catch(NSException *e) {
            jlog(@"ApplySkin: InitWithSkinId EXCEPTION: %@", e);
        }
    } else if(!g_playerActorObj) {
        jlog(@"ApplySkin: player Actor not found yet (need to enter battle first)");
    } else if(!g_fInitWithSkinId) {
        jlog(@"ApplySkin: InitWithSkinId not found, only hooking get_SkinId (visual only)");
    }
    
    jlog(@"ApplySkin: skin=%d weapon=%d (get_SkinId hook active, InitWithSkinId=%s)",
         g_skinId, g_weaponId, g_fInitWithSkinId?"YES":"NO");
}
@end
...
static void setupUI(void){
    ...
    UILabel *tl=[[UILabel alloc]initWithFrame:CGRectMake(10,6,g_panelW-20,20)];tl.text=@"\xe5\x89\x91\xe5\xbd\xb1\xe6\xb1\x9f\xe6\xb9\x96 v80";tl.textColor=IMGUI_ACCENT;tl.font=[UIFont boldSystemFontOfSize:15];tl.textAlignment=NSTextAlignmentCenter;[tb addSubview:tl];
...
static void initialize(void){
    ...
    jlog(@"========== JYJH v80 (v79 base + skin fix) ==========");
