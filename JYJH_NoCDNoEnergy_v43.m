/**
 * v43.0 - 全屏秒杀(DecreaseHP扣血修复) + 皮肤修改(写Actor字段) + 皮肤ID列表 + 大菜单居中可调
 * v42问题:
 *   1. DecreaseHP扩散没触发: g_oDecreaseHP为NULL(只存了函数地址g_fDecreaseHP,没存原始调用指针)
 *   2. Tables field=0x0: il2cpp_class_get_field_from_name找不到字段名
 *   3. 查询皮肤ID按钮点击没反应(其实有输出但Tables读取失败)
 *   4. 菜单贴着悬浮球太小
 */
...
static int64_t hDamage(void *f,void *atkEnt,void *atkCF,void *tgtEnt,void *tgtCF,
    ...
    if(atkP&&r>0){
        ...
        // v43: DecreaseHP直接扣血 - 用g_origDecreaseHP(直接保存的函数指针,不经过Hook)
        if(g_fullScreen&&!tgtP&&g_playerEntity&&g_enemyCount>0&&g_origDecreaseHP){
            for(int i=0;i<g_enemyCount;i++){
                ...
                int64_t hr=g_origDecreaseHP(f,g_playerEntity,eEnt,eCF,r);
...
static void findIL2CPP(void) {
    ...
    for(size_t a=0;a<assemCount;a++){
        ...
        for(size_t c=0;c<cnt;c++){
            ...
            // Tables类 - 用遍历字段方式找Instance
            if(cn&&strcmp(cn,"Tables")==0&&!classTables){
                classTables=klass;
                jlog(@"FOUND class Tables=%p",klass);
...
// 面板大小
static CGFloat g_panelW=280, g_panelH=540;
...
static void layoutPanelCenter(void){
    ...
    CGFloat py=(sc.size.height-g_panelH)/2;
...
static void setupUI(void){
    ...
    // Buttons
    CGFloat bx=12,bw=g_panelW-24,bh=34,by0=40,bdy=38;
