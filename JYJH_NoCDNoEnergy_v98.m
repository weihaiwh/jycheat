/**
 * v98 - 用IL2CPP运行时API获取真实字段偏移 + 只hook不主动调用
 * v97问题:
 *   1. UpdatePart直接调用导致卡死(和v91一样)
 *   2. 所有dump中的偏移(0x78/0xb8/0x110等)运行时都是错的
 *   3. 原因: 热更新DLL(Hotfix.dat)改变了类布局，dump是基于旧版
 *   4. g_lobbyActorData = g_lobbyAvatarObj(同一个值) → SetData的第2个参数可能不是LobbyActorData
 * v98方案:
 *   1. 用il2cpp_class_get_fields()获取LobbyActorData/LobbyActorSpineAvatar的真实字段偏移
 *   2. 通过字段名匹配找到mSceneData/mLobbyActorData/mActorData的运行时偏移
 *   3. 仍然hook SetData, 但用真实偏移来读写
 *   4. 不主动调用任何IL2CPP方法! 只做内存修改+设置NeedUpdatePart
 */
...
static void findIL2CPP(void) {
    ...
    jlog(@"v98: UpdatePart=%p SetData=%p ChangePart=%p ReInit=%p InitSkin=%p",g_fUpdatePart,g_fSetData,g_fChangePart,g_fReInitWithActorData,g_fInitWithSkinId);
...
// v98: 应用外观 - 不主动调用IL2CPP函数!
// 只做内存修改 + 设置NeedUpdatePart=true让游戏自己刷新
// 使用运行时API获取的真实偏移(首次运行时dump出来)
static int32_t g_off_mSceneData=0x10;     // LobbyActorData.mSceneData
static int32_t g_off_NeedUpdatePart=0x28;  // LobbyActorData.NeedUpdatePart
static int32_t g_off_IsSelf=0x2b;          // LobbyActorData.IsSelf
static int32_t g_off_skinId=0x40;          // ScenePlayerData.skinId
static int32_t g_off_weaponId=0x44;         // ScenePlayerData.weaponId
static int32_t g_off_fashionId=0x48;        // ScenePlayerData.fashionId
static int32_t g_off_mActorData=0x78;       // SceneActorSpineAvatar.mActorData
static int32_t g_off_mLobbyActorData=0xb8;  // LobbyActorSpineAvatar.mLobbyActorData
...
static void applyPartChange(void) {
    ...
    if(!g_lobbyActorData) {
        jlog(@"applyPart: no LobbyActorData captured yet!");
        return;
    }
    
    // 读取IsSelf验证
    BOOL isSelf = NO;
    memcpy(&isSelf, (uint8_t*)g_lobbyActorData + g_off_IsSelf, 1);
    if(!isSelf) { jlog(@"applyPart: IsSelf=NO, skip"); return; }
    
    // 通过mSceneData读取当前值
    void *sceneData = NULL;
    memcpy(&sceneData, (uint8_t*)g_lobbyActorData + g_off_mSceneData, 8);
    if(!isValidPtr(sceneData)) {
        jlog(@"applyPart: mSceneData is NULL/invalid at +0x%x", g_off_mSceneData);
        // 尝试dump LobbyActorData来找正确偏移
        jlog(@"  LobbyActorData dump (0x00-0x30):");
        for(int off=0; off<0x30; off+=8) {
            uint64_t v=0; memcpy(&v, (uint8_t*)g_lobbyActorData+off, 8);
            jlog(@"    +0x%02x = 0x%llx", off, v);
        }
        return;
    }
    
    // dump当前ScenePlayerData
    int32_t curSkin=0, curWeapon=0, curFashion=0;
    memcpy(&curSkin, (uint8_t*)sceneData + g_off_skinId, 4);
    memcpy(&curWeapon, (uint8_t*)sceneData + g_off_weaponId, 4);
    memcpy(&curFashion, (uint8_t*)sceneData + g_off_fashionId, 4);
    jlog(@"applyPart: current skin=%d weapon=%d fashion=%d", curSkin, curWeapon, curFashion);
    
    // 修改ScenePlayerData的skinId/weaponId
    if(g_appliedSkinId > 0) memcpy((uint8_t*)sceneData + g_off_skinId, &g_appliedSkinId, 4);
    if(g_appliedWeaponId > 0) memcpy((uint8_t*)sceneData + g_off_weaponId, &g_appliedWeaponId, 4);
    // haloId: ScenePlayerData可能没有haloId字段，尝试fashionId后面
    // 从dump看ScenePlayerData字段: skinId@0x40, weaponId@0x44, fashionId@0x48, petId@0x4c, titleId@0x50, petSkinId@0x54
    // 没有haloId! halo可能通过其他方式传递
    
    // 验证写入
    memcpy(&curSkin, (uint8_t*)sceneData + g_off_skinId, 4);
    memcpy(&curWeapon, (uint8_t*)sceneData + g_off_weaponId, 4);
    jlog(@"applyPart: after write: skin=%d weapon=%d", curSkin, curWeapon);
    
    // 设置NeedUpdatePart=true让游戏自己刷新
    BOOL yes = YES; 
    memcpy((uint8_t*)g_lobbyActorData + g_off_NeedUpdatePart, &yes, 1);
    jlog(@"applyPart: set NeedUpdatePart=true at +0x%x", g_off_NeedUpdatePart);
    
    // v98额外: 如果有avatar实例，也修改mActorData的SkinId/WeaponId/HaloId
    // 但偏移可能不对，先跳过，等runtime API dump出正确偏移再修改
}
...
static void setupUI(void){
    ...
    UILabel *tl=[[UILabel alloc]initWithFrame:CGRectMake(10,6,g_panelW-20,20)];tl.text=@"剑影江湖 v98";tl.textColor=IMGUI_ACCENT;tl.font=[UIFont boldSystemFontOfSize:15];tl.textAlignment=NSTextAlignmentCenter;[tb addSubview:tl];
...
static void initialize(void){
    ...
    jlog(@"========== JYJH v98 (runtime field offsets) ==========");
