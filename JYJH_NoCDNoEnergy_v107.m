/**
 * v107 - SSO bypass: GC heap scan for LoginModel instance
 * v106问题:
 *   il2cpp_class_get_method_from_name不存在(iOS 15.6 IL2CPP版本太老)
 *   Nested类全量扫描太慢(102 assemblies * 数千类) → 卡住
 * v107方案:
 *   1. 用il2cpp_gc_foreach_heap/segment遍历GC堆
 *   2. 找klass指针==LoginModel的对象 → 就是单例实例
 *   3. 读CurSSOResponData(offset 0xf0) → 写code=200+banTime=0
 *   4. 保留DoHandShakeCheck hook
 *   5. 去掉Nested类扫描(太慢)
 */
...
// v107: 保留g_fGetCode/g_fGetBanTime用于搜索, 但不再MSHookFunction它们
...
// v107: 直接内存修改SSOResponData (备用方案, 声明在前, 定义在isValidPtr/getObjClassName之后)
...
// v107: 直接内存修改SSOResponData (实现)
// 前几版问题: 各种API都不work (static_field_data=0, get_method_from_name不存在, Nested扫描太慢)
// v107: 用GC堆扫描找LoginModel实例 — 已知klass指针, 找对象
static void *g_foundLoginModel = NULL; // 缓存找到的LoginModel实例

static void patchSSOResponDataMemory(void) {
    if(!g_classLoginModel) {jlog(@"SSO: LoginModel class not found");return;}

    void *loginModelInstance = g_foundLoginModel; // 用缓存

    // 方案1: GC堆扫描 — 遍历所有GC段找klass==LoginModel的对象
    if(!isValidPtr(loginModelInstance)) {
        jlog(@"SSO: Scanning GC heap for LoginModel instance...");
        void *h=dlopen(NULL,RTLD_LAZY); if(!h) return;

        // 尝试il2cpp_gc_foreach_heap (Unity 2020+)
        typedef void (*GCForeachFn)(void(*)(void*,void*),void*);
        GCForeachFn gc_foreach=(GCForeachFn)dlsym(h,"il2cpp_gc_foreach_heap");

        // 尝试il2cpp_gc_heap_segment_next
        typedef void* (*SegmentNextFn)(void*);
        SegmentNextFn seg_next=(SegmentNextFn)dlsym(h,"il2cpp_gc_heap_segment_next");

        // 尝试il2cpp_gc_heap_segment_data
        typedef void* (*SegmentDataFn)(void*);
        SegmentDataFn seg_data=(SegmentDataFn)dlsym(h,"il2cpp_gc_heap_segment_data");

        // 尝试il2cpp_gc_heap_segment_size
        typedef size_t (*SegmentSizeFn)(void*);
        SegmentSizeFn seg_size=(SegmentSizeFn)dlsym(h,"il2cpp_gc_heap_segment_size");

        // 尝试il2cpp_gc_heap_segment_first
        typedef void* (*SegmentFirstFn)(void*);
        SegmentFirstFn seg_first=(SegmentFirstFn)dlsym(h,"il2cpp_gc_heap_segment_first");

        if(gc_foreach) {
            jlog(@"SSO: Found il2cpp_gc_foreach_heap, using it...");
            // 使用GC foreach回调扫描
            __block void *foundInstance = NULL;
            void (^scanBlock)(void*,void*) = ^(void *segment, void *userData) {
                if(foundInstance) return;
                // segment数据区: 对象连续排列, 每个对象前8字节是klass指针
                // 需要读取segment的data/size来遍历
                // 但GC foreach的回调参数是heap, 不是segment
                // 简化: 尝试直接读取segment内存
            };
            // gc_foreach的回调签名不太明确, 暂时跳过
        }

        // 方案2: 用vm_read扫描进程内存 (简单粗暴但有效)
        // 已知LoginModel类指针, 扫描堆内存找klass==g_classLoginModel的对象
        // 限制扫描范围避免太慢: 只扫描0x140000000-0x150000000(游戏堆区)
        jlog(@"SSO: Using vm_read memory scan...");
        vm_address_t address = 0x140000000;
        vm_size_t size = 0x10000000; // 256MB
        vm_region_basic_info_data_64_t info;
        mach_msg_type_number_t info_count = VM_REGION_BASIC_INFO_COUNT_64;
        memory_object_name_t object;

        int foundCount = 0;
        vm_address_t scan_addr = address;
        vm_address_t scan_end = address + size;

        while(scan_addr < scan_end && foundCount < 3) {
            // 查询可读内存区域
            kern_return_t kr = vm_region_64(mach_task_self(), &scan_addr, &size, VM_REGION_BASIC_INFO_64, (vm_region_info_t)&info, &info_count, &object);
            if(kr != KERN_SUCCESS) break;
            if(!(info.protection & VM_PROT_READ)) { scan_addr += size; continue; }

            // 读取内存
            if(size > 0x1000000) size = 0x1000000; // 最多读16MB一段
            uint8_t *buf = (uint8_t*)malloc(size);
            vm_size_t readSize = 0;
            kr = vm_read_overwrite(mach_task_self(), scan_addr, size, (vm_address_t)buf, &readSize);

            if(kr == KERN_SUCCESS && readSize > 8) {
                // 扫描: 每个IL2CPP对象前8字节是klass指针
                // 但不是每8字节都对齐到对象, 需要启发式
                for(vm_size_t off = 0; off + 16 <= readSize; off += 8) {
                    void *testKlass = NULL;
                    memcpy(&testKlass, buf + off, 8);
                    if(testKlass == g_classLoginModel) {
                        void *candidateObj = (void*)(scan_addr + off);
                        // 验证: 检查CurSSOResponData(offset 0xf0)是否有合理指针
                        void *testSSO = NULL;
                        memcpy(&testSSO, buf + off + 0xf0, 8);
                        // offset 0xf0必须在readSize范围内
                        if(off + 0xf0 + 8 <= readSize && isValidPtr(testSSO)) {
                            const char *ssoCn = getObjClassName(testSSO);
                            if(ssoCn && (strcmp(ssoCn,"SSOResponData")==0 || strstr(ssoCn,"SSO")!=NULL)) {
                                loginModelInstance = candidateObj;
                                foundCount++;
                                jlog(@"SSO: Found LoginModel at %p (SSO=%p class=%s)", candidateObj, testSSO, ssoCn);
                            }
                        }
                        // 也试试即使SSOResponData还没设置(可能是NULL)
                        if(!isValidPtr(loginModelInstance) && foundCount == 0) {
                            // 检查字段值是否合理(LastOpenServers offset 0x10应该是List指针或NULL)
                            // 简化: 只要klass匹配就记录
                            loginModelInstance = candidateObj;
                            foundCount++;
                            jlog(@"SSO: Found LoginModel candidate at %p (klass match)", candidateObj);
                        }
                    }
                }
            }
        ...
        if(isValidPtr(loginModelInstance)) {
            g_foundLoginModel = loginModelInstance; // 缓存
            jlog(@"SSO: Cached LoginModel=%p", loginModelInstance);
        }
    }

    if(!isValidPtr(loginModelInstance)) {
        jlog(@"SSO: LoginModel instance not found");
        return;
    }
...
static void findIL2CPP(void) {
    ...
    for(size_t a=0;a<assemCount;a++){
        ...
        for(size_t c=0;c<cnt;c++){
            ...
            // v107: 搜索LoginModel类 (Singleton, 保存CurSSOResponData)
            ...
            // v107: 搜索LoginCtrl类 (有DoHandShakeCheck方法)
            ...
            while((m=get_methods(klass,&iter))!=NULL){
                ...
                // v107: LoginCtrl.DoHandShakeCheck(int32 code) — 唯一地址0x2b7eb2c, 不被泛型共享
                ...
                // v102→v107: 保留get_code/get_banTime搜索仅用于日志确认, 不再MSHookFunction它们
    ...
    // v107: 用il2cpp_class_from_name搜索热更新DLL中的LoginModel/LoginCtrl/DoHandShakeCheck
            ...
            for(size_t a=0;a<assemCount;a++){
                ...
                if(!g_classLoginModel) {
                    ...
                    if(lmClass) {
                        ...
                        jlog(@"v107: Found LoginModel class in assembly %zu: %p", a, lmClass);
                    }
                }
                ...
                if(!g_fDoHandShakeCheck) {
                    ...
                    if(lcClass) {
                        ...
                        jlog(@"v107: Found LoginCtrl class in assembly %zu: %p", a, lcClass);
                ...
                if(!g_fGetCode) {
                    ...
                    if(ssoRspClass) {
                        jlog(@"v107: Found SSOResponData class: %p", ssoRspClass);
    ...
    jlog(@"v107: DoHandShakeCheck=%p LoginModel=%p LoginCtrl=%p",
         g_fDoHandShakeCheck, g_classLoginModel, g_classLoginCtrl);
    if(!g_fDoHandShakeCheck) jlog(@"v107: DoHandShakeCheck NOT FOUND!");
    if(!g_classLoginModel) jlog(@"v107: LoginModel class NOT FOUND!");
}
...
-(void)onBypassSSO{
    ...
    if(g_bypassSSO){findIL2CPP();
        // v107: 优先hook DoHandShakeCheck (唯一地址, 不被泛型共享)
...
static void initialize(void){
    ...
    jlog(@"========== JYJH v107 (SSO bypass: GC heap scan) ==========");
