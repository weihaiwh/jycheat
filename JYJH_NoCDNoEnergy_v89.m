/**
 * v89 - 皮肤/武器/光环: 直接写Actor字段 + hook InitSpineSkin触发刷新
 * v88问题: get_SkinId和get_WeaponId地址相同(0x2ae94bc), IL2CPP泛型共享实现, hook一个覆盖另一个
 * 解决: 不hook getter, 直接写Actor对象字段(0x110=SkinId,0x114=WeaponId,0x118=HaloId)
 *        + hook InitSpineSkin(fashionId,weaponId,haloId) @ 0x2bfb050 触发游戏重新加载外观
 * 新增: Halo光环扫描(TbHalo_skin @ offset 0x258)
 * 保留v88其他所有功能
 */
...
// v89: InitSpineSkin(fashionId, weaponId, haloId) 实例方法
typedef void (*InitSpineSkinFunc)(void*,void*,int32_t,int32_t,int32_t);
...
// v89: InitSpineSkin hook
static void *g_fInitSpineSkin=NULL; static InitSpineSkinFunc g_oInitSpineSkin=NULL; static BOOL g_hInitSpineSkin=NO;
...
// ===== v89: InitSpineSkin Hook =====
// InitSpineSkin是实例方法: (MethodInfo*, self, int fashionId, int weaponId, int haloId)
static int g_initSpineLC=0;
static void hInitSpineSkin(void *mi, void *self, int32_t fashionId, int32_t weaponId, int32_t haloId) {
    if(self && !g_playerActorObj) {
        g_playerActorObj = self;
        jlog(@"InitSpineSkin: captured Actor=%p skin=%d weapon=%d halo=%d", self, fashionId, weaponId, haloId);
    }
    int32_t useFashion = (g_appliedSkinId > 0) ? g_appliedSkinId : fashionId;
    ...
    if(g_appliedSkinId > 0 || g_appliedWeaponId > 0 || g_appliedHaloId > 0) {
        if(g_initSpineLC < 30) {
            g_initSpineLC++;
            jlog(@"InitSpineSkin: %d/%d/%d -> %d/%d/%d", fashionId, weaponId, haloId, useFashion, useWeapon, useHalo);
        }
    }
    
    if(g_oInitSpineSkin) g_oInitSpineSkin(mi, self, useFashion, useWeapon, useHalo);
    
    // 同时直接写Actor字段作为备份
    if(self && g_playerActorObj == self) {
        if(g_appliedSkinId > 0) memcpy((uint8_t*)self + 0x110, &g_appliedSkinId, 4);
        if(g_appliedWeaponId > 0) memcpy((uint8_t*)self + 0x114, &g_appliedWeaponId, 4);
        if(g_appliedHaloId > 0) memcpy((uint8_t*)self + 0x118, &g_appliedHaloId, 4);
    }
}
...
static void findIL2CPP(void) {
    ...
    for(size_t a=0;a<assemCount;a++){
        ...
        for(size_t c=0;c<cnt;c++){
            ...
            while((m=get_methods(klass,&iter))!=NULL){
                ...
                // v89: 搜索InitSpineSkin
                else if(strcmp(n,"InitSpineSkin")==0&&pc==3&&!g_fInitSpineSkin){g_fInitSpineSkin=fa;found++;jlog(@"FOUND %s.%s p=%u %p",cn?:"?",n,pc,fa);}
            }
        }
    }
    ...
    jlog(@"v89: UnityGameEntry=%p HotfixGameEntry=%p InitSpineSkin=%p",g_classUnityGameEntry,g_classHotfixGameEntry,g_fInitSpineSkin);
...
// v89: 通用应用函数 - 写Actor字段 + 重调InitSpineSkin触发刷新
static void applyPartChange(void) {
    if(!g_playerActorObj) {
        jlog(@"applyPartChange: no Actor obj yet");
        return;
    }
    // 直接写Actor字段
    if(g_appliedSkinId > 0) memcpy((uint8_t*)g_playerActorObj + 0x110, &g_appliedSkinId, 4);
    if(g_appliedWeaponId > 0) memcpy((uint8_t*)g_playerActorObj + 0x114, &g_appliedWeaponId, 4);
    if(g_appliedHaloId > 0) memcpy((uint8_t*)g_playerActorObj + 0x118, &g_appliedHaloId, 4);
    
    // 调用InitSpineSkin触发游戏刷新外观
    // 如果已经hook了InitSpineSkin, 我们的hook会自动替换参数
    // 所以这里直接用原函数即可(传入当前值, hook会替换)
    int32_t curSkin=0, curWeapon=0, curHalo=0;
    memcpy(&curSkin, (uint8_t*)g_playerActorObj + 0x110, 4);
    memcpy(&curWeapon, (uint8_t*)g_playerActorObj + 0x114, 4);
    memcpy(&curHalo, (uint8_t*)g_playerActorObj + 0x118, 4);
    
    if(g_oInitSpineSkin) {
        jlog(@"applyPartChange: calling InitSpineSkin(%d,%d,%d) on Actor=%p", curSkin, curWeapon, curHalo, g_playerActorObj);
        g_oInitSpineSkin(NULL, g_playerActorObj, curSkin, curWeapon, curHalo);
    } else {
        jlog(@"applyPartChange: InitSpineSkin not hooked, only wrote fields");
    }
}
...
// v89: 皮肤/武器/光环 独立应用 - hook InitSpineSkin + 直接写字段
-(void)ensureInitSpineSkinHooked {
    if(!g_hInitSpineSkin) {
        findIL2CPP();
        if(g_fInitSpineSkin) hookOneFunc(g_fInitSpineSkin, hInitSpineSkin, (void**)&g_oInitSpineSkin, &g_hInitSpineSkin, "InitSpineSkin");
    }
}
-(void)onApplySkin{
    if(!g_skinIdsLoaded){jlog(@"ApplySkin: scan first");return;}
    [self ensureInitSpineSkinHooked];
...
static void setupUI(void){
    ...
    UILabel *tl=[[UILabel alloc]initWithFrame:CGRectMake(10,6,g_panelW-20,20)];tl.text=@"剑影江湖 v89";tl.textColor=IMGUI_ACCENT;tl.font=[UIFont boldSystemFontOfSize:15];tl.textAlignment=NSTextAlignmentCenter;[tb addSubview:tl];
...
static void initialize(void){
    ...
    jlog(@"========== JYJH v89 (InitSpineSkin + Halo) ==========");
