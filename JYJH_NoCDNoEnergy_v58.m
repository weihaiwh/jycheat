/**
 * v58.0 - 修复扫描皮肤ID卡死(静态字段偏移错误+内存访问崩溃)
 * v57问题:
 *   日志: staticData=0x129c0ae60 configComp=0x129cac0d2 tables=0x202020202020202
 *   configComp=0x129cac0d2 不像合法指针(地址太低且奇数), tables=0x2020...是ASCII空格
 *   → staticData+0xa8读到了错误数据, GameEntryMain._config偏移0xa8不对
 *   → 继续用错误指针读取tables后, 0x202020202020202解引用→崩溃/卡死
 * v58方案:
 *   1. 修复静态字段偏移: IL2CPP static field的实际偏移需要通过
 *      il2cpp_field_get_offset()获取,而非用dump偏移
 *   2. 在findIL2CPP中用il2cpp_class_get_field_from_name获取GameEntryMain._config字段
 *      然后用il2cpp_field_get_offset获取真实偏移
...
// v58: 用il2cpp_field_get_offset获取真实静态字段偏移,不再硬编码
typedef void* (*Il2CppClassGetStaticFieldDataFunc)(void*);
static void *g_classGameEntryMain=NULL;
static size_t g_configFieldOffset=0; // GameEntryMain._config的真实偏移
// 指针合法性检查: 非NULL且看起来像合法的堆指针(高位非全0/全F)
static BOOL isValidPtr(void *p) {
    if(!p) return NO;
    uint64_t v=0; memcpy(&v,&p,8);
    // ARM64堆指针通常在0x100000000~0x1FFFFFFFFF范围内
...
static void scanSkinIds(void) {
    ...
    jlog(@"ScanSkin: staticData=%p configOffset=%zu",staticData,g_configFieldOffset);
    
    // v58: 用il2cpp_field_get_offset获取的偏移, 如果为0则用dump值0xa8试
    size_t off=g_configFieldOffset?g_configFieldOffset:0xa8;
    void *configComp=NULL;
    memcpy(&configComp,(uint8_t*)staticData+off,8);
    jlog(@"ScanSkin: off=0x%zx configComp=%p",off,configComp);
    
    // 如果指针不合法,试其他偏移
    if(!isValidPtr(configComp)){
        jlog(@"ScanSkin: configComp invalid, trying offsets...");
        // 试 dump-0x10 = 0x98
        off=0x98; memcpy(&configComp,(uint8_t*)staticData+off,8);
        jlog(@"ScanSkin: off=0x98: %p",configComp);
        if(!isValidPtr(configComp)){
            // 试其他常见偏移
            size_t tries[]={0x0,0x8,0x10,0x18,0x20,0x28,0x30,0x38,0x40,0x48,0x50,0x58,0x60,0x68,0x70,0x78,0x80,0x88,0x90};
            for(int i=0;i<19;i++){
                memcpy(&configComp,(uint8_t*)staticData+tries[i],8);
                if(isValidPtr(configComp)){
                    jlog(@"ScanSkin: found valid ptr at off=0x%zx: %p",tries[i],configComp);
                    off=tries[i];
                    break;
                }
                configComp=NULL;
    ...
    if(!isValidPtr(configComp)){jlog(@"ScanSkin: no valid ConfigComponent ptr found");return;}
    ...
    if(!isValidPtr(tables)){
        // 可能ConfigComponent也需要减0x10, 试+0x18
        memcpy(&tables,(uint8_t*)configComp+0x18,8);
        jlog(@"ScanSkin: retry tables at +0x18: %p",tables);
    }
    if(!isValidPtr(tables)){jlog(@"ScanSkin: Tables ptr invalid");return;}
    ...
    if(!isValidPtr(tbRoleSkin)){jlog(@"ScanSkin: TbRoleSkin ptr invalid");return;}
    ...
    if(!itemsArray||listSize<=0||!isValidPtr(itemsArray)){jlog(@"ScanSkin: List empty or invalid itemsArray");return;}
    ...
    for(size_t a=0;a<assemCount;a++){
        ...
        for(size_t c=0;c<cnt;c++){
            ...
            if(cn&&strcmp(cn,"GameEntryMain")==0&&!g_classGameEntryMain){
                ...
                // v58: 用il2cpp_field_get_offset获取_config字段的真实偏移
                if(class_get_field){
                    void *f=class_get_field(klass,"_config");
                    if(f&&field_get_offset){
                        g_configFieldOffset=field_get_offset(f);
                        jlog(@"FOUND GameEntryMain._config offset=%zu(0x%zx)",g_configFieldOffset,g_configFieldOffset);
                    }
                }
                // 打印所有静态字段偏移用于调试
                if(get_fields&&field_get_name&&field_get_offset){
                    void *fi=NULL; int fc=0;
                    while(fc<30){
                        void *fld=get_fields(klass,&fi);
                        if(!fld)break;
                        const char *fn=field_get_name(fld);
                        size_t fo=field_get_offset(fld);
                        uint32_t ff=field_get_flags?field_get_flags(fld):0;
                        jlog(@"  GEM field[%d]: %s offset=0x%zx flags=0x%x",fc,fn?:"?",fo,ff);
                        fc++;
                    }
                }
            }
...
static void setupUI(void){
    ...
    UILabel *tl=[[UILabel alloc]initWithFrame:CGRectMake(10,6,g_panelW-20,20)];tl.text=@"\xe5\x89\x91\xe5\xbd\xb1\xe6\xb1\x9f\xe6\xb9\x96 v58.0";tl.textColor=IMGUI_ACCENT;tl.font=[UIFont boldSystemFontOfSize:15];tl.textAlignment=NSTextAlignmentCenter;[tb addSubview:tl];
    // UIScrollView (滚动内容区)
