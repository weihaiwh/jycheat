/**
 * v88 - 基于v71完整功能 + MSHookFunction
 * 恢复v71的技能替换(UseSkill修改skillStateType) + UpdateSkillCD跳过
 * 恢复v83的get_SkinId hook实现皮肤替换
 * 新增: 皮肤和武器分开应用(独立按钮)
 * 新增: 伤害倍数(Damage hook多次调用原函数)
 * 伤害范围1-100000, 默认100
...
static int g_damageMulti=1; // v88: 伤害倍数(1-10)
...
typedef int32_t (*GetWeaponIdFunc)(void*);
...
// v88: 皮肤hook - get_SkinId / get_WeaponId 分开
static void *g_fGetSkinId=NULL; static GetSkinIdFunc g_oGetSkinId=NULL; static BOOL g_hGetSkinId=NO;
static void *g_fGetWeaponId=NULL; static GetWeaponIdFunc g_oGetWeaponId=NULL; static BOOL g_hGetWeaponId=NO;
static void *g_playerActorObj=NULL;
static int32_t g_appliedSkinId=0, g_appliedWeaponId=0;
static int g_skinIdHookLC=0, g_weaponIdHookLC=0;
...
static int32_t hGetWeaponId(void *self) {
    int32_t r=g_oGetWeaponId?g_oGetWeaponId(self):0;
    if(self==g_playerActorObj && g_appliedWeaponId>0) {
        if(g_weaponIdHookLC<10){g_weaponIdHookLC++;jlog(@"get_WeaponId: %d->%d",r,g_appliedWeaponId);}
        return g_appliedWeaponId;
    }
    return r;
}
...
static void *getConfigViaHotfixEntry(void) {
    ...
    for(int i=0;i<24;i++){ uint64_t v=0; memcpy(&v,(uint8_t*)sd+i*8,8); if(v) jlog(@"  HFGE.SD[+0x%x]=0x%llx",i*8,v); }
    void *config=NULL;
    memcpy(&config,(uint8_t*)sd+0xa8,8);
    if(isValidPtr(config)) { jlog(@"getConfigHF: _config=%p",config); return config; }
    return NULL;
}
...
static void scanSkinIds(void) {
    ...
    if(!isValidPtr(cc)){jlog(@"ScanSkin: ConfigComponent NULL");g_skinIdsLoaded=NO;return;}
    jlog(@"ScanSkin: configComp=%p",cc);
    void *tables_l=NULL; memcpy(&tables_l,(uint8_t*)cc+0x28,8);
    if(!isValidPtr(tables_l)){jlog(@"ScanSkin: tables invalid");g_skinIdsLoaded=NO;return;}
    void *tbRoleSkin=NULL; memcpy(&tbRoleSkin,(uint8_t*)tables_l+0x230,8);
    if(!isValidPtr(tbRoleSkin)){jlog(@"ScanSkin: TbRoleSkin invalid");g_skinIdsLoaded=NO;return;}
    void *dataList=NULL; memcpy(&dataList,(uint8_t*)tbRoleSkin+0x18,8);
    if(!isValidPtr(dataList)){jlog(@"ScanSkin: _dataList invalid");g_skinIdsLoaded=NO;return;}

    jlog(@"ScanSkin: dumping dataList (96 bytes):");
    for(int i=0;i<12;i++){ uint64_t v=0; memcpy(&v,(uint8_t*)dataList+i*8,8); jlog(@"  DL[+0x%x]=0x%llx",i*8,v); }
    ...
    if(!itemsArray||listSize<=0){jlog(@"ScanSkin: List empty");g_skinIdsLoaded=NO;return;}
    ...
    for(int i=0;i<maxScan&&i<arrayLen;i++){
        ...
        if(!p)continue; int32_t id=0; memcpy(&id,(uint8_t*)p+0x10,4);
        if(id>0) g_roleSkinIds[g_roleSkinCount++]=id;
    }
    g_skinIdsLoaded=YES;
    ...
    // WeaponSkin
    void *tbWeaponSkin=NULL; memcpy(&tbWeaponSkin,(uint8_t*)tables_l+0x248,8);
    if(isValidPtr(tbWeaponSkin)){
        void *wDataList=NULL; memcpy(&wDataList,(uint8_t*)tbWeaponSkin+0x18,8);
        if(isValidPtr(wDataList)){
            ...
            if(wItemsArray&&wListSize>0){
                int32_t wArrLen=0; memcpy(&wArrLen,(uint8_t*)wItemsArray+0x18,4);
                ...
                for(int i=0;i<wMax&&i<wArrLen;i++){
                    void *wsPtr=NULL; memcpy(&wsPtr,(uint8_t*)wItemsArray+0x20+i*8,8);
                    if(!wsPtr)continue; int32_t wId=0; memcpy(&wId,(uint8_t*)wsPtr+0x10,4);
...
static int64_t hDamage(void *f,void *atkEnt,void *atkCF,void *tgtEnt,void *tgtCF,
    ...
    if(atkP && g_damageMulti>1 && !tgtP) {
        ...
        if(g_dmgLC<20){g_dmgLC++;jlog(@"Dmg[%d]=%lld enemies=%d sBtn=%d",g_dmgLC,r,g_enemyCount,sBtn);}
    }
    return r;
}
...
// v88: UseSkill hook - 恢复v71的技能替换逻辑
// MSHookFunction下修改参数传给原函数是OK的(v83/v87日志已验证)
...
// v88: UpdateSkillCD - 恢复v71逻辑: 技能替换时跳过CD更新
...
static void hHitSystemUpdate(void *self, void *framePtr) {
    ...
    if(g_hitSysLC<3) {
        ...
        for(int i=0;i<16;i++){ int64_t v=0; memcpy(&v,p+i*8,8); jlog(@"  [+0x%x]=%lld(0x%llx)",i*8,v,v); }
    }
    ...
    for(size_t a=0;a<assemCount;a++){
        ...
        for(size_t c=0;c<cnt;c++){
            ...
            while((m=get_methods(klass,&iter))!=NULL){
                ...
                // v88: 也搜索get_WeaponId
                else if(strcmp(n,"get_WeaponId")==0&&pc==0&&cn&&strcmp(cn,"Actor")==0&&!g_fGetWeaponId){g_fGetWeaponId=fa;found++;jlog(@"FOUND %s.%s p=%u %p",cn?:"?",n,pc,fa);}
    ...
    if(!g_classHotfixGameEntry){
        for(int i=0;i<g_gameEntryCount;i++){
            if(g_allGameEntryNS[i]==NULL||strcmp(g_allGameEntryNS[i],"")==0){
                ...
                jlog(@"v88: assume GameEntry[%d] ns=empty is HotfixGameEntry=%p",i,g_allGameEntries[i]);
                break;
            }
        }
    }
    jlog(@"v88: UnityGameEntry=%p HotfixGameEntry=%p",g_classUnityGameEntry,g_classHotfixGameEntry);
...
static UIButton *g_btnApplySkin=nil,*g_btnApplyWeapon=nil,*g_btnScanSkin=nil;
static UISlider *g_slider=nil,*g_skinSlider=nil,*g_weaponSlider=nil,*g_speedSlider=nil,*g_dmgMultiSlider=nil;
static UILabel *g_sliderLabel=nil,*g_skinLabel=nil,*g_weaponLabel=nil,*g_speedLabel=nil,*g_dmgMultiLabel=nil;
static BOOL g_panelOpen=NO;
static CGFloat g_panelW=360, g_panelH=700;
...
static void refreshBtns(void){
    ...
    if(g_btnApplySkin){
        ...
        else{[g_btnApplySkin setTitle:@"先扫描皮肤" forState:UIControlStateNormal];g_btnApplySkin.backgroundColor=[UIColor colorWithRed:0.35 green:0.14 blue:0.14 alpha:0.95];g_btnApplySkin.layer.borderColor=IMGUI_RED.CGColor;}
    }
    // v88: 武器按钮
    if(g_btnApplyWeapon){
        ...
        else{[g_btnApplyWeapon setTitle:@"先扫描皮肤" forState:UIControlStateNormal];g_btnApplyWeapon.backgroundColor=[UIColor colorWithRed:0.35 green:0.14 blue:0.14 alpha:0.95];g_btnApplyWeapon.layer.borderColor=IMGUI_RED.CGColor;}
    }
    if(g_btnScanSkin){
        if(g_skinIdsLoaded){[g_btnScanSkin setTitle:[NSString stringWithFormat:@"已扫描:皮肤%d/武器%d",g_roleSkinCount,g_weaponSkinCount] forState:UIControlStateNormal];g_btnScanSkin.backgroundColor=[UIColor colorWithRed:0.16 green:0.52 blue:0.28 alpha:0.95];}
...
-(void)dmgMultiChanged:(UISlider*)s{
    g_damageMulti=(int)roundf(s.value);
    if(g_damageMulti<1) g_damageMulti=1;
    g_dmgMultiLabel.text=[NSString stringWithFormat:@"伤害倍数: ×%d",g_damageMulti];
    if(g_damageMulti>1 && !g_hDamage) {
        findIL2CPP();
        if(!g_hDamage) hookOneFunc(g_fDamage,hDamage,(void**)&g_oDamage,&g_hDamage,"Damage");
    }
    jlog(@"DamageMulti: ×%d",g_damageMulti);
}
...
-(void)onApplySkin{
    ...
    if(!g_fGetSkinId){findIL2CPP();if(!g_fGetSkinId){jlog(@"ApplySkin: get_SkinId not found");return;}}
    if(!g_hGetSkinId) hookOneFunc(g_fGetSkinId,hGetSkinId,(void**)&g_oGetSkinId,&g_hGetSkinId,"get_SkinId");
    g_appliedSkinId=g_skinId;
    jlog(@"ApplySkin: skin=%d (via get_SkinId hook)",g_skinId);
    refreshBtns();
}
-(void)onApplyWeapon{
    if(!g_skinIdsLoaded){jlog(@"ApplyWeapon: scan skins first");return;}
    if(!g_fGetWeaponId){findIL2CPP();if(!g_fGetWeaponId){jlog(@"ApplyWeapon: get_WeaponId not found, trying skinId hook for weapon");return;}}
    if(!g_hGetWeaponId) hookOneFunc(g_fGetWeaponId,hGetWeaponId,(void**)&g_oGetWeaponId,&g_hGetWeaponId,"get_WeaponId");
    g_appliedWeaponId=g_weaponId;
    jlog(@"ApplyWeapon: weapon=%d (via get_WeaponId hook)",g_weaponId);
    refreshBtns();
}
-(void)onDumpSkinIds{
...
static void setupUI(void){
    ...
    UILabel *tl=[[UILabel alloc]initWithFrame:CGRectMake(10,6,g_panelW-20,20)];tl.text=@"剑影江湖 v88";tl.textColor=IMGUI_ACCENT;tl.font=[UIFont boldSystemFontOfSize:15];tl.textAlignment=NSTextAlignmentCenter;[tb addSubview:tl];
    ...
    // 5: 1 2 3 4 5 小按钮
    // 6: --- 分隔线 ---
    //     伤害上限 slider (1-100000, 默认100)
    //     伤害倍数 slider (1-10, 默认1)
    //     --- 分隔线 ---
    //     皮肤/武器区域
    //     皮肤slider + 皮肤按钮
    //     武器slider + 武器按钮
    ...
    CGFloat contentH=by0+bdy*6+8+28+20+28+20+28+8+18+20+28+20+28+24+8+18+20+28+60;
    ...
    secT.text=g_skinIdsLoaded?[NSString stringWithFormat:@"皮肤/武器 (皮肤%d/武器%d)",g_roleSkinCount,g_weaponSkinCount]:@"皮肤/武器 (先点扫描)";
    secT.textColor=IMGUI_ACCENT;secT.font=[UIFont boldSystemFontOfSize:11];[sv addSubview:secT];

    // 皮肤slider
    g_skinLabel=[[UILabel alloc]initWithFrame:CGRectMake(bx,ssy+20,bw,18)];g_skinLabel.text=[NSString stringWithFormat:@"皮肤ID: %d",g_skinId];g_skinLabel.textColor=IMGUI_DIMTEXT;g_skinLabel.font=[UIFont systemFontOfSize:12];[sv addSubview:g_skinLabel];
    ...
    g_btnApplySkin=mkBtn(CGRectMake(bx,ssy+68,bw,bh),@selector(onApplySkin));[sv addSubview:g_btnApplySkin];

    // 武器slider
    g_weaponLabel=[[UILabel alloc]initWithFrame:CGRectMake(bx,ssy+96,bw,18)];g_weaponLabel.text=[NSString stringWithFormat:@"武器ID: %d",g_weaponId];g_weaponLabel.textColor=IMGUI_DIMTEXT;g_weaponLabel.font=[UIFont systemFontOfSize:12];[sv addSubview:g_weaponLabel];
    g_weaponSlider=[[UISlider alloc]initWithFrame:CGRectMake(bx,ssy+114,bw,28)];g_weaponSlider.minimumValue=0;g_weaponSlider.maximumValue=weaponMax;g_weaponSlider.value=0;[g_weaponSlider addTarget:[JYJHActionHandler shared] action:@selector(weaponSliderChanged:) forControlEvents:UIControlEventValueChanged];[sv addSubview:g_weaponSlider];
    g_btnApplyWeapon=mkBtn(CGRectMake(bx,ssy+144,bw,bh),@selector(onApplyWeapon));[sv addSubview:g_btnApplyWeapon];

    g_btnScanSkin=mkBtn(CGRectMake(bx,ssy+172,bw,bh),@selector(onDumpSkinIds));[sv addSubview:g_btnScanSkin];

    // === 移动速度 ===
    CGFloat s3Y=ssy+200;UIView *s3=[[UIView alloc]initWithFrame:CGRectMake(bx,s3Y,bw,1)];s3.backgroundColor=IMGUI_BORDER;[sv addSubview:s3];
    CGFloat spy=s3Y+4;
...
static void initialize(void){
    ...
    jlog(@"========== JYJH v88 (基于v71+MSHookFunction) ==========");
