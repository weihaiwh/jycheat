/**
 * v99 - 用运行时真实偏移修复所有字段 + SetData hook正确提取LobbyActorData
 * v98运行时API dump结果:
 *   LobbyActorData: mSceneData@0x10, NeedUpdatePart@0x28, IsSelf@0x2b — 和dump一致!
 *   LobbyActorSpineAvatar: mLobbyActorData@0xd0 (不是0xb8!)
 *   ScenePlayerData: skinId@0x40, weaponId@0x44, fashionId@0x48 — 和dump一致!
 *   ActorData: SkinId@0x14, WeaponId@0x18, HaloId@0x1c — 和dump一致!
 * v98问题:
 *   1. mLobbyActorData偏移用0xb8(错), 应该是0xd0
 *   2. SetData第2参数不是LobbyActorData, g_lobbyActorData指向错误对象
 *   3. IsSelf=NO因为检查了错误的对象
 * v99方案:
 *   1. 用运行时偏移: mLobbyActorData@0xd0
 *   2. SetData hook中: 从self+0xd0读mLobbyActorData, 而不是用第2参数
 *   3. 通过mLobbyActorData->mSceneData(0x10)修改skinId/weaponId
 *   4. 设置NeedUpdatePart=true
 *   5. 不主动调用任何IL2CPP函数!
 */
...
// v99: 运行时真实偏移 (v98 il2cpp_class_get_fields确认)
...
static int32_t g_off_mActorData=0x78;       // SceneActorSpineAvatar.mActorData (NOT confirmed)
static int32_t g_off_mLobbyActorData=0xd0;  // LobbyActorSpineAvatar.mLobbyActorData ✓ (0xb8→0xd0!)
...
// ===== v99: Hook LobbyActorSpineAvatar.SetData =====
// 关键: SetData的第2参数不是LobbyActorData! 
// 必须从self+0xd0读mLobbyActorData
static int g_setDataLC=0;
static void hSetData(void *mi, void *self, void *param2) {
    if(g_oSetData) ((SetDataOrigFunc)g_oSetData)(mi, self, param2);
    if(!self) return;
    g_lobbyAvatarObj = self;
    
    // v99: 从avatar实例读mLobbyActorData (偏移0xd0, 运行时确认!)
    void *lobbyData = NULL;
    memcpy(&lobbyData, (uint8_t*)self + g_off_mLobbyActorData, 8);
    if(isValidPtr(lobbyData)) {
        g_lobbyActorData = lobbyData;
        
        // 验证IsSelf
        BOOL isSelf = NO;
        memcpy(&isSelf, (uint8_t*)lobbyData + g_off_IsSelf, 1);
        if(isSelf && g_setDataLC < 5) {
            g_setDataLC++;
            jlog(@"SetData[IsSelf]: avatar=%p mLobbyActorData=%p (from +0x%x)", self, lobbyData, g_off_mLobbyActorData);
            
            // 读mSceneData验证
            void *sd = NULL;
            memcpy(&sd, (uint8_t*)lobbyData + g_off_mSceneData, 8);
            if(isValidPtr(sd)) {
                int32_t s=0,w=0,f=0;
                memcpy(&s,(uint8_t*)sd+g_off_skinId,4);
                memcpy(&w,(uint8_t*)sd+g_off_weaponId,4);
                memcpy(&f,(uint8_t*)sd+g_off_fashionId,4);
                jlog(@"  mSceneData=%p skin=%d weapon=%d fashion=%d", sd, s, w, f);
            } else {
                jlog(@"  mSceneData at +0x%x is NULL/invalid", g_off_mSceneData);
                // dump mLobbyActorData来找mSceneData
                jlog(@"  mLobbyActorData dump (0x00-0x30):");
                for(int off=0; off<0x30; off+=8) {
                    uint64_t v=0; memcpy(&v,(uint8_t*)lobbyData+off,8);
...
static void setupUI(void){
    ...
    UILabel *tl=[[UILabel alloc]initWithFrame:CGRectMake(10,6,g_panelW-20,20)];tl.text=@"剑影江湖 v99";tl.textColor=IMGUI_ACCENT;tl.font=[UIFont boldSystemFontOfSize:15];tl.textAlignment=NSTextAlignmentCenter;[tb addSubview:tl];
...
static void initialize(void){
    ...
    jlog(@"========== JYJH v99 (real offsets: mLobbyActorData@0xd0) ==========");
