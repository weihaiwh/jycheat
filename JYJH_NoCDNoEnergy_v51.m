/**
 * v51.0 - 移除MonsterDeadSignal + 修复UI(面板拖动+大空隙)
 * v50问题:
 *   1. MonsterDeadSignal调用导致游戏卡住 - Signal不能直接调用
 *   2. 面板拖动: 顶部有空间时不能继续往上移动
 *   3. 下半部分仍然没有空隙 - resize把手无法点击
 * v51方案:
 *   1. 移除MonsterDeadSignal调用,保留纯DecreaseHP扩散
 *   2. 面板拖动: y∈[20,screenH-panelH-20]确保上下都不顶边
 *   3. ScrollView高度panelH-100,底部留70px大空隙
 *   4. 接受现实: 敌人空血不死可能是帧同步网络验证无法绕过
 */
...
-(void)touchesMoved:(NSSet*)t withEvent:(UIEvent*)e{
    ...
    if(g_scrollView){
        // v51: 留出底部大空隙70px
        g_scrollView.frame=CGRectMake(0,32,nw,nh-100);
        // contentSize宽度也要更新
...
-(void)touchesMoved:(NSSet*)t withEvent:(UIEvent*)e{
    ...
    // v51: 修复拖动限制,确保上下都不顶边
    f.origin.y=MAX(20,MIN(sc.size.height-f.size.height-20,f.origin.y+dy));
    g_panel.frame=f;
    _ts=c;
}
@end
...
static void setupUI(void){
    ...
    UILabel *tl=[[UILabel alloc]initWithFrame:CGRectMake(10,6,g_panelW-20,20)];tl.text=@"\xe5\x89\x91\xe5\xbd\xb1\xe6\xb1\x9f\xe6\xb9\x96 v51.0";tl.textColor=IMGUI_ACCENT;tl.font=[UIFont boldSystemFontOfSize:15];tl.textAlignment=NSTextAlignmentCenter;[tb addSubview:tl];
    // UIScrollView (滚动内容区, v51: 留出大空隙)
    UIScrollView *sv=[[UIScrollView alloc]initWithFrame:CGRectMake(0,32,g_panelW,g_panelH-100)];
    ...
    CGFloat contentH=by0+bdy*4+60+120-80; // 底部留更多空间
