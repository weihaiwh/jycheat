/**
 * v77 - 基于稳定v74 + Time.timeScale内存直写
 * v75 DobbyHook timeScale闪退, v76 IL2CPP函数调用传NULL MethodInfo*可能崩
 * v77方案: 通过il2cpp_class_get_static_field_data找到Time类静态数据区,
 *   直接memcpy写s_timeScale字段(float), 不走任何函数调用
 *   用GCD timer每0.5s刷新, 确保游戏不会重置timeScale
 *   保留v74所有稳定功能, 去掉MoveStep hook(帧同步游戏效果有限)
 */
...
static float g_gameSpeed=1.0f; // v77: 游戏速度倍率(timeScale) 1.0=正常

// ===== v77: Time.timeScale 内存直写 =====
...
static void initTimeScale(void) {
    ...
    if(!g_timeKlass) { jlog(@"initTimeScale: Time klass not found (run findIL2CPP first)"); return; }
    g_timeClassSD=get_sd(g_timeKlass);
    jlog(@"initTimeScale: Time SD=%p",g_timeClassSD);
    if(!g_timeClassSD) { jlog(@"initTimeScale: SD is NULL"); return; }
    ...
    if(g_timeScaleOff>=0) {
        ...
        jlog(@"initTimeScale: FAILED to find timeScale offset");
    }
}
...
// v77: MoveStep hook removed (帧同步游戏效果有限, 改用timeScale)
...
// v77: hMoveStep removed, using timeScale instead
    ...
    for(size_t a=0;a<assemCount;a++){
        ...
        for(size_t c=0;c<cnt;c++){
            ...
            while((m=get_methods(klass,&iter))!=NULL){
                ...
                // v77: MoveStep removed, using timeScale instead
                // v77: Time类通过klass搜索, 不需要方法搜索
            }
        }
    }
    ...
    if(!g_classHotfixGameEntry){
        for(int i=0;i<g_gameEntryCount;i++){
            if(g_allGameEntryNS[i]==NULL||strcmp(g_allGameEntryNS[i],"")==0){
                ...
                jlog(@"v77: assume GameEntry[%d] ns=empty is HotfixGameEntry=%p",i,g_allGameEntries[i]);
                break;
            }
        }
    }
    jlog(@"v77: UnityGameEntry=%p HotfixGameEntry=%p TimeKlass=%p",g_classUnityGameEntry,g_classHotfixGameEntry,g_timeKlass);
...
// v77: 游戏速度slider (timeScale内存直写)
-(void)speedSliderChanged:(UISlider*)s{
    g_gameSpeed=s.value;
    if(g_gameSpeed>1.0f) {
        if(!g_timeKlass) findIL2CPP();
        if(g_timeKlass && !g_timeClassSD) initTimeScale();
        if(g_timeClassSD && g_timeScaleOff>=0) {
            writeTimeScale(g_gameSpeed);
            startSpeedTimer();
        } else {
            jlog(@"speedSlider: timeScale not initialized, cannot apply");
        }
    } else {
        // 恢复正常速度
        stopSpeedTimer();
...
static void setupUI(void){
    ...
    UILabel *tl=[[UILabel alloc]initWithFrame:CGRectMake(10,6,g_panelW-20,20)];tl.text=@"\xe5\x89\x91\xe5\xbd\xb1\xe6\xb1\x9f\xe6\xb9\x96 v77";tl.textColor=IMGUI_ACCENT;tl.font=[UIFont boldSystemFontOfSize:15];tl.textAlignment=NSTextAlignmentCenter;[tb addSubview:tl];
...
static void initialize(void){
    ...
    jlog(@"========== JYJH v77 (timeScale memory write) ==========");
    jlog(@"iOS %@",[[UIDevice currentDevice] systemVersion]);
    // v77: 无反检测, Shadow处理越狱隐藏; timeScale通过内存直写
    // 直接启动: 2s后findIL2CPP+applyAllHooks+initTimeScale, 5s后setupUI
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW,(int64_t)(2.0*NSEC_PER_SEC)),dispatch_get_main_queue(),^{
        jlog(@"2s: finding IL2CPP methods...");
        findIL2CPP();
        applyAllHooks();
        if(g_timeKlass) {
            jlog(@"2s: initializing timeScale...");
            initTimeScale();
        }
