/**
 * v81 - 基于稳定v71, 修复皮肤+移动速度
 * v71稳定但: 移动速度没效果, 皮肤没效果
 * v81改动:
 *   1. 移动速度: MoveStep hook修改cf->MoveSpped已有, 但也放大moveDir位移向量
 *   2. 皮肤: hook get_SkinId返回值 + 调用InitWithSkinId真正切换模型
 * 保持v71的5秒延迟启动(2秒启动会闪退)
 * 不加timeScale(帧同步游戏, 伤害由服务器帧驱动)
 */
...
// v81: 皮肤Hook - get_SkinId返回值 + InitWithSkinId真正切换模型
...
// v81: InitWithSkinId - SceneActorSpineAvatar.InitWithSkinId(int skinId, int changeType)
...
// v81: set_SkinId - Actor.set_SkinId(int value)
    ...
    for(size_t a=0;a<assemCount;a++){
        ...
        for(size_t c=0;c<cnt;c++){
            ...
            while((m=get_methods(klass,&iter))!=NULL){
                ...
                else if(strcmp(n,"InitWithSkinId")==0&&pc==2&&!g_fInitWithSkinId){g_fInitWithSkinId=fa;found++;jlog(@"FOUND %s.%s p=%u %p (skin switch)",cn?:"?",n,pc,fa);}
    ...
    if(!g_classHotfixGameEntry){
        for(int i=0;i<g_gameEntryCount;i++){
            if(g_allGameEntryNS[i]==NULL||strcmp(g_allGameEntryNS[i],"")==0){
                ...
                jlog(@"v81: assume GameEntry[%d] ns=empty is HotfixGameEntry=%p",i,g_allGameEntries[i]);
                break;
            }
        }
    }
    jlog(@"v81: UnityGameEntry=%p HotfixGameEntry=%p",g_classUnityGameEntry,g_classHotfixGameEntry);
    jlog(@"v81: InitWithSkinId=%p set_SkinId=%p get_SkinId=%p",
         g_fInitWithSkinId,g_fSetSkinId,g_fGetSkinId);
...
-(void)onApplySkin{
    ...
    if(g_playerActorObj && g_fInitWithSkinId) {
        ...
        jlog(@"ApplySkin: InitWithSkinId not found, only hooking get_SkinId");
    }
...
static void setupUI(void){
    ...
    UILabel *tl=[[UILabel alloc]initWithFrame:CGRectMake(10,6,g_panelW-20,20)];tl.text=@"\xe5\x89\x91\xe5\xbd\xb1\xe6\xb1\x9f\xe6\xb9\x96 v81";tl.textColor=IMGUI_ACCENT;tl.font=[UIFont boldSystemFontOfSize:15];tl.textAlignment=NSTextAlignmentCenter;[tb addSubview:tl];
