/**
 * v87 - 修复大招闪退 + 去掉皮肤功能
 * v86日志: 大招st=15->19后crash, 皮肤skin=167722928(offset错)
 * 问题1: UseSkill替换skillStateType后传给原函数crash
 *   原因: MSHookFunction的trampoline在修改参数后传给原函数时可能不兼容
 *   修复: UseSkill不再修改skillStateType, 技能替换只靠IsReady/AttackCanUse返回YES
 * 问题2: 皮肤get_SkinId hook在MSHookFunction下闪退
 *   修复: 暂时去掉皮肤hook, 只保留扫描功能
 */
...
// v87: 去掉皮肤hook - get_SkinId hook在MSHookFunction下闪退
// 保留扫描功能供查看
static int32_t g_appliedSkinId=0, g_appliedWeaponId=0;
static void *g_fGetSkinId=NULL; // 仅搜索不Hook
...
// v87: UseSkill hook - 不修改参数, 技能替换通过IsReady/AttackCanUse实现
// v86的问题: 修改skillStateType=19后传给原函数crash (MSHookFunction trampoline不兼容参数修改)
// 新方案: 不替换skillStateType, 技能替换只靠IsReady/AttackCanUse返回YES
// 大招通过exSkillNoCD开关让st>=17的技能随时可用
...
static void hUseSkill(void *f,void *entity,void *cf,int skillStateType,BOOL isRight,void *states,void *state,void *playerInfo) {
    // v87: 不修改任何参数, 直接传给原函数
    if(g_useSkillLC<5){g_useSkillLC++;jlog(@"UseSkill: st=%d isRight=%d",skillStateType,isRight);}
    ...
    for(size_t a=0;a<assemCount;a++){
        ...
        for(size_t c=0;c<cnt;c++){
            ...
            while((m=get_methods(klass,&iter))!=NULL){
                ...
                else if(strcmp(n,"get_SkinId")==0&&pc==0&&cn&&strcmp(cn,"Actor")==0&&!g_fGetSkinId){g_fGetSkinId=fa;found++;jlog(@"FOUND %s.%s p=%u %p (no hook)",cn?:"?",n,pc,fa);}
...
-(void)onApplySkin{
    // v87: 皮肤hook已禁用, 只记录日志
    jlog(@"ApplySkin: skin feature disabled in v87");
}
@end
...
static void setupUI(void){
    ...
    UILabel *tl=[[UILabel alloc]initWithFrame:CGRectMake(10,6,g_panelW-20,20)];tl.text=@"\xe5\x89\x91\xe5\xbd\xb1\xe6\xb1\x9f\xe6\xb9\x96 v87";tl.textColor=IMGUI_ACCENT;tl.font=[UIFont boldSystemFontOfSize:15];tl.textAlignment=NSTextAlignmentCenter;[tb addSubview:tl];
