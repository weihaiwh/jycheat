/**
 * v121 - 修复怪不动冻结所有AI + 自动Y轴对齐
 * v120问题:
 *   1. g_playerEntity在AIAgent.Update hook触发时还是0x0 — Damage hook还没执行!
 *      所有AI都被冻结了包括玩家(自动战斗不动)
 *      日志: AI: AIAgent.Update FROZEN entity=0x800000070000000a (player=0x0)
 *   2. 用户需求: 全屏秒杀+怪不动 → 玩家只需上下移动对齐怪Y坐标
 *      不用走到怪面前, 只需要对齐X或Y就能触发全屏伤害
 * v121核心改动:
 *   1. 怪不动判断: 不再依赖g_playerEntity — 改为在AIAgent.Update hook中
 *      通过AIParams._entity(EntityRef)获取CharacterFiled, 判断Camp
 *      Camp=0 → 玩家(不冻结), Camp≠0 → 敌方(跳过Update)
 *      方法: 用FrameBase.GetPointer(EntityRef, componentIndex)获取CF
 *      但不能主动调用IL2CPP函数! → 改用已知的playerEntityRef全局变量
 *      更简单: 直接记录玩家entityRef — 在CanbeAttack/IsReady等hook中
 *      isPlayerCF(cf)时记录entityRef → 在AIAgent.Update中比较
 *   2. 自动Y对齐: 当怪不动+全屏秒杀时, 自动把敌人Y坐标对齐到玩家Y
 *      玩家上下移动即可, 不需要左右走到怪面前
 *      在vacuumTick中同时修改enemy Transform2D.Position.Y → 等于player.Y
 */
...
static void hAIAgentUpdate(void *mi_or_self, void *self_or_params, void *maybe_params) {
    if(g_monsterFreeze) {
        ...
        // v121: 判断是否是玩家实体
        // 方案1: g_playerEntityRaw已知(Damage hook设置) → 精确比较
        // 方案2: g_playerEntityRaw未知 → 只冻结已知敌人的entity(Damage/trackEnemy记录)
        //   未被trackEnemy记录的entity → 可能是玩家 → 放行
        //   这确保在Damage触发前也不会误冻玩家!
        BOOL isPlayer = NO;
        
        if(g_playerEntityRaw != 0) {
            isPlayer = (entityRaw == g_playerEntityRaw);
        } else {
            // 安全判断: 只冻结已知是敌人的entity
            isPlayer = !isKnownEnemyEntity(entityRaw);
        }
        
        if(!isPlayer) {
            // 敌方AI → 跳过Update
            if(g_aiFreezeLC<10){g_aiFreezeLC++;jlog(@"AI: AIAgent.Update FROZEN entity=0x%llx (player=0x%llx knownEnemy=%d)",entityRaw,g_playerEntityRaw,isKnownEnemyEntity(entityRaw));}
            return;
        }
        // 玩家AI → 放行(保留自动战斗)
    }
