/**
 * v68.0 - 单技能替换选择(5个独立toggle) + 皮肤应用修复(不写帧同步内存)
 * v67测试:
 *   1. 技能替换生效! UseSkill player st=14->19, st=16->19 ✓
 *   2. 用户需求: 不要1-5全部替换, 改为选择单个技能替换成大招
 *      -> v68: 5个独立toggle按钮(Skill1~5→大招), 每个可单独开关
 *   3. 皮肤应用卡住: 写Actor backing field导致帧同步冲突
 *      -> v68: 移除写Actor backing field, 仅Hook get_SkinId返回目标ID
 *   4. Slider越界: 超过扫描数量后变0重新递增
 *      -> v68: 限制slider范围为0~count-1, 超界自动clamp
 */
...
// v68: 单技能替换开关 - Skill1~5各自独立, 替换为大招(Skill6)
...
static BOOL hIsReady(void *f,int st,void *cf,void *st2){
    ...
    // v68: 只对用户选择的技能+大招返回YES
    if(g_skillReplace&&st>=14&&st<=19){
        if(st==19) return YES; // 大招自身始终可用
        BOOL sel=NO;
        if(st==14) sel=g_replaceSkill1;
        if(st==15) sel=g_replaceSkill2;
        if(st==16) sel=g_replaceSkill3;
        if(st==17) sel=g_replaceSkill4;
        if(st==18) sel=g_replaceSkill5;
        if(sel){if(g_isReadyLC<30){g_isReadyLC++;jlog(@"IsReady: st=%d->YES",st);}return YES;}
    }
...
static BOOL hAttackCanUse(void *f,int st,void *cf,void *st2){
    ...
    if(g_skillReplace&&st>=14&&st<=19){
        ...
        if(st==17) sel=g_replaceSkill4;
        if(st==18) sel=g_replaceSkill5;
        if(sel){if(g_attackLC<30){g_attackLC++;jlog(@"AttackCanUse: st=%d->YES",st);}return YES;}
    }
    ...
    if(!g_classHotfixGameEntry){
        for(int i=0;i<g_gameEntryCount;i++){
            if(g_allGameEntryNS[i]==NULL||strcmp(g_allGameEntryNS[i],"")==0){
                ...
                jlog(@"v68: assume GameEntry[%d] ns=empty is HotfixGameEntry=%p",i,g_allGameEntries[i]);
                break;
            }
        }
    }
    jlog(@"v68: UnityGameEntry=%p HotfixGameEntry=%p",g_classUnityGameEntry,g_classHotfixGameEntry);
...
static UIButton *g_btnRepS1=nil,*g_btnRepS2=nil,*g_btnRepS3=nil,*g_btnRepS4=nil,*g_btnRepS5=nil; // v68: 单技能替换按钮
...
-(void)onReplaceS1;-(void)onReplaceS2;-(void)onReplaceS3;-(void)onReplaceS4;-(void)onReplaceS5; // v68
-(void)onApplySkin;
...
-(void)onApplySkin{
    // v68: 移除写Actor backing field(帧同步内存,写后卡死)
    // 仅Hook get_SkinId返回目标ID(只改本地显示,不影响帧同步)
...
static void setupUI(void){
    ...
    UILabel *tl=[[UILabel alloc]initWithFrame:CGRectMake(10,6,g_panelW-20,20)];tl.text=@"\xe5\x89\x91\xe5\xbd\xb1\xe6\xb1\x9f\xe6\xb9\x96 v68.0";tl.textColor=IMGUI_ACCENT;tl.font=[UIFont boldSystemFontOfSize:15];tl.textAlignment=NSTextAlignmentCenter;[tb addSubview:tl];
    ...
    CGFloat contentH=by0+bdy*6+60+188;
