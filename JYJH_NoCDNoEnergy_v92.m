/**
 * v92 - 皮肤/武器/光环: 只靠hook被动修改, 不主动调用任何IL2CPP函数
 * v91问题: applyPartChange()主动调用GetMainActorData(NULL)和ChangePart(NULL,...,NULL)导致卡死
 * v92方案:
 * 1. Hook LobbyActorData.UpdatePart(skinId,weaponId,haloId) — 替换参数 (被动)
 * 2. Hook ActorData.GetMainActorData() — 返回时修改ActorData字段 (被动)
 * 3. 点"应用"只设g_appliedXxxId, 不调用任何IL2CPP函数
 * 4. 下次游戏调用UpdatePart/GetMainActorData时hook自动替换生效
 */
...
// v92: 不再需要ChangePart和InitWithActorData
...
// v92: UpdatePart + GetMainActorData (不主动调用任何IL2CPP函数)
...
// v92: 主角色ActorData指针
static void *g_mainActorData=NULL;
...
// ===== v92: LobbyActorData.UpdatePart Hook =====
// 游戏自己换装时调用, 我们替换参数
...
static void* hGetMainActorData(void *mi) {
    ...
    if(result) {
        ...
        static int lc=0; if(lc<20){lc++;jlog(@"GetMainActorData: %p skin=%d weapon=%d halo=%d", result, skinId, weaponId, haloId);}
    }
    return result;
}
...
static void findIL2CPP(void) {
    ...
    for(size_t a=0;a<assemCount;a++){
        ...
        for(size_t c=0;c<cnt;c++){
            ...
            while((m=get_methods(klass,&iter))!=NULL){
                ...
                // v92: 搜索LobbyActorData.UpdatePart(skinId,weaponId,haloId) 3参数实例方法
                else if(strcmp(n,"UpdatePart")==0&&pc==3&&!g_fUpdatePart){g_fUpdatePart=fa;found++;jlog(@"FOUND %s.%s p=%u %p",cn?:"?",n,pc,fa);}
                // v92: 搜索ActorData.GetMainActorData() 静态方法 0参数
    ...
    jlog(@"v92: UpdatePart=%p GetMainActorData=%p",g_fUpdatePart,g_fGetMainActorData);
...
// v92: 应用外观 - 只设变量, 不调用IL2CPP函数
// 游戏下次调用UpdatePart/GetMainActorData时hook自动替换生效
static void applyPartChange(void) {
    jlog(@"applyPart: set skin=%d weapon=%d halo=%d (will apply on next game call)", g_appliedSkinId, g_appliedWeaponId, g_appliedHaloId);
    // 如果已经有MainActorData指针, 直接修改字段
    if(g_mainActorData) {
        if(g_appliedSkinId > 0) memcpy((uint8_t*)g_mainActorData + 0x14, &g_appliedSkinId, 4);
        if(g_appliedWeaponId > 0) memcpy((uint8_t*)g_mainActorData + 0x18, &g_appliedWeaponId, 4);
        if(g_appliedHaloId > 0) memcpy((uint8_t*)g_mainActorData + 0x1c, &g_appliedHaloId, 4);
        jlog(@"applyPart: wrote fields to MainActorData=%p", g_mainActorData);
    }
}
...
static void setupUI(void){
    ...
    UILabel *tl=[[UILabel alloc]initWithFrame:CGRectMake(10,6,g_panelW-20,20)];tl.text=@"剑影江湖 v92";tl.textColor=IMGUI_ACCENT;tl.font=[UIFont boldSystemFontOfSize:15];tl.textAlignment=NSTextAlignmentCenter;[tb addSubview:tl];
...
static void initialize(void){
    ...
    jlog(@"========== JYJH v92 (passive hook only) ==========");
