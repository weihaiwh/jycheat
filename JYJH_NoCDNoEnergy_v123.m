/**
 * v123 - 用EntityType代替Camp判断玩家 + 修复多个偏移bug
 * v122问题分析:
 *   1. isPlayerCF用Camp==0判断, 但玩家Camp值可能不是0!
 *      dump: Camp @ 0x34 (actual 0x24), 但EntityType @ 0x48 (actual 0x38)
 *      MapStaticType.Player = 100 — 用EntityType==100判断玩家更可靠!
 *   2. isDead是QBoolean(4字节int), 不是1字节bool
 *      ChangeCharacterToDead中写isDead=0应该写4字节, 不是1字节
 *   3. g_enemyEntities全是0x0 — trackEnemy(cf,NULL)从CanBeAttack调用时没entity
 *      只从Damage hook调用trackEnemy(cf,ent)才有entity
 *   4. AIAgent.Update hook参数不确定 — 需要增加调试日志确认
 * v123核心改动:
 *   1. isPlayerCF改用EntityType==100(MapStaticType.Player)判断
 *      EntityType @ dump 0x48 → actual 0x38 (4字节int)
 *   2. isDeadCF改用isDead(QBoolean) @ dump 0x58 → actual 0x48 (4字节int, !=0表示dead)
 *   3. ChangeCharacterToDead: isDead写4字节0, Invincible写1字节1
 *   4. 增加运行时dump CharacterFiled字段偏移 — 验证所有offset
 *   5. AIAgent.Update hook增加EntityType调试日志
 *   6. trackEnemy只从Damage hook调用(有entity), CanBeAttack不再调用trackEnemy
 */
...
static BOOL hMove(void *mi,void *f,void *tf,void *cf,void *targetPos,void *moveSpeed,void *fDeltaTime,BOOL updateFootY) {
    // v123: 增加EntityType调试 — 打印EntityType值来判断isPlayerCF为什么失败
    if(g_moveDebugLC < 30) {
        g_moveDebugLC++;
        int32_t entityType=-1; if(cf) memcpy(&entityType,(uint8_t*)cf+0x38,4);
        int32_t camp=-1; if(cf) memcpy(&camp,(uint8_t*)cf+0x24,4);
        int32_t isDead48=0; if(cf) memcpy(&isDead48,(uint8_t*)cf+0x48,4);
        int32_t isAI44=0; if(cf) memcpy(&isAI44,(uint8_t*)cf+0x44,4);
        jlog(@"Move[%d]: mi=%p f=%p tf=%p cf=%p etype=%d camp=%d isAI=%d dead=%d validTF=%d",
             g_moveDebugLC, mi, f, tf, cf, entityType, camp, isAI44, isDead48, isValidPtr(tf));
    }
    
    // v123: 用EntityType==100判断玩家(MapStaticType.Player=100)
    if(cf && tf && isValidPtr(tf)) {
        ...
        if(entityType == 100) {
            ...
            if(!g_playerTransform2D || g_playerTransform2D != tf) {
                ...
                if(g_moveHookLC<20){g_moveHookLC++;jlog(@"Move: PLAYER tf=%p etype=%d camp=%d",tf,entityType,getCamp(cf));}
            }
        } else {
            // 敌人: 更新Transform2D指针
            for(int i=0;i<g_enemyCount;i++) {
...
static void findIL2CPP(void) {
    ...
    if(class_from_name && get_fields && field_name && field_offset) {
        ...
        for(size_t a=0;a<assemCount;a++){
            ...
            if(cfklass) {
                jlog(@"=== v123: Dump CharacterFiled fields (runtime offsets) ===");
                void *fiter=NULL, *field=NULL;
    ...
    jlog(@"v123: Move=%p AIAgentUpdate=%p", g_fMove, g_fAIAgentUpdate);
...
static void setupUI(void){
    ...
    UILabel *tl=[[UILabel alloc]initWithFrame:CGRectMake(10,6,g_panelW-20,20)];tl.text=@"剑影江湖 v123";tl.textColor=IMGUI_ACCENT;tl.font=[UIFont boldSystemFontOfSize:15];tl.textAlignment=NSTextAlignmentCenter;[tb addSubview:tl];
...
static void initialize(void){
    ...
    jlog(@"========== JYJH v123 (EntityType判断+偏移修复) ==========");
