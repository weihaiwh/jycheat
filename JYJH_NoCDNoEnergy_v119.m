/**
 * v119 - 修复怪不动卡死 + 怪不动也不攻击
 * v118问题:
 *   1. hook 6个AI移动节点OnUpdate返回Failure → AI行为树死循环卡死!
 *      AISelector: 子节点Failure→尝试下一个→AILoop(LoopForever)→全部Failure→重新开始→无限循环
 *      特别是怪快死时ChangeCharacterToDead被hook阻止→怪不死→AI持续运行但全部Failure→死锁卡住
 *   2. 怪仍会攻击放技能 — 因为AttackEnemy/AISelectSkill没被冻结
 * v119核心改动:
 *   1. 不再hook单个AI节点(会死循环) — 改为hook AISystem.Update直接跳过整个AI系统更新
 *      AISystem.Update(Frame f) @ 0x30c7d1c — 遍历所有AI Agent执行Update
 *      hook后完全跳过AI更新 → 怪不动+不攻击+不释放技能
 *      但Damage/ChangeCharacterToDead仍正常 → 怪仍可被杀死, 不会卡住
 *   2. 保留攻击范围+功能不变
 *   3. 删除v118的6个AI移动节点hook(会导致死循环)
 * 关键dump偏移:
 *   AISystem.Update @ 0x30c7d1c
 *   AIAgent.Update @ 0x30682cc
 *   AIStatus: Inactive=0, Success=1, Failure=2, Running=3
...
// v119: 怪不动 — 改为hook AISystem.Update直接跳过整个AI系统
// v118方案(已废弃): hook 6个AI移动节点OnUpdate → AI行为树死循环卡死!
//   原因: AISelector子节点Failure→尝试下一个→AILoop重新开始→无限循环
// v119方案: hook AISystem.Update完全跳过AI更新 → 怪不动+不攻击+不释放技能
//   但Damage/ChangeCharacterToDead仍正常 → 怪仍可被杀, 不会卡住
// AISystem.Update(Frame f) — 1参数实例方法, MSHookFunction可能传mi
typedef void (*AISystemUpdateFunc)(void*, void*); // (self, frame)
static void *g_fAISystemUpdate=NULL; static AISystemUpdateFunc g_oAISystemUpdate=NULL; static BOOL g_hAISystemUpdate=NO;
...
// ===== v119: AISystem.Update Hook — 怪不动+不攻击 =====
// v118方案(已废弃): hook单个AI移动节点返回Failure → 行为树死循环卡死
// v119方案: hook AISystem.Update直接跳过整个AI系统更新
//   → 怪不移动+不攻击+不释放技能
//   → 但Damage/ChangeCharacterToDead仍正常工作, 怪仍可被杀死, 不会卡住
// AISystem.Update(Frame f) — 实例方法, 1参数
// MSHookFunction可能传mi → hook 3参数(mi,self,frame) 或 2参数(self,frame)
static int g_aiFreezeLC=0;
static void hAISystemUpdate(void *mi_or_self, void *self_or_frame, void *maybe_frame) {
    if(g_monsterFreeze) {
        if(g_aiFreezeLC<5){g_aiFreezeLC++;jlog(@"AI: AISystem.Update FROZEN (mi=%p self=%p frame=%p)",mi_or_self,self_or_frame,maybe_frame);}
        return; // 完全跳过AI更新 — 怪不动+不攻击
    }
    // 调用原函数: 尝试2参数(不含mi)
    if(g_oAISystemUpdate) g_oAISystemUpdate(mi_or_self, self_or_frame);
}
...
static void findIL2CPP(void) {
    ...
    for(size_t a=0;a<assemCount;a++){
        ...
        for(size_t c=0;c<cnt;c++){
            ...
            while((m=get_methods(klass,&iter))!=NULL){
                ...
                // v119: 搜索AISystem.Update — 怪不动功能
                // v118方案(废弃): hook 6个AI移动节点 → 行为树死循环卡死
                // v119方案: hook AISystem.Update直接跳过整个AI系统
                else if(strcmp(n,"Update")==0&&pc==1&&cn&&strcmp(cn,"AISystem")==0&&!g_fAISystemUpdate){g_fAISystemUpdate=fa;found++;jlog(@"FOUND %s.%s p=%u %p (v119 freeze)",cn,n,pc,fa);}
                // v117: 记录AdsHelper类指针
    ...
    jlog(@"v119: Move=%p AISystemUpdate=%p", g_fMove, g_fAISystemUpdate);
...
// v119: 怪不动 — hook AISystem.Update跳过整个AI系统
-(void)onMonsterFreeze {
    g_monsterFreeze=!g_monsterFreeze;
    if(g_monsterFreeze) {
        findIL2CPP();
        // v119方案: 直接hook AISystem.Update — 完全跳过AI系统更新
        // 怪不移动+不攻击+不释放技能, 但Damage/ChangeCharacterToDead正常
        if(g_fAISystemUpdate && !g_hAISystemUpdate) {
            hookOneFunc(g_fAISystemUpdate, hAISystemUpdate, (void**)&g_oAISystemUpdate, &g_hAISystemUpdate, "AISystem.Update");
        }
        jlog(@"monsterFreeze: ON - AISystem.Update hooked=%d (addr=%p)", g_hAISystemUpdate, g_fAISystemUpdate);
    } else {
