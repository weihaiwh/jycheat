/**
 * v103 - SSO bypass改为直接内存修改 + DoHandShakeCheck hook
 * v102问题:
 *   get_code/get_banTime的MSHookFunction hook无效 — 
 *   IL2CPP泛型属性共享: 0x2ae94bc被上百个Int32 getter共享
 *   hook它等于hook所有Int32 getter, 返回值是无关字段的值(1100429216等)
 *   get_banTime同理: 0x2b32118被所有Int64 getter共享
 * v103方案:
 *   1. 移除get_code/get_banTime的MSHookFunction hook
 *   2. 新增直接内存修改: LoginModel单例→CurSSOResponData→写code=200+banTime=0
 *      LoginModel是Singleton, CurSSOResponData在offset 0xf0
 *      SSOResponData.code在offset 0x10, banTime在offset 0x38
 *   3. 新增hook DoHandShakeCheck(int32 code) — 地址0x2b7eb2c是唯一的
 *      这个方法接收code参数, 很可能就是检查code==502的地方
 *      hook它让code强制为200, 绕过检测
 *   4. 保留所有其他已验证可用的功能(技能/伤害/无敌等)
 */
...
// v103: 保留g_fGetCode/g_fGetBanTime用于搜索, 但不再MSHookFunction它们
...
// v103: 直接内存修改SSOResponData (备用方案, 声明在前, 定义在isValidPtr/getObjClassName之后)
...
// v103: 直接内存修改SSOResponData (实现)
// LoginModel是Singleton, 需要找到实例 → 读CurSSOResponData(offset 0xf0)
// → 写code=200(offset 0x10) + banTime=0(offset 0x38)
static void patchSSOResponDataMemory(void) {
    if(!g_classLoginModel) {
        jlog(@"SSO: LoginModel class not found, cannot patch memory");
        return;
    }
    void *h=dlopen(NULL,RTLD_LAZY); if(!h) return;
    typedef void* (*GetStaticDataFn)(void*);
    GetStaticDataFn getStaticData=(GetStaticDataFn)dlsym(h,"il2cpp_class_get_static_field_data");
    if(!getStaticData) {jlog(@"SSO: il2cpp_class_get_static_field_data not found");return;}
    
    void *staticData=getStaticData(g_classLoginModel);
    if(!isValidPtr(staticData)) {jlog(@"SSO: LoginModel staticData=%p invalid",staticData);return;}
    
    // 遍历字段找Singleton._instance
    typedef void* (*GetFieldsFn)(void*,void**);
    GetFieldsFn get_fields=(GetFieldsFn)dlsym(h,"il2cpp_class_get_fields");
    typedef const char* (*GetFieldNameFn)(void*);
    GetFieldNameFn field_name=(GetFieldNameFn)dlsym(h,"il2cpp_field_get_name");
    typedef int32_t (*GetFieldOffsetFn)(void*);
    GetFieldOffsetFn field_offset=(GetFieldOffsetFn)dlsym(h,"il2cpp_field_get_offset");
    
    void *loginModelInstance=NULL;
    int32_t instanceOffset=-1;
    
    if(get_fields && field_name && field_offset) {
        void *iter=NULL, *field=NULL;
        while((field=get_fields(g_classLoginModel,&iter))!=NULL) {
            const char *fn=field_name(field);
            int32_t fo=field_offset(field);
            if(fn) jlog(@"  LoginModel field: %s offset=%d", fn, fo);
            if(fn && (strcmp(fn,"_instance")==0 || strcmp(fn,"s_instance")==0 || strcmp(fn,"instance")==0)) {
                instanceOffset=fo;
                jlog(@"SSO: Found LoginModel instance field '%s' at offset %d", fn, fo);
            }
        }
    }
    
    // 如果找不到instance字段名, 尝试扫描静态数据区
    if(instanceOffset<0) {
        jlog(@"SSO: No named instance field, scanning static data...");
        for(int off=0;off<=0x18;off+=8) {
            void *testPtr=NULL;
            memcpy(&testPtr,(uint8_t*)staticData+off,8);
            if(isValidPtr(testPtr)) {
                const char *cn=getObjClassName(testPtr);
                if(cn && strcmp(cn,"LoginModel")==0) {
                    loginModelInstance=testPtr;
                    instanceOffset=off;
                    jlog(@"SSO: Found LoginModel instance at staticData+%d: %p", off, testPtr);
                    break;
                }
            }
        }
    } else {
        memcpy(&loginModelInstance,(uint8_t*)staticData+instanceOffset,8);
    }
    
    if(!isValidPtr(loginModelInstance)) {
        jlog(@"SSO: LoginModel instance=%p not found", loginModelInstance);
        return;
    }
    ...
    if(!isValidPtr(ssoRspData)) {
        jlog(@"SSO: CurSSOResponData=%p not set yet (SSO request not completed)", ssoRspData);
        return;
    }
...
static void findIL2CPP(void) {
    ...
    for(size_t a=0;a<assemCount;a++){
        ...
        for(size_t c=0;c<cnt;c++){
            ...
            // v103: 搜索LoginModel类 (Singleton, 保存CurSSOResponData)
            ...
            // v103: 搜索LoginCtrl类 (有DoHandShakeCheck方法)
            ...
            while((m=get_methods(klass,&iter))!=NULL){
                ...
                // v103: LoginCtrl.DoHandShakeCheck(int32 code) — 唯一地址0x2b7eb2c, 不被泛型共享
                ...
                // v102→v103: 保留get_code/get_banTime搜索仅用于日志确认, 不再MSHookFunction它们
    ...
    // v103: 用il2cpp_class_from_name搜索热更新DLL中的LoginModel/LoginCtrl/DoHandShakeCheck
            ...
            for(size_t a=0;a<assemCount;a++){
                ...
                if(!g_classLoginModel) {
                    ...
                    if(lmClass) {
                        ...
                        jlog(@"v103: Found LoginModel class in assembly %zu: %p", a, lmClass);
                    }
                }
                ...
                if(!g_fDoHandShakeCheck) {
                    ...
                    if(lcClass) {
                        ...
                        jlog(@"v103: Found LoginCtrl class in assembly %zu: %p", a, lcClass);
                ...
                if(!g_fGetCode) {
                    ...
                    if(ssoRspClass) {
                        jlog(@"v103: Found SSOResponData class: %p", ssoRspClass);
    ...
    jlog(@"v103: DoHandShakeCheck=%p LoginModel=%p LoginCtrl=%p",
         g_fDoHandShakeCheck, g_classLoginModel, g_classLoginCtrl);
    if(!g_fDoHandShakeCheck) jlog(@"v103: DoHandShakeCheck NOT FOUND!");
    if(!g_classLoginModel) jlog(@"v103: LoginModel class NOT FOUND!");
}
...
-(void)onBypassSSO{
    ...
    if(g_bypassSSO){findIL2CPP();
        // v103: 优先hook DoHandShakeCheck (唯一地址, 不被泛型共享)
...
static void setupUI(void){
    ...
    UILabel *tl=[[UILabel alloc]initWithFrame:CGRectMake(10,6,g_panelW-20,20)];tl.text=@"\xe5\x89\x91\xe5\xbd\xb1\xe6\xb1\x9f\xe6\xb9\x96 v102";tl.textColor=IMGUI_ACCENT;tl.font=[UIFont boldSystemFontOfSize:15];tl.textAlignment=NSTextAlignmentCenter;[tb addSubview:tl];
...
static void initialize(void){
    ...
    jlog(@"========== JYJH v103 (SSO bypass: DoHandShakeCheck+memory patch) ==========");
