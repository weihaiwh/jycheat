/**
 * v49.0 - 移除isDead检查 + UI优化(按钮缩小+面板加宽+移除皮肤按钮)
 * v48问题:
 *   1. 敌人血条清空但不死 - isDeadCF判定为true后跳过,但视觉上敌人还站着
 *   2. ScrollView仍然无法滚动 - 内容高度问题
 *   3. 按钮太大,面板太窄
 *   4. 皮肤ID按钮无用(Tables无static实例)
 * v49方案:
 *   1. 移除isDead检查,对所有tracked敌人强制扩散
 *   2. 增加伤害倍率到3倍,持续扩散确保死亡
 *   3. 按钮高度28->24,间距34->28,面板宽度320->360
 *   4. 移除"查询皮肤ID列表"按钮,调整contentHeight
 */
...
static void setupUI(void){
    ...
    UILabel *tl=[[UILabel alloc]initWithFrame:CGRectMake(10,6,g_panelW-20,20)];tl.text=@"\xe5\x89\x91\xe5\xbd\xb1\xe6\xb1\x9f\xe6\xb9\x96 v49.0";tl.textColor=IMGUI_ACCENT;tl.font=[UIFont boldSystemFontOfSize:15];tl.textAlignment=NSTextAlignmentCenter;[tb addSubview:tl];
    ...
    CGFloat contentH=by0+bdy*4+60+120; // 4按钮+伤害滑块+皮肤区域
