/**
 * v118 - AI自动战斗修改: 怪不动+攻击距离增大
 * v117已修复: Move精确匹配CharacterFiled.Move, ShowVideo hook免广告
 * v118核心改动(基于dump深度分析):
 *   1. 新增"怪不动"功能 — hook AIMoveByPath.OnUpdate/AIMoveInCD.OnUpdate/AttackEnemy.OnUpdate/AIPatrol.OnUpdate等
 *      AI行为树流程: AICheckEnemy→SerachEnemy(GetNearestEnemy)→AINeedMove→AIMoveByPath/AIMoveInCD→AttackEnemy
 *      阻止敌方AI的移动节点返回Running, 敌人不移动但AI树继续运行(仍会攻击)
 *   2. 新增"攻击范围+"功能 — 扩大FPBounds2的Extents, 使技能判定范围大幅增加
 *      FPBounds2结构: Center(FPVector2)@0x10, Extents(FPVector2)@0x20
 *      在HitSystem.Update中扩大collBound.Extents → 敌人不用靠近就能被命中
 *   3. 修复: Move搜索会匹配两个pc==7重载, 用运行时参数检测区分(有Transform2D*的是第一个)
 *   4. 新增CharacterFiled.Move第二个重载(无Transform2D*)的搜索: MoveToPos
 *   5. 搜索SerachEnemy.OnUpdate @ 0x306fc88 — 距离判断核心, 可修改MaxDistance
 * 关键dump偏移:
 *   SerachEnemy.OnUpdate @ 0x306fc88, GetNearestEnemy @ 0x30645b4
 *   AIMoveByPath.OnUpdate @ 0x306e0a4
 *   AIMoveInCD.OnUpdate @ 0x306e628
 *   AIPatrol.OnUpdate @ 0x306ef28
 *   AttackEnemy.OnUpdate @ 0x306f4e8
 *   AICheckEnemy.OnUpdate @ 0x306cee4
 *   AINeedMove.OnUpdate @ 0x306c3a0
 *   AIReturnMove.OnUpdate @ 0x306b4ac
 *   HandleSkillRange @ 0x307ed7c
 *   FPBounds2.Intersects @ 0x2f51130
 *   CharacterFiled.Move(Frame,TF*,CF*,FPVector2,FP,FP,Boolean) @ 0x30754b8
 *   CharacterFiled.Move(Frame,ER,CF*,FPVector2,FP,FP,CSA) @ 0x3075690
 *   CharacterFiled.MoveToPos @ 0x3072914
 */
...
// v118: AI移动相关hook — 怪不动功能
// AI行为树节点都是IL2CPP序列化类, OnUpdate是虚方法
// AIMoveByPath.OnUpdate(AIParams) — 1参数, 非静态 → hook 2参数(含mi)或1参数
// AIMoveInCD.OnUpdate(AIParams) — 同上
// AIPatrol.OnUpdate(AIParams) — 同上
// AttackEnemy.OnUpdate(AIParams) — 同上
// AIReturnMove.OnUpdate(AIParams) — 同上
// 所有AILeaf/AIDecorator的OnUpdate签名: protected override AIStatus OnUpdate(AIParams Params)
// AIParams是struct(值类型), C#中ref传递
// dump: OnUpdate都只有1个参数(AIParams)
// AIStatus enum: Inactive=0, Success=1, Failure=2, Running=3
typedef int32_t (*AIOnUpdateFunc)(void*, void*); // (self/this, AIParams*)
static void *g_fAIMoveByPath=NULL; static AIOnUpdateFunc g_oAIMoveByPath=NULL; static BOOL g_hAIMoveByPath=NO;
static void *g_fAIMoveInCD=NULL; static AIOnUpdateFunc g_oAIMoveInCD=NULL; static BOOL g_hAIMoveInCD=NO;
static void *g_fAIPatrol=NULL; static AIOnUpdateFunc g_oAIPatrol=NULL; static BOOL g_hAIPatrol=NO;
static void *g_fAIReturnMove=NULL; static AIOnUpdateFunc g_oAIReturnMove=NULL; static BOOL g_hAIReturnMove=NO;
// AttackEnemy不冻结 — 让敌人仍能攻击(只是不移动)
static void *g_fAIForceMoveAndSkill=NULL; static AIOnUpdateFunc g_oAIForceMoveAndSkill=NULL; static BOOL g_hAIForceMoveAndSkill=NO;
static void *g_fMoveToPoint=NULL; static AIOnUpdateFunc g_oMoveToPoint=NULL; static BOOL g_hMoveToPoint=NO;

// v118: 搜索到的AI方法的param count, 用于判断签名
static uint32_t g_aiMoveByPathPC=0, g_aiMoveInCDPC=0, g_aiPatrolPC=0, g_aiReturnMovePC=0;
static uint32_t g_aiForceMoveSkillPC=0, g_aiMoveToPointPC=0;
...
// ===== v118: AI移动冻结Hook =====
// AI行为树节点OnUpdate返回AIStatus: Inactive=0, Success=1, Failure=2, Running=3
// 冻结策略: 让移动类AI节点返回Failure(2)而不是Running(3), AI树会跳过移动继续执行
// 这样敌人不会移动, 但AI树仍会尝试攻击(AttackEnemy节点不受影响)
// 
// 关键: 这些OnUpdate是C#实例方法, MSHookFunction可能传mi也可能不传
// 签名: AIStatus OnUpdate(AIParams Params) — 1参数
// hook可能2参数(mi,self,params)或1参数(self,params)
// 用运行时检测: 如果mi看起来像有效指针且self看起来不像IL2CPP对象, 则mi存在
// 
// 更安全的方案: 直接用固定偏移hook, 不搜索方法名
// AIMoveByPath.OnUpdate @ 0x306e0a4
// AIMoveInCD.OnUpdate @ 0x306e628
// AIPatrol.OnUpdate @ 0x306ef28
// AIReturnMove.OnUpdate @ 0x306b4ac
// AIForceMoveAndSkill.OnUpdate @ 0x306d9f4
// MoveToPoint.OnUpdate @ 0x306f81c

static int g_aiFreezeLC=0;

// 通用AI移动冻结hook — 2参数版本(含mi)
static int32_t hAIMoveByPath(void *mi_or_self, void *self_or_params, void *maybe_params) {
    if(g_monsterFreeze) {
        if(g_aiFreezeLC<10){g_aiFreezeLC++;jlog(@"AI: AIMoveByPath FROZEN (mi=%p self=%p params=%p)",mi_or_self,self_or_params,maybe_params);}
        return 2; // Failure — AI树跳过此节点
    }
    // 尝试调用原函数: 先试2参数(不含mi), 再试3参数(含mi)
    if(g_oAIMoveByPath) return g_oAIMoveByPath(mi_or_self, self_or_params);
    return 0;
}

static int32_t hAIMoveInCD(void *mi_or_self, void *self_or_params, void *maybe_params) {
    if(g_monsterFreeze) {
        if(g_aiFreezeLC<10){g_aiFreezeLC++;}
        return 2;
    }
    if(g_oAIMoveInCD) return g_oAIMoveInCD(mi_or_self, self_or_params);
    return 0;
}

static int32_t hAIPatrol(void *mi_or_self, void *self_or_params, void *maybe_params) {
    if(g_monsterFreeze) {
        if(g_aiFreezeLC<10){g_aiFreezeLC++;}
        return 2;
    }
    if(g_oAIPatrol) return g_oAIPatrol(mi_or_self, self_or_params);
    return 0;
}

static int32_t hAIReturnMove(void *mi_or_self, void *self_or_params, void *maybe_params) {
    if(g_monsterFreeze) {
        if(g_aiFreezeLC<10){g_aiFreezeLC++;}
        return 2;
    }
    if(g_oAIReturnMove) return g_oAIReturnMove(mi_or_self, self_or_params);
    return 0;
}

static int32_t hAIForceMoveAndSkill(void *mi_or_self, void *self_or_params, void *maybe_params) {
    if(g_monsterFreeze) {
        if(g_aiFreezeLC<10){g_aiFreezeLC++;}
        return 2;
    }
    if(g_oAIForceMoveAndSkill) return g_oAIForceMoveAndSkill(mi_or_self, self_or_params);
    return 0;
}

static int32_t hMoveToPoint(void *mi_or_self, void *self_or_params, void *maybe_params) {
    if(g_monsterFreeze) {
        if(g_aiFreezeLC<10){g_aiFreezeLC++;}
        return 2;
    }
    if(g_oMoveToPoint) return g_oMoveToPoint(mi_or_self, self_or_params);
    return 0;
}
...
static void findIL2CPP(void) {
    ...
    for(size_t a=0;a<assemCount;a++){
        ...
        for(size_t c=0;c<cnt;c++){
            ...
            while((m=get_methods(klass,&iter))!=NULL){
                ...
                // v118: 搜索AI移动节点OnUpdate — 怪不动功能
                // 这些都是AILeaf/AIDecorator子类的OnUpdate方法, 1参数(AIParams)
                // 但MSHookFunction可能传mi → 实际hook时用运行时检测
                // 用类名精确匹配, 避免匹配到其他类的OnUpdate
                else if(strcmp(n,"OnUpdate")==0&&cn) {
                    if(strcmp(cn,"AIMoveByPath")==0&&!g_fAIMoveByPath){g_fAIMoveByPath=fa;g_aiMoveByPathPC=pc;found++;jlog(@"FOUND %s.%s p=%u %p (v118 freeze)",cn,n,pc,fa);}
                    else if(strcmp(cn,"AIMoveInCD")==0&&!g_fAIMoveInCD){g_fAIMoveInCD=fa;g_aiMoveInCDPC=pc;found++;jlog(@"FOUND %s.%s p=%u %p (v118 freeze)",cn,n,pc,fa);}
                    else if(strcmp(cn,"AIPatrol")==0&&!g_fAIPatrol){g_fAIPatrol=fa;g_aiPatrolPC=pc;found++;jlog(@"FOUND %s.%s p=%u %p (v118 freeze)",cn,n,pc,fa);}
                    else if(strcmp(cn,"AIReturnMove")==0&&!g_fAIReturnMove){g_fAIReturnMove=fa;g_aiReturnMovePC=pc;found++;jlog(@"FOUND %s.%s p=%u %p (v118 freeze)",cn,n,pc,fa);}
                    else if(strcmp(cn,"AIForceMoveAndSkill")==0&&!g_fAIForceMoveAndSkill){g_fAIForceMoveAndSkill=fa;g_aiForceMoveSkillPC=pc;found++;jlog(@"FOUND %s.%s p=%u %p (v118 freeze)",cn,n,pc,fa);}
                    else if(strcmp(cn,"MoveToPoint")==0&&!g_fMoveToPoint){g_fMoveToPoint=fa;g_aiMoveToPointPC=pc;found++;jlog(@"FOUND %s.%s p=%u %p (v118 freeze)",cn,n,pc,fa);}
                }
                // v117: 记录AdsHelper类指针
    ...
    jlog(@"v118: Move=%p AIMoveByPath=%p AIMoveInCD=%p AIPatrol=%p AIReturnMove=%p ForceMoveSkill=%p MoveToPoint=%p",
         g_fMove, g_fAIMoveByPath, g_fAIMoveInCD, g_fAIPatrol, g_fAIReturnMove, g_fAIForceMoveAndSkill, g_fMoveToPoint);
...
// v118: 怪不动 — hook AI移动节点
-(void)onMonsterFreeze {
    g_monsterFreeze=!g_monsterFreeze;
    if(g_monsterFreeze) {
        findIL2CPP();
        // Hook所有AI移动节点 — 让它们返回Failure(2)而不是Running(3)
        // 这样AI树跳过移动, 但仍会执行攻击节点
        if(g_fAIMoveByPath && !g_hAIMoveByPath) hookOneFunc(g_fAIMoveByPath, hAIMoveByPath, (void**)&g_oAIMoveByPath, &g_hAIMoveByPath, "AIMoveByPath");
        if(g_fAIMoveInCD && !g_hAIMoveInCD) hookOneFunc(g_fAIMoveInCD, hAIMoveInCD, (void**)&g_oAIMoveInCD, &g_hAIMoveInCD, "AIMoveInCD");
        if(g_fAIPatrol && !g_hAIPatrol) hookOneFunc(g_fAIPatrol, hAIPatrol, (void**)&g_oAIPatrol, &g_hAIPatrol, "AIPatrol");
        if(g_fAIReturnMove && !g_hAIReturnMove) hookOneFunc(g_fAIReturnMove, hAIReturnMove, (void**)&g_oAIReturnMove, &g_hAIReturnMove, "AIReturnMove");
        if(g_fAIForceMoveAndSkill && !g_hAIForceMoveAndSkill) hookOneFunc(g_fAIForceMoveAndSkill, hAIForceMoveAndSkill, (void**)&g_oAIForceMoveAndSkill, &g_hAIForceMoveAndSkill, "AIForceMoveAndSkill");
        if(g_fMoveToPoint && !g_hMoveToPoint) hookOneFunc(g_fMoveToPoint, hMoveToPoint, (void**)&g_oMoveToPoint, &g_hMoveToPoint, "MoveToPoint");
        
        jlog(@"monsterFreeze: ON - AI move nodes hooked: ByPath=%d InCD=%d Patrol=%d Return=%d Force=%d ToPoint=%d",
             g_hAIMoveByPath, g_hAIMoveInCD, g_hAIPatrol, g_hAIReturnMove, g_hAIForceMoveAndSkill, g_hMoveToPoint);
    } else {
        jlog(@"monsterFreeze: OFF (hooks remain active but return original values)");
    }
    refreshBtns();
}
