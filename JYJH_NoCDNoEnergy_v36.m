/**
 * 剑影江湖 v36.0 - 修复退图卡住 + 全屏秒杀
 *
 * v35问题:
 *   1. HP=99999999写入帧同步内存 → 战斗结算异常 → 退图卡住
 *   2. 无全屏秒杀功能
 *
 * v36修复:
 *   1. GodMode只设Invincible=1, 不改HP → 不破坏帧同步结算
 *   2. 新增全屏秒杀: 修改normalBoundExtents碰撞箱范围为极大值
 *      normalBoundCenter: dump 0xd8 → 实际 CF+0xc8 (FPVector2)
 *      normalBoundExtents: dump 0xe8 → 实际 CF+0xd8 (FPVector2)
 *      FPVector2 = 2个FP, 每个FP=0x18字节(0x10 header + 0x08 RawValue)
 *      extents.x.RawValue: CF+0xd8+0x10 = CF+0xe8
 *      extents.y.RawValue: CF+0xd8+0x18+0x10 = CF+0x100
 *      center.x.RawValue: CF+0xc8+0x10 = CF+0xd8
 *      center.y.RawValue: CF+0xc8+0x18+0x10 = CF+0xf0
 *   3. limitDamage默认30000(配合全屏秒杀)
 *   4. GCD定时器同时设GodMode+FullScreen(防帧同步回滚)
 */
...
static int g_damageLimit = 30000;  // v36: 默认30000配合全屏秒杀
...
// isPlayerCF: 只检查CF+0x44==0 (IsAI.Value的真实偏移)
// dump偏移0x54 - 0x10虚拟header = 实际0x44
// QBoolean.Value: dump 0x10 - 0x10 = 实际0x00, 所以Value在IsAI+0x00=CF+0x44
...
// v36: 只设Invincible=1, 不改HP(修复退图卡住)
// Invincible dump=0x10 → 实际CF+0x00
static void makePlayerGodMode(void *cf) {
    if (!cf) return;
    uint8_t inv = 1;
    memcpy((uint8_t*)cf + 0x00, &inv, 1);  // Invincible=1
}

// v36: 全屏秒杀 - 修改碰撞箱范围
// FP结构: 0x10 header + 0x08 RawValue(Int64), RAW_ONE=65536
// normalBoundExtents dump=0xe8 → 实际CF+0xd8
//   extents.x.RawValue: CF+0xd8+0x10 = CF+0xe8
//   extents.y.RawValue: CF+0xd8+0x18+0x10 = CF+0x100
// normalBoundCenter dump=0xd8 → 实际CF+0xc8
//   center.x.RawValue: CF+0xc8+0x10 = CF+0xd8
//   center.y.RawValue: CF+0xc8+0x18+0x10 = CF+0xf0
static void makeFullScreen(void *cf) {
    if (!cf) return;
    // RawValue=1000000000 → 约15258个单位, 远超屏幕范围
    int64_t hugeBound = 1000000000LL;
    // 碰撞箱中心归零(以自身为中心)
    int64_t zero = 0;
    memcpy((uint8_t*)cf + 0xd8, &zero, 8);    // center.x.RawValue = 0
    memcpy((uint8_t*)cf + 0xf0, &zero, 8);    // center.y.RawValue = 0
    // 碰撞箱范围极大
    memcpy((uint8_t*)cf + 0xe8, &hugeBound, 8);   // extents.x.RawValue
    memcpy((uint8_t*)cf + 0x100, &hugeBound, 8);   // extents.y.RawValue
}
...
static void dumpCF(void *cf, int stateType, const char *source) {
    ...
    jlog(@"CFDump[%d] %s st=%d cf=%p camp24=%d isAI44=%d %s",
         g_isPlayerLogCount, source, stateType, cf, camp24, isAI44,
         isAI44==0 ? "★玩家!" : "(怪物)");
    jlog(@"  %s", hex);
}

static int g_godModeLogCount = 0;
static int g_fullScreenLogCount = 0;
...
static BOOL hookCheckSkillIsReady(void *frame, int stateType, void *characterField, void *states) {
    if (g_godMode && characterField) {
        ...
        if (isPlayerCF(characterField)) {
            ...
            if (g_godModeLogCount < 10) { g_godModeLogCount++; jlog(@"GodMode[%d] Invincible=1", g_godModeLogCount); }
        }
    }
...
static void startGodTimer(void) {
    ...
    dispatch_source_set_event_handler(g_godTimer, ^{
        if (g_playerCFLearned && g_playerCF) {
            if (g_godMode) makePlayerGodMode(g_playerCF);
            if (g_fullScreen) makeFullScreen(g_playerCF);
        }
    });
...
static void findIL2CPP(void) {
    jlog(@"=== v36.0 IL2CPP Runtime Search ===");
...
static void refreshButtons(void) {
    ...
    [g_btnFullScreen setTitle: g_fullScreen ? @"\U00002705 \u5168\u5c4f\u79d2\u6740" : @"\U0000274c \u5168\u5c4f\u79d2\u6740" forState:UIControlStateNormal];
...
static void layoutPanel(UIView *bv) {
    ...
    CGFloat pw=260, ph=310;  // v36: 加高放第4个按钮
...
- (void)onGodMode {
    ...
    if (g_godMode) {
        ...
        startGodTimer();
        jlog(@"GodMode启动: 只设Invincible=1不改HP(修复退图卡住)");
    }
...
- (void)onFullScreen {
    ...
    if (g_fullScreen) {
        findIL2CPP();
        if (!g_attackCanUseHooked) hookOneFunc(g_funcCheckSkillAttackCanUse, hookCheckSkillAttackCanUse, (void**)&g_origCheckSkillAttackCanUse, &g_attackCanUseHooked, "6.AttackCanUse(FullScreen)");
        if (!g_isReadyHooked) hookOneFunc(g_funcCheckSkillIsReady, hookCheckSkillIsReady, (void**)&g_origCheckSkillIsReady, &g_isReadyHooked, "5.IsReady(FullScreen)");
        startGodTimer();
        jlog(@"FullScreen启动: 修改normalBoundExtents碰撞箱范围");
    }
    refreshButtons(); jlog(@"Toggle 全屏秒杀: %d playerCF=%p learned=%d", g_fullScreen, g_playerCF, g_playerCFLearned);
}
...
static void setupUI(void) {
    ...
    title.text=@"\u5251\u5f71\u6c5f\u6e56 v36.0"; title.textColor=[UIColor cyanColor];
...
static void initialize(void) {
    ...
    if (loaded) return;
    loaded = YES;
    jlog(@"========== JYJH v36.0 (修复退图卡住+全屏秒杀) ==========");
    jlog(@"iOS %@", [[UIDevice currentDevice] systemVersion]);
    jlog(@"v36修复: GodMode只设Invincible=1不改HP(修复退图卡住)");
    jlog(@"v36新增: 全屏秒杀(修改normalBoundExtents碰撞箱范围)");
    jlog(@"v36: limitDamage默认30000(配合全屏秒杀)");
