/**
 * v44.0 - Tables命名空间修复 + Damage扩散替代DecreaseHP + UI宽屏优化
 * v43问题:
 *   1. Tables找到的是Dictionary内部Tables类,需要用namespace过滤找cfg.Tables
 *   2. DecreaseHP(ret=2300)但敌人仍只在水平线掉血->尝试Damage完整12参数扩散
 *   3. 菜单太长展示不全,宽度太窄
 */
...
static int64_t hDamage(void *f,void *atkEnt,void *atkCF,void *tgtEnt,void *tgtCF,
    ...
    if(atkP&&r>0){
        ...
        // v44: 全屏秒杀 - Damage完整12参数扩散 + DecreaseHP双保险
        if(g_fullScreen&&!tgtP&&g_playerEntity&&g_enemyCount>0){
            for(int i=0;i<g_enemyCount;i++){
                ...
                // 方案A: 用完整Damage扩散(12参数,传入原始碰撞参数)
                int64_t dr=g_oDamage(f,g_playerEntity,atkCF,eEnt,eCF,hitEid,hitSnd,isR,sBtn,sPart,hurtF,exS);
                // 方案B: 用DecreaseHP直接扣血(双保险)
                int64_t hr=0;
                if(g_origDecreaseHP){hr=g_origDecreaseHP(f,g_playerEntity,eEnt,eCF,r);}
                if(g_fullDmgLC<30){g_fullDmgLC++;jlog(@"FullDmg[%d]:cf=%p DmgRet=%lld DecHPRet=%lld",g_fullDmgLC,eCF,dr,hr);}
            }
        }
    }
    return r;
    ...
    for(size_t a=0;a<assemCount;a++){
        ...
        for(size_t c=0;c<cnt;c++){
            ...
            if(cn&&strcmp(cn,"Tables")==0&&!classTables){
                ...
                // 遍历所有字段找static instance
                if(get_fields&&field_get_name&&field_get_offset&&field_static_get){
                    void *fiter=NULL; void *field=NULL;
                    while((field=get_fields(klass,&fiter))!=NULL){
                        const char *fn=field_get_name(field);
                        size_t foff=field_get_offset(field);
                        uint32_t fflags=0;
                        if(field_get_flags)fflags=field_get_flags(field);
                        // static field: flags & 0x10 (FIELD_ATTRIBUTE_STATIC)
                        BOOL isStatic=(fflags&0x10)!=0;
                        if(fn)jlog(@"Tables field: %s off=%zu static=%d",fn,foff,isStatic);
                        // 尝试读所有static字段
                        if(isStatic&&field_static_get&&fn){
                            void *val=NULL;
                            field_static_get(field,&val);
                            if(val){
                                jlog(@"Tables static '%s' = %p",fn,val);
                                // 检查是否是Tables实例(读+0x230看是否非空)
                                void *tbRS=NULL;
                                memcpy(&tbRS,(uint8_t*)val+0x230,sizeof(void*));
                                if(tbRS){
                                    jlog(@"*** Tables instance found via '%s' = %p TbRoleSkin=%p ***",fn,val,tbRS);
                                    // 读取皮肤ID
                                    ...
                                    void *tbWS=NULL; memcpy(&tbWS,(uint8_t*)val+0x248,sizeof(void*));
                                    if(tbWS){
                                        void *dl2=NULL; memcpy(&dl2,(uint8_t*)tbWS+0x18,sizeof(void*));
                                        if(dl2){
                                            void *items2=NULL; memcpy(&items2,(uint8_t*)dl2+0x10,sizeof(void*));
                                            if(items2){
                                                int32_t arrLen2=0; memcpy(&arrLen2,(uint8_t*)items2+0x10,4);
                                                void *arrData2=(uint8_t*)items2+0x18;
                                                for(int i=0;i<arrLen2&&g_weaponSkinCount<MAX_SKIN_IDS;i++){
                                                    void *e=NULL; memcpy(&e,(uint8_t*)arrData2+i*8,sizeof(void*));
                                                    if(!e)continue;
...
// 面板大小
static CGFloat g_panelW=320, g_panelH=520;
static UIView *g_resizeHandle=nil;
...
-(void)onFullScreen{
    ...
    if(g_fullScreen){findIL2CPP();
        ...
        jlog(@"FullScreen ON: DecHP spread, enemies=0");
...
-(void)onDumpSkinIds{
    ...
    if(g_roleSkinCount==0&&g_weaponSkinCount==0)jlog(@"No skin IDs loaded - Tables instance not found, will try again next fight");
}
@end
...
static void setupUI(void){
    ...
    // 大面板 - 屏幕居中
    g_panel=[[UIView alloc]initWithFrame:CGRectMake(0,0,g_panelW,g_panelH)];g_panel.backgroundColor=IMGUI_BG;g_panel.layer.cornerRadius=10;g_panel.layer.borderWidth=1;g_panel.layer.borderColor=IMGUI_BORDER.CGColor;g_panel.hidden=YES;g_panel.clipsToBounds=YES;[win addSubview:g_panel];
    layoutPanelCenter();
    // Title
    UIView *tb=[[UIView alloc]initWithFrame:CGRectMake(0,0,g_panelW,32)];tb.backgroundColor=IMGUI_TITLE_BG;[g_panel addSubview:tb];
    UILabel *tl=[[UILabel alloc]initWithFrame:CGRectMake(10,6,g_panelW-20,20)];tl.text=@"\xe5\x89\x91\xe5\xbd\xb1\xe6\xb1\x9f\xe6\xb9\x96 v43.0";tl.textColor=IMGUI_ACCENT;tl.font=[UIFont boldSystemFontOfSize:15];tl.textAlignment=NSTextAlignmentCenter;[tb addSubview:tl];
    // Buttons
    CGFloat bx=12,bw=g_panelW-24,bh=30,by0=36,bdy=34;
    ...
    // Sep1
    CGFloat s1Y=by0+bdy*4;UIView *s1=[[UIView alloc]initWithFrame:CGRectMake(bx,s1Y,bw,1)];s1.backgroundColor=IMGUI_BORDER;[g_panel addSubview:s1];
    // Damage slider
    CGFloat sy=s1Y+8;
    g_sliderLabel=[[UILabel alloc]initWithFrame:CGRectMake(bx,sy,bw,18)];g_sliderLabel.text=[NSString stringWithFormat:@"\xe4\xbc\xa4\xe5\xae\xb3\xe4\xb8\x8a\xe9\x99\x90: %d",g_damageLimit];g_sliderLabel.textColor=IMGUI_DIMTEXT;g_sliderLabel.font=[UIFont systemFontOfSize:12];[g_panel addSubview:g_sliderLabel];
    g_slider=[[UISlider alloc]initWithFrame:CGRectMake(bx,sy+20,bw,28)];g_slider.minimumValue=1;g_slider.maximumValue=5000;g_slider.value=g_damageLimit;[g_slider addTarget:[JYJHActionHandler shared] action:@selector(sliderChanged:) forControlEvents:UIControlEventValueChanged];[g_panel addSubview:g_slider];
    // Sep2
    CGFloat s2Y=sy+52;UIView *s2=[[UIView alloc]initWithFrame:CGRectMake(bx,s2Y,bw,1)];s2.backgroundColor=IMGUI_BORDER;[g_panel addSubview:s2];
    // Skin section
    CGFloat ssy=s2Y+6;
    UILabel *secT=[[UILabel alloc]initWithFrame:CGRectMake(bx,ssy,bw,18)];secT.text=@"\xe7\x9a\xae\xe8\x82\xa4/\xe6\xad\xa6\xe5\x99\xa8" "(\xe6\x88\x98\xe6\x96\x97\xe4\xb8\xad\xe5\x86\x99" "Actor" "\xe5\xad\x97\xe6\xae\xb5)";secT.textColor=IMGUI_ACCENT;secT.font=[UIFont boldSystemFontOfSize:11];[g_panel addSubview:secT];
    // Skin slider
    g_skinLabel=[[UILabel alloc]initWithFrame:CGRectMake(bx,ssy+20,bw,18)];g_skinLabel.text=[NSString stringWithFormat:@"\xe7\x9a\xae\xe8\x82\xa4" "ID: %d",g_skinId];g_skinLabel.textColor=IMGUI_DIMTEXT;g_skinLabel.font=[UIFont systemFontOfSize:12];[g_panel addSubview:g_skinLabel];
    ...
    // Weapon slider
    g_weaponLabel=[[UILabel alloc]initWithFrame:CGRectMake(bx,ssy+68,bw,18)];g_weaponLabel.text=[NSString stringWithFormat:@"\xe6\xad\xa6\xe5\x99\xa8" "ID: %d",g_weaponId];g_weaponLabel.textColor=IMGUI_DIMTEXT;g_weaponLabel.font=[UIFont systemFontOfSize:12];[g_panel addSubview:g_weaponLabel];
    ...
    [g_panel addSubview:g_resizeHandle];
    refreshBtns();
}
