/**
 * v101 - 彻底换方案: hook UpdatePart修改参数
 * v100问题:
 *   1. NeedUpdatePart=true导致无限循环: 游戏检测true→SetSecneData→hook又设true→死循环→卡死
 *   2. 只改ScenePlayerData数据没有视觉效果 — 游戏不重新渲染
 * v101方案:
 *   1. 完全放弃SetSecneData/SetData hook (改数据无效+NeedUpdatePart循环)
 *   2. hook LobbyActorData.UpdatePart(skinId, weaponId, haloId) — 3参数
 *      游戏主动调用时修改参数 → 替换skin/weapon/halo ID
 *   3. 不设NeedUpdatePart! 不主动调用任何IL2CPP函数!
 *   4. 不hook SetData/SetSecneData (同地址冲突)
 */
...
// v101: hook UpdatePart + InitWithSkinId
static void *g_fUpdatePart=NULL;          // LobbyActorData.UpdatePart(skinId, weaponId, haloId) 3参数
static void *g_oUpdatePart=NULL; static BOOL g_hUpdatePart=NO;
typedef void (*UpdatePartOrigFunc)(void*,void*,int32_t,int32_t,int32_t);
static void *g_fInitWithSkinId=NULL;      // SceneActorSpineAvatar.InitWithSkinId(skinId, changeType) 2参数
static void *g_oInitWithSkinId=NULL; static BOOL g_hInitWithSkinId=NO;
typedef void (*InitWithSkinIdOrigFunc)(void*,void*,int32_t,int32_t);
...
// ===== v101: Hook LobbyActorData.UpdatePart =====
// UpdatePart(skinId, weaponId, haloId) — 游戏主动调用时修改参数
// self=LobbyActorData, p2=MethodInfo*, p3=skinId, p4=weaponId, p5=haloId
static int g_updatePartLC=0;
static void hUpdatePart(void *mi, void *self, int32_t skinId, int32_t weaponId, int32_t haloId) {
    if(!self) { if(g_oUpdatePart) ((UpdatePartOrigFunc)g_oUpdatePart)(mi, self, skinId, weaponId, haloId); return; }
    
    // 检查IsSelf
    BOOL isSelf = NO;
    memcpy(&isSelf, (uint8_t*)self + 0x2b, 1);
    
    if(g_updatePartLC < 20) {
        g_updatePartLC++;
        jlog(@"UpdatePart: self=%p isSelf=%d skin=%d weapon=%d halo=%d (applied: skin=%d weapon=%d halo=%d)", 
             self, isSelf, skinId, weaponId, haloId, g_appliedSkinId, g_appliedWeaponId, g_appliedHaloId);
    }
    
    // 只修改IsSelf的角色
    if(isSelf) {
        if(g_appliedSkinId > 0) skinId = g_appliedSkinId;
        if(g_appliedWeaponId > 0) weaponId = g_appliedWeaponId;
        if(g_appliedHaloId > 0) haloId = g_appliedHaloId;
        jlog(@"UpdatePart[IsSelf]: calling with skin=%d weapon=%d halo=%d", skinId, weaponId, haloId);
    }
    
    if(g_oUpdatePart) ((UpdatePartOrigFunc)g_oUpdatePart)(mi, self, skinId, weaponId, haloId);
}

// ===== v101: Hook SceneActorSpineAvatar.InitWithSkinId =====
// InitWithSkinId(skinId, changeType) — Spine初始化时替换skinId
// self=SceneActorSpineAvatar, p2=MethodInfo*, p3=skinId, p4=changeType
static int g_initSkinIdLC=0;
static void hInitWithSkinId(void *mi, void *self, int32_t skinId, int32_t changeType) {
    if(g_initSkinIdLC < 20) {
        g_initSkinIdLC++;
        jlog(@"InitWithSkinId: self=%p skinId=%d changeType=%d (applied: skin=%d)", self, skinId, changeType, g_appliedSkinId);
    }
    // 替换skinId
    if(g_appliedSkinId > 0) {
        skinId = g_appliedSkinId;
        jlog(@"InitWithSkinId: replaced skinId=%d", skinId);
    }
    if(g_oInitWithSkinId) ((InitWithSkinIdOrigFunc)g_oInitWithSkinId)(mi, self, skinId, changeType);
}
...
static void findIL2CPP(void) {
    ...
    if(g_classLobbyActorData && get_fields && field_name && field_offset) {
        jlog(@"=== v101: Dump LobbyActorData fields (runtime offsets) ===");
        void *fiter=NULL, *field=NULL;
        while((field=get_fields(g_classLobbyActorData,&fiter))!=NULL) {
            const char *fn=field_name(field);
            int32_t off=field_offset(field);
    ...
    for(size_t a=0;a<assemCount2;a++){
        ...
        for(size_t c=0;c<cnt;c++){
            ...
            while((m=get_methods(klass,&iter))!=NULL){
                ...
                // v101: 只搜索UpdatePart (3参数, LobbyActorData类)
                ...
                // v101: 搜索InitWithSkinId (2参数, SceneActorSpineAvatar类)
    ...
    jlog(@"v101: UpdatePart=%p InitWithSkinId=%p",g_fUpdatePart,g_fInitWithSkinId);
...
// v101: 应用外观 - 只设置g_appliedXxxId
// UpdatePart hook会在游戏下次调用时自动替换参数
// 不设NeedUpdatePart! 不主动调用任何IL2CPP函数!

static void applyPartChange(void) {
    jlog(@"applyPart: skin=%d weapon=%d halo=%d (UpdatePart hook will apply on next call)", 
         g_appliedSkinId, g_appliedWeaponId, g_appliedHaloId);
    // v101: 不做任何内存修改！
    // UpdatePart hook会在游戏主动调用时替换参数
    // 如果UpdatePart没被调用，说明游戏不调用这个方法来更新外观
}
...
-(void)ensureSkinHooksActive {
    ...
    // hook InitWithSkinId
    if(g_fInitWithSkinId && !g_hInitWithSkinId) hookOneFunc(g_fInitWithSkinId, hInitWithSkinId, (void**)&g_oInitWithSkinId, &g_hInitWithSkinId, "InitWithSkinId");
}
-(void)onApplySkin{
...
static void setupUI(void){
    ...
    UILabel *tl=[[UILabel alloc]initWithFrame:CGRectMake(10,6,g_panelW-20,20)];tl.text=@"剑影江湖 v101";tl.textColor=IMGUI_ACCENT;tl.font=[UIFont boldSystemFontOfSize:15];tl.textAlignment=NSTextAlignmentCenter;[tb addSubview:tl];
...
static void initialize(void){
    ...
    jlog(@"========== JYJH v101 (UpdatePart+InitWithSkinId hook) ==========");
