/**
 * v104 - SSO bypass: 修复static_field_data返回0x0
 * v103问题:
 *   1. il2cpp_class_get_static_field_data返回0x0 — iOS 15.6上不可用
 *   2. DoHandShakeCheck hook成功但没被调用
 *   3. Singleton<T>实例在Nested<T>中, 不在LoginModel静态区
 * v104方案:
 *   1. 用il2cpp_field_static_get_value获取Singleton实例
 *   2. 遍历LoginModel/LoginCtrl的所有字段(包括继承), 找instance静态字段
 *   3. 备选: 搜索所有Nested类, 找Nested<LoginModel>的instance字段
 *   4. 保留DoHandShakeCheck hook + 定时内存修补
 */
...
// v104: 保留g_fGetCode/g_fGetBanTime用于搜索, 但不再MSHookFunction它们
...
// v104: 直接内存修改SSOResponData (备用方案, 声明在前, 定义在isValidPtr/getObjClassName之后)
...
// v104: 直接内存修改SSOResponData (实现)
// v103问题: il2cpp_class_get_static_field_data返回0x0 (iOS 15.6不可用)
// v104方案: 1)调用get_Instance() 2)il2cpp_field_static_get_value 3)LoginCtrl.mModel
static void patchSSOResponDataMemory(void) {
    ...
    // 方案1: 调用LoginModel.get_Instance()获取单例
    {
        typedef void* (*GMFn)(void*,void**); GMFn gm=(GMFn)dlsym(h,"il2cpp_class_get_methods");
        typedef const char* (*MNFn)(void*); MNFn mn=(MNFn)dlsym(h,"il2cpp_method_get_name");
        typedef void* (*CMFn)(void*); CMFn cm=(CMFn)dlsym(h,"il2cpp_compile_method");
        if(gm&&mn&&cm) {
            void *it=NULL,*mi=NULL;
            while((mi=gm(g_classLoginModel,&it))!=NULL) {
                const char *name=mn(mi);
                if(name&&strcmp(name,"get_Instance")==0) {
                    void *fp=cm(mi);
                    if(fp) {
                        jlog(@"SSO: get_Instance native=%p",fp);
                        typedef void* (*GIFn)(void*);
                        loginModelInstance=((GIFn)fp)(NULL);
                        jlog(@"SSO: get_Instance()=%p",loginModelInstance);
                    }
                    break;
                }
            }
    ...
    // 方案2: il2cpp_field_static_get_value读取静态instance字段
    if(!isValidPtr(loginModelInstance)) {
        jlog(@"SSO: get_Instance failed, trying field_static_get_value...");
        ...
        typedef int32_t (*GFOFn)(void*); GFOFn gfo=(GFOFn)dlsym(h,"il2cpp_field_get_offset");
        typedef void (*FSGVFn)(void*,void*); FSGVFn fsgv=(FSGVFn)dlsym(h,"il2cpp_field_static_get_value");
        typedef uint32_t (*FAFn)(void*); FAFn fa=(FAFn)dlsym(h,"il2cpp_field_get_flags");
        if(gfd&&gfn&&fsgv) {
            void *it=NULL,*fld=NULL;
            while((fld=gfd(g_classLoginModel,&it))!=NULL) {
                const char *fn=gfn(fld);
                int32_t fo=gfo?gfo(fld):-1;
                uint32_t attr=fa?fa(fld):0;
                BOOL isStatic=(attr&0x0010)!=0;
                if(fn) jlog(@"  LoginModel field: %s offset=%d attr=0x%x%s",fn,fo,attr,isStatic?" [STATIC]":"");
                if(isStatic&&fn&&(strcmp(fn,"instance")==0||strcmp(fn,"_instance")==0||strcmp(fn,"s_instance")==0)) {
                    void *val=NULL;
                    fsgv(fld,&val);
                    jlog(@"SSO: field_static_get_value('%s')=%p",fn,val);
                    if(isValidPtr(val)){loginModelInstance=val;break;}
                }
            }
        }
    }

    // 方案3: 通过LoginCtrl单例 -> mModel字段(offset 0x10)
    if(!isValidPtr(loginModelInstance)&&g_classLoginCtrl) {
        jlog(@"SSO: Trying LoginCtrl.mModel...");
        typedef void* (*GMFn)(void*,void**); GMFn gm=(GMFn)dlsym(h,"il2cpp_class_get_methods");
        typedef const char* (*MNFn)(void*); MNFn mn=(MNFn)dlsym(h,"il2cpp_method_get_name");
        typedef void* (*CMFn)(void*); CMFn cm=(CMFn)dlsym(h,"il2cpp_compile_method");
        if(gm&&mn&&cm) {
            void *it=NULL,*mi=NULL;
            while((mi=gm(g_classLoginCtrl,&it))!=NULL) {
                const char *name=mn(mi);
                if(name&&strcmp(name,"get_Instance")==0) {
                    void *ctrlFunc=cm(mi);
                    if(ctrlFunc) {
                        jlog(@"SSO: LoginCtrl.get_Instance native=%p",ctrlFunc);
                        typedef void* (*GIFn)(void*);
                        void *loginCtrlInstance=((GIFn)ctrlFunc)(NULL);
                        jlog(@"SSO: LoginCtrl.get_Instance()=%p",loginCtrlInstance);
                        if(isValidPtr(loginCtrlInstance)) {
                            // LoginCtrl.mModel @ offset 0x10
                            ...
                            if(isValidPtr(loginModelInstance)) {
                                ...
                                jlog(@"SSO: mModel class=%s",cn?cn:"null");
                            }
                        }
                    }
                    break;
...
static void findIL2CPP(void) {
    ...
    for(size_t a=0;a<assemCount;a++){
        ...
        for(size_t c=0;c<cnt;c++){
            ...
            // v104: 搜索LoginModel类 (Singleton, 保存CurSSOResponData)
            ...
            // v104: 搜索LoginCtrl类 (有DoHandShakeCheck方法)
            ...
            while((m=get_methods(klass,&iter))!=NULL){
                ...
                // v104: LoginCtrl.DoHandShakeCheck(int32 code) — 唯一地址0x2b7eb2c, 不被泛型共享
                ...
                // v102→v104: 保留get_code/get_banTime搜索仅用于日志确认, 不再MSHookFunction它们
    ...
    // v104: 用il2cpp_class_from_name搜索热更新DLL中的LoginModel/LoginCtrl/DoHandShakeCheck
            ...
            for(size_t a=0;a<assemCount;a++){
                ...
                if(!g_classLoginModel) {
                    ...
                    if(lmClass) {
                        ...
                        jlog(@"v104: Found LoginModel class in assembly %zu: %p", a, lmClass);
                    }
                }
                ...
                if(!g_fDoHandShakeCheck) {
                    ...
                    if(lcClass) {
                        ...
                        jlog(@"v104: Found LoginCtrl class in assembly %zu: %p", a, lcClass);
                ...
                if(!g_fGetCode) {
                    ...
                    if(ssoRspClass) {
                        jlog(@"v104: Found SSOResponData class: %p", ssoRspClass);
    ...
    jlog(@"v104: DoHandShakeCheck=%p LoginModel=%p LoginCtrl=%p",
         g_fDoHandShakeCheck, g_classLoginModel, g_classLoginCtrl);
    if(!g_fDoHandShakeCheck) jlog(@"v104: DoHandShakeCheck NOT FOUND!");
    if(!g_classLoginModel) jlog(@"v104: LoginModel class NOT FOUND!");
}
...
-(void)onBypassSSO{
    ...
    if(g_bypassSSO){findIL2CPP();
        // v104: 优先hook DoHandShakeCheck (唯一地址, 不被泛型共享)
...
static void initialize(void){
    ...
    jlog(@"========== JYJH v104 (SSO bypass: get_Instance+field_static+Ctrl) ==========");
