/**
 * v65.0 - 修复技能替换(HandleSkillRange搜索+skillButton含义) + 皮肤扫描链表修复
 * v64测试:
 *   1. 技能替换无效: HandleSkillRange搜索条件pc==3匹配不到(v64日志: not found)
 *      -> v65: 改搜索条件为name=="HandleSkillRange"且pc>=3
 *      -> 发现skillButton不是CharacterStateType枚举(14-19)! 而是内部编号(0,4,6,3000等)
 *      -> 改为Hook UseSkill(Frame, EntityRef, CharacterFiled*, CharacterStateType skillStateType, ...)替换stateType
 *   2. 皮肤扫描: LinkedList布局修正
 *      -> v64 dump: listObj+0x0是head指针(0x12f844520), 不是版本号!
 *      -> head=List[+0x0], 从head开始遍历节点
 *      -> LinkedListNode: +0x0=list, +0x8=prev, +0x10=next, +0x18=item (IL2CPP对象header偏移)
 */
...
// v65: UseSkill类型: void UseSkill(Frame, EntityRef, CharacterFiled*, CharacterStateType skillStateType, BOOL isRight, CharacterStatesAsset states, BaseCharacterState state, PlayerInfo* playerInfo)
...
// v65: HandleSkillRange Hook - 仍需Hook来修改技能范围
// v65: UseSkill Hook - 技能替换核心: CharacterStateType 14-18->19
...
static void hUseSkill(void *f,void *entity,void *cf,int skillStateType,BOOL isRight,void *states,void *state,void *playerInfo) {
    if(g_skillReplace&&skillStateType>=14&&skillStateType<=18){
        if(g_useSkillLC<20){g_useSkillLC++;jlog(@"UseSkill: stateType %d->19",skillStateType);}
        skillStateType=19;
    }
    ...
    if(!g_classHotfixGameEntry){
        for(int i=0;i<g_gameEntryCount;i++){
            if(g_allGameEntryNS[i]==NULL||strcmp(g_allGameEntryNS[i],"")==0){
                ...
                jlog(@"v65: assume GameEntry[%d] ns=empty is HotfixGameEntry=%p",i,g_allGameEntries[i]);
                break;
            }
        }
    }
    jlog(@"v65: UnityGameEntry=%p HotfixGameEntry=%p",g_classUnityGameEntry,g_classHotfixGameEntry);
...
static void setupUI(void){
    ...
    UILabel *tl=[[UILabel alloc]initWithFrame:CGRectMake(10,6,g_panelW-20,20)];tl.text=@"\xe5\x89\x91\xe5\xbd\xb1\xe6\xb1\x9f\xe6\xb9\x96 v65.0";tl.textColor=IMGUI_ACCENT;tl.font=[UIFont boldSystemFontOfSize:15];tl.textAlignment=NSTextAlignmentCenter;[tb addSubview:tl];
