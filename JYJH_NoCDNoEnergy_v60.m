/**
 * v60.0 - 修复皮肤扫描 + 大招增强改为Hook IsExSkillInCD
 * v59测试:
 *   HitSystem偏移0x48/0x50正确(战斗中非零,城镇中0=未初始化)
 *   皮肤扫描: GetComponent(C字符串/IL2CPP String)都返回NULL, 需用runtime_invoke
 *   大招增强: TryTriggerExSkill透传无效果, 改Hook IsExSkillInCD返回NO
 * v60方案:
 *   1. 皮肤扫描: 找ConfigComponent类→找get_Tables方法→il2cpp_runtime_invoke调用→获取Tables
 *   2. 大招增强: Hook IsExSkillInCD(private方法at 0x30b9a78)返回NO, ExSkill永不在CD
 *   3. HitSystem: 偏移已验证正确, 不修改(战斗中有效)
 */
...
// v60: 大招增强 - Hook IsExSkillInCD 让所有ExSkill永远不在CD
// IsExSkillInCD(FP now, ExSkillData* skillp, ExSkillInfo info) at 0x30b9a78
// 是private static方法, 在ExSkillHelper.TryTriggerExSkill中被调用判断CD
// Hook返回NO → 所有ExSkill(被动/大招/触发技能)永远不在CD
typedef BOOL (*IsExSkillInCDFunc)(void*,void*,void*);
...
static BOOL hIsExSkillInCD(void *now,void *skillp,void *info) {
    if(g_exSkillAlways) {
        if(g_exSkillInCDLC<20){g_exSkillInCDLC++;jlog(@"IsExSkillInCD: forced NO");}
        return NO; // 永远不在CD
    }
    return g_oIsExSkillInCD?g_oIsExSkillInCD(now,skillp,info):NO;
}
...
// v60: 皮肤扫描 - Hook ConfigComponent.Awake()保存实例, 然后读Tables字段
...
static void *g_configCompInstance=NULL; // ConfigComponent实例, 由Awake Hook保存
...
// Hook ConfigComponent.Awake() 保存实例
typedef void (*ConfigCompAwakeFunc)(void*);
static void *g_fConfigCompAwake=NULL; static ConfigCompAwakeFunc g_oConfigCompAwake=NULL; static BOOL g_hConfigCompAwake=NO;
static void hConfigCompAwake(void *self) {
    if(self && !g_configCompInstance) {
        g_configCompInstance=self;
        jlog(@"FOUND ConfigComponent instance=%p",self);
    }
    if(g_oConfigCompAwake) g_oConfigCompAwake(self);
}
...
static void scanSkinIds(void) {
    ...
    if(!isValidPtr(g_configCompInstance)){jlog(@"ScanSkin: ConfigComponent instance not found yet");return;}

    // ConfigComponent.<Tables>k__BackingField at +0x28 (class field, dump=actual)
    void *tables_l=NULL;
    memcpy(&tables_l,(uint8_t*)g_configCompInstance+0x28,8);
    jlog(@"ScanSkin: configComp=%p tables=%p",g_configCompInstance,tables_l);
    if(!isValidPtr(tables_l)){
        // 尝试+0x20
        memcpy(&tables_l,(uint8_t*)g_configCompInstance+0x20,8);
    ...
    if(!isValidPtr(tables_l)){
        ...
        for(int i=0;i<12;i++){
            uint64_t v=0; memcpy(&v,(uint8_t*)g_configCompInstance+i*8,8);
    ...
    for(size_t a=0;a<assemCount;a++){
        ...
        for(size_t c=0;c<cnt;c++){
            ...
            while((m=get_methods(klass,&iter))!=NULL){
                ...
                // v60: 搜索ConfigComponent.Awake - 保存实例用于皮肤扫描
                else if(strcmp(n,"Awake")==0&&pc==0&&cn&&strcmp(cn,"ConfigComponent")==0&&!g_fConfigCompAwake){
                    g_fConfigCompAwake=fa;found++;jlog(@"FOUND ConfigComponent.Awake %p",fa);
                }
                // v60: 搜索ExSkillHelper.IsExSkillInCD - 大招CD检测
                else if(strcmp(n,"IsExSkillInCD")==0&&pc==3&&!g_fIsExSkillInCD){
                    g_fIsExSkillInCD=fa;found++;jlog(@"FOUND IsExSkillInCD p=%u %p",pc,fa);
                }
            }
        }
    }
...
-(void)onExSkillAlways{
    ...
    if(g_exSkillAlways){findIL2CPP();
        if(!g_hIsExSkillInCD && g_fIsExSkillInCD) hookOneFunc(g_fIsExSkillInCD,hIsExSkillInCD,(void**)&g_oIsExSkillInCD,&g_hIsExSkillInCD,"IsExSkillInCD");
    }
    refreshBtns();jlog(@"Toggle ExSkillAlways: %d func=%p hooked=%d",g_exSkillAlways,g_fIsExSkillInCD,g_hIsExSkillInCD);
}
...
-(void)onDumpSkinIds{
    if(!g_classConfigComponent){jlog(@"DumpSkin: ConfigComponent not found, running findIL2CPP...");findIL2CPP();}
    // Hook ConfigComponent.Awake以捕获实例(如果还没有)
    if(g_fConfigCompAwake && !g_hConfigCompAwake) {
        hookOneFunc(g_fConfigCompAwake,hConfigCompAwake,(void**)&g_oConfigCompAwake,&g_hConfigCompAwake,"ConfigComp.Awake");
    }
