/**
 * v82 - 纯v71代码, 但不自动hook limitDmg
 * 所有crash都发生在applyAllHooks的DobbyHook(limitDmg)
 * v82: applyAllHooks为空函数, 所有hook都是懒加载(点按钮才hook)
 * 这样如果findIL2CPP本身不crash, 游戏就能正常启动
 * 如果DobbyHook某个函数crash, 只影响那个功能, 不会整体闪退
 */
...
static void findIL2CPP(void) {
    ...
    g_il2cppDone=YES;
    jlog(@"v82: found %d targets, scanned %d methods",found,totalMethods);
    jlog(@"v82: limitDmg=%p IsReady=%p AttackCanUse=%p CanBeAttack=%p Damage=%p",
         g_fLimitDmg,g_fIsReady,g_fAttackCanUse,g_fCanBeAttack,g_fDamage);
    jlog(@"v82: Intersects=%p CheckHit=%p HitSysUpd=%p MoveStep=%p",
         g_fIntersects,g_fCheckHit,g_fHitSystemUpdate,g_fMoveStep);
    jlog(@"v82: UseSkill=%p UpdateSkillCD=%p HandleSkillRange=%p Unlock=%p",
         g_fUseSkill,g_fUpdateSkillCD,g_fHandleSkillRange,g_fUnlock);
    jlog(@"v82: getSkinId=%p InitWithSkinId=%p Actor=%p",
...
static void hookOneFunc(void *fa,void *hf,void **of,BOOL *hf2,const char *name){
    ...
    jlog(@"%s: hooking %p...",name,fa);
    int r=DobbyHook(fa,hf,of);
    if(r==0){*hf2=YES;jlog(@"%s: OK orig=%p",name,*of);}
    else jlog(@"%s: FAILED ret=%d",name,r);
}

// v82: 不自动hook! applyAllHooks为空
static void applyAllHooks(void){jlog(@"v82: applyAllHooks skipped (lazy hook mode)");}
