/**
 * v117 - 修复吸怪(精确匹配CharacterFiled.Move) + 修复免广告(ShowVideo hook)
 * v116问题:
 *   1. Move搜索用strstr(cn,"Character")匹配到了CharacterController3D.Move — 错误!
 *      v116日志: FOUND CharacterController3D.Move p=7 — 这是3D控制器, 不是2D的CharacterFiled.Move
 *      CharacterFiled.Move(Frame,Transform2D*,CharacterFiled*,FPVector2,FP,FP,Boolean) @ 0x30754b8 才是带Transform2D*的
 *   2. IsAdAvailable hook签名不确定(mi问题) — C#实例方法, MSHookFunction对mi传递不确定
 *      4参数(mi,self,id,showButton)如果mi不存在则参数全部错位
 * v117改动:
 *   1. Move搜索改为strcmp(cn,"CharacterFiled")==0 && pc==7 精确匹配
 *   2. 免广告改为: hook ShowVideo(5参数)直接跳过广告播放 + 修改AdsHelper单例字段
 *   3. Move hook增加详细调试日志(打印所有参数指针值)
 *   4. 保留IsAdAvailable hook但改为同时试3参数和4参数(运行时检测)
 */
...
static void findIL2CPP(void) {
    ...
    jlog(@"v117: SetSecneData=%p UpdatePart=%p ChangeCharacterToDead=%p Move=%p IsAdAvailable=%p ShowVideo=%p",g_fSetSecneData,g_fUpdatePart,g_fChangeCharacterToDead,g_fMove,g_fIsAdAvailable,g_fShowVideo);
...
static CGFloat g_panelW=360, g_panelH=900;
...
static void setupUI(void){
    ...
    CGFloat contentH=by0+bdy*8+8+52+52+8+18+20+28+20+28+20+28+24+24+24+8+18+20+28+60+28;
    ...
    g_btnFullScreen=mkBtn(CGRectMake(bx,by0+bdy*6,bw,bh),@selector(onFullScreen));[sv addSubview:g_btnFullScreen];
    g_btnSkillReplace=mkBtn(CGRectMake(bx,by0+bdy*7,bw,bh),@selector(onSkillReplace));[sv addSubview:g_btnSkillReplace];
    CGFloat repY=by0+bdy*8;
    ...
    // === 伤害区域 ===
    CGFloat s1Y=by0+bdy*8+4;UIView *s1=[[UIView alloc]initWithFrame:CGRectMake(bx,s1Y,bw,1)];s1.backgroundColor=IMGUI_BORDER;[sv addSubview:s1];
    CGFloat sy=s1Y+8;
