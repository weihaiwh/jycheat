/**
 * v48.0 - 修复DecreaseHP扩散日志限制 + ScrollView交互 + 面板可拖动
 * v47问题:
 *   1. DecreaseHP扩散被日志限制if(g_fullDmgLC<5)阻止,只扩散5次就停了
 *   2. ScrollView无法滚动 - 可能事件被拦截
 *   3. 面板无法拖动 - 没有拖动手势
 *   4. 血条清空需要再打一下 - 可能伤害值或isDead判定问题
 * v48方案:
 *   1. 移除扩散循环内的日志限制,改为每次攻击重置计数器
 *   2. ScrollView设置delaysContentTouches=NO确保立即响应
 *   3. 标题栏添加拖动手势移动整个面板
 *   4. 增大伤害倍率确保敌人死亡
 */
...
-(void)touchesMoved:(NSSet*)t withEvent:(UIEvent*)e{
    ...
    f.origin.y=MAX(0,MIN(sc.size.height-f.size.height,f.origin.y+dy));
    g_panel.frame=f;
    _ts=c;
}
@end
...
static void setupUI(void){
    ...
    UILabel *tl=[[UILabel alloc]initWithFrame:CGRectMake(10,6,g_panelW-20,20)];tl.text=@"\xe5\x89\x91\xe5\xbd\xb1\xe6\xb1\x9f\xe6\xb9\x96 v48.0";tl.textColor=IMGUI_ACCENT;tl.font=[UIFont boldSystemFontOfSize:15];tl.textAlignment=NSTextAlignmentCenter;[tb addSubview:tl];
    // UIScrollView (滚动内容区, v48: 确保可滚动)
