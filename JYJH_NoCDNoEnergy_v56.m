/**
 * v56.0 - 修复启动卡死: scanSkinIds在主线程阻塞
 * v55问题:
 *   findIL2CPP()末尾自动调用scanSkinIds(), 在主线程循环2000次GetRoleSkinById
 *   游戏初始化阶段调用IL2CPP静态方法→死锁/卡死
 * v56方案:
 *   1. 移除findIL2CPP()末尾的自动scanSkinIds()调用
 *   2. scanSkinIds()改为用户点击按钮时在后台线程执行
 *   3. 新增"扫描皮肤ID"按钮(仅在用户点击时触发)
 *   4. 保留v55的HitSystem偏移修正(0x48/0x50)和皮肤卡死修复
 */
...
static void scanSkinIds(void) {
    ...
    g_skinIdsLoaded=YES;
    if(!g_fGetRoleSkinById) {jlog(@"ScanSkinIds: GetRoleSkinById not found");return;}
    g_roleSkinCount=0; g_weaponSkinCount=0;
    jlog(@"ScanSkinIds: scanning 1~2000...");
...
// v56: 读取皮肤ID列表 - 主线程执行但用户手动触发(不在init时自动调用)
-(void)onDumpSkinIds{
    if(!g_fGetRoleSkinById){jlog(@"DumpSkin: GetRoleSkinById not found, running findIL2CPP...");findIL2CPP();}
    if(!g_fGetRoleSkinById){jlog(@"DumpSkin: still not found, abort");return;}
    if(g_skinIdsLoaded){
        jlog(@"DumpSkin: already scanned, role=%d",g_roleSkinCount);
        refreshBtns();
        return;
    }
    // v56: 在主线程执行但限制扫描范围,先扫描1~500看效果
    // 如果需要更多可以后续扩大
    jlog(@"DumpSkin: starting scan...");
    scanSkinIds();
    refreshBtns();
}
@end
...
static void setupUI(void){
    ...
    UILabel *tl=[[UILabel alloc]initWithFrame:CGRectMake(10,6,g_panelW-20,20)];tl.text=@"\xe5\x89\x91\xe5\xbd\xb1\xe6\xb1\x9f\xe6\xb9\x96 v56.0";tl.textColor=IMGUI_ACCENT;tl.font=[UIFont boldSystemFontOfSize:15];tl.textAlignment=NSTextAlignmentCenter;[tb addSubview:tl];
    // UIScrollView (滚动内容区)
