 ...
 * v124 - Y轴限制 + 吸怪瞬移整合到全屏
 * v123基础上改动:
 *   1. Move hook增加Y轴限制: g_fullScreen开启且是玩家时, dir.x=0 → 只能上下移动
 *      Frida v12b确认: Move(Frame,Transform2D*,CF*,FPVector2 dir,FP speed,FP dt,Boolean)
 *      MSHookFunction传mi → 8参数hook(mi,f,tf,cf,dir.x,dir.y,speed,dt,updateFootY)
 *      dir.x=args[3], dir.y=args[4]; 设dir.x=0禁止水平移动
 *   2. 全屏按钮整合吸怪: 开启全屏=全屏秒杀+Y轴限制+吸怪(一键三连)
 *      全屏开启时自动启动vacuumTimer把怪瞬移到玩家位置
 *      全屏关闭时自动停止vacuumTimer
 *   3. 吸怪改进: vacuumTick同时修改Position.X和Position.Y(瞬移到玩家脚下)
 *      之前只改X(同一垂直线), 现在X+Y都改(直接到玩家位置)
 */
...
// v116: 8参数hook(含mi), 原函数7参数(不含mi)
// v124b: 回退到v123的7参数原函数指针(9参数hook不触发!)
...
// v124c: 从Damage hook捕获玩家CF, 不依赖Move hook
// Damage hook正常工作, 每次玩家攻击时都能拿到atkCF
static void *g_playerCFFromDmg=NULL;
...
// v123: 用EntityType==100(MapStaticType.Player)判断玩家
// CharacterFiled是值类型struct, dump offset减0x10得actual offset
// EntityType @ dump 0x48 → actual 0x38 (4字节int, MapStaticType枚举)
// MapStaticType: Teleporter=0, Normal=1, Elitist=2, Boss=3, SkyBoss=4, Npc=10, SceneObject=20, Player=100
// Camp @ dump 0x34 → actual 0x24 (4字节int) — 但Camp值不确定, 不用
static BOOL isPlayerCF(void *cf) { if(!cf)return NO; int32_t v=-1; memcpy(&v,(uint8_t*)cf+0x38,4); return v==100; }
// isDead @ dump 0x58 → actual 0x48 (QBoolean=4字节int, !=0表示dead)
static BOOL isDeadCF(void *cf) { if(!cf)return NO; int32_t v=0; memcpy(&v,(uint8_t*)cf+0x48,4); return v!=0; }
// IsAI @ dump 0x54 → actual 0x44 (QBoolean=4字节int)
static BOOL isAICF(void *cf) { if(!cf)return NO; int32_t v=0; memcpy(&v,(uint8_t*)cf+0x44,4); return v!=0; }
// EntityType @ dump 0x48 → actual 0x38
static int32_t getEntityType(void *cf) { if(!cf)return -1; int32_t v=-1; memcpy(&v,(uint8_t*)cf+0x38,4); return v; }
// Camp @ dump 0x34 → actual 0x24
static int32_t getCamp(void *cf) { if(!cf)return -1; int32_t v=-1; memcpy(&v,(uint8_t*)cf+0x24,4); return v; }
...
// v121: 记录玩家EntityRef.Raw — 在Damage hook中设置
static uint64_t g_playerEntityRaw=0;

// 检查entity是否是已知的敌人(g_enemyEntities中存的是void*但值是EntityRef.Raw)
static BOOL isKnownEnemyEntity(uint64_t entityRaw) {
    for(int i=0;i<g_enemyCount;i++) {
        uint64_t enemyRaw=0;
        memcpy(&enemyRaw, &g_enemyEntities[i], 8);
        if(enemyRaw == entityRaw) return YES;
    }
    return NO;
}
...
static void hAIAgentUpdate(void *mi_or_self, void *self_or_params, void *maybe_params) {
    if(g_monsterFreeze) {
        // AIAgent是struct, Update(ref AIParams& Params) 是实例方法, 1参数
        // MSHookFunction对实例方法可能传mi → 3参数(mi,self,params) 或 2参数(self,params)
        // 需要判断哪个参数是params:
        //   如果2参数: mi_or_self=self, self_or_params=params
        //   如果3参数: mi_or_self=mi, self_or_params=self, maybe_params=params
        // AIParams*指向的结构: _entity @ actual 0x10 (EntityRef.Raw, 8字节)
        
        void *params = self_or_params;
        
        // v123: 尝试两种参数布局 — 检查哪个看起来像有效指针
        // 如果3参数模式: maybe_params才是params
        // 如果2参数模式: self_or_params是params
        // 先试self_or_params, 如果_entity=0再试maybe_params
        uint64_t entityRaw = 0;
        if(params && isValidPtr(params)) {
            memcpy(&entityRaw, (uint8_t*)params + 0x10, 8);
        }
        // 如果entityRaw=0且maybe_params有效, 试试3参数模式
        if(entityRaw == 0 && maybe_params && isValidPtr(maybe_params)) {
            uint64_t testRaw = 0;
            memcpy(&testRaw, (uint8_t*)maybe_params + 0x10, 8);
            if(testRaw != 0) {
                params = maybe_params;
                entityRaw = testRaw;
            }
        ...
        // v123: 通过g_playerEntityRaw精确判断玩家
        // g_playerEntityRaw在Damage hook中设置(玩家作为攻击者时)
        BOOL isPlayer = NO;
        
        if(g_playerEntityRaw != 0) {
            isPlayer = (entityRaw == g_playerEntityRaw);
        } else {
            // Damage hook还没触发过 → 安全判断: 只冻结已知敌人
            isPlayer = !isKnownEnemyEntity(entityRaw);
        }
        ...
        if(!isPlayer) {
            ...
            if(g_aiFreezeLC<20){
                g_aiFreezeLC++;
                jlog(@"AI[%d]: FROZEN entity=0x%llx (player=0x%llx knownEnemy=%d)",g_aiFreezeLC,entityRaw,g_playerEntityRaw,isKnownEnemyEntity(entityRaw));
            }
            return;
        }
        // 玩家AI → 放行(保留自动战斗)
        if(g_aiFreezeLC<5){g_aiFreezeLC++;jlog(@"AI[%d]: PLAYER entity=0x%llx → ALLOWED",g_aiFreezeLC,entityRaw);}
    }
    if(g_oAIAgentUpdate) g_oAIAgentUpdate(mi_or_self, self_or_params);
}
...
// v124c: Move hook回退到v123签名, 但增加日志确认是否触发
// 关键发现: v124日志显示Move hook从未触发! MSHookFunction对CharacterFiled.Move不工作
// 保留hook但主要是诊断用途, 实际Y轴限制和吸怪走vacuumTick
...
static BOOL hMove(void *mi,void *f,void *tf,void *cf,void *targetPos,void *moveSpeed,void *fDeltaTime,BOOL updateFootY) {
    ...
    if(g_moveDebugLC < 30) {
        g_moveDebugLC++;
        int32_t entityType=-1; if(cf) memcpy(&entityType,(uint8_t*)cf+0x38,4);
        jlog(@"Move[%d]: mi=%p f=%p tf=%p cf=%p etype=%d validTF=%d",
             g_moveDebugLC, mi, f, tf, cf, entityType, tf?isValidPtr(tf):0);
    }
    
    // 用EntityType==100判断玩家
    if(cf && tf && isValidPtr(tf)) {
        int32_t entityType=-1; memcpy(&entityType,(uint8_t*)cf+0x38,4);
        if(entityType == 100) {
            // 玩家: 记录Transform2D
            if(!g_playerTransform2D || g_playerTransform2D != tf) {
                g_playerTransform2D = tf;
                jlog(@"Move: PLAYER tf=%p etype=%d camp=%d",tf,entityType,getCamp(cf));
            }
        } else {
            // 敌人: 更新Transform2D指针
            for(int i=0;i<g_enemyCount;i++) {
...
// v124c: 吸怪+Y轴限制 — 不依赖Move hook!
// 从Damage hook获取的g_playerCFFromDmg读取玩家位置
// CharacterFiled是值类型, 包含FootY字段 @ actual 0x78
// 但更完整的位置数据在AIMoveToPos @ actual 0xa8 (FPVector2, 16字节)
// 
// 方案: 在vacuumTick中:
//   1. 读取玩家CF的AIMoveToPos(目标位置, 敌人也会往这里走)
//   2. 对每个敌人CF, 直接修改其位置相关字段把怪拉到玩家位置
//   3. Y轴限制: 不需要Move hook, 在vacuumTick中持续回写玩家CF的X位置
//
// 但CharacterFiled是值类型struct, 修改CF的字段不一定影响实际位置
// 因为帧同步游戏的位置由服务器/回放决定
//
// 更好的方案: 读取CF中存储的指针型字段来找到Transform2D
// 从CF dump: SkillInfo @ 0x200-0x10=0x1f0, 这个可能是对象引用
// 实际上CharacterFiled后面紧跟着的就是IComponent数组
//
// v124c最终方案: 简化vacuumTick, 从CF dump猜测Transform2D的相对位置
// 搜索CF附近的内存找Position数据(FP RawValue在合理范围内)
static int g_vacuumTotalTicks=0;

// 从CharacterFiled中尝试读取Transform2D指针
// CF后面0x200+的区域可能包含IComponent指针
// 或者CF的某个字段本身就是Transform2D*的引用
static void* findTransform2DFromCF(void *cf) {
    if(!cf) return NULL;
    // 从CF结构后面扫描可能的指针值
    // CharacterFiled SIZE大约0x358(dump) = 0x348(actual)
    // 扫描CF+0x300到CF+0x400范围内的指针
    for(int off = 0x300; off < 0x400; off += 8) {
        void *ptr = NULL;
        memcpy(&ptr, (uint8_t*)cf + off, 8);
        if(isValidPtr(ptr)) {
            // 检查这个指针指向的内存是否像Transform2D
            // Transform2D: Position.X @ +0x00 (FP RawValue), Position.Y @ +0x08
            // FP RawValue在合理范围内: 0x10000 ~ 0x100000000 (1.0 ~ 65536.0)
            int64_t x=0, y=0;
            memcpy(&x, (uint8_t*)ptr + 0x00, 8);
            memcpy(&y, (uint8_t*)ptr + 0x08, 8);
            // 位置值应该是正的FP值 (绝对值 < 0x100000000 = 65536.0 in FP)
            if(x != 0 && (x > 0x1000 && x < 0x100000000LL) && (y > 0x1000 && y < 0x100000000LL)) {
                return ptr;
            }
            // 也可能是负值
            if(x != 0 && (x > -0x100000000LL && x < -0x1000) && (y > -0x100000000LL && y < -0x1000)) {
                return ptr;
            }
        }
    }
...
static void vacuumTick(NSTimer *t) {
    ...
    // v124c: 不依赖g_playerTransform2D(来自Move hook), 用g_playerCFFromDmg
    void *playerCF = g_playerCFFromDmg;
    if(!playerCF) {
        if(g_vacuumTotalTicks % 50 == 1) jlog(@"vacuumTick[%d]: no playerCF yet (from Damage)", g_vacuumTotalTicks);
        return;
    }
    
    // 尝试从playerCF找到Transform2D
    if(!g_playerTransform2D) {
        g_playerTransform2D = findTransform2DFromCF(playerCF);
        if(g_playerTransform2D) {
            int64_t x=0,y=0;
            memcpy(&x,(uint8_t*)g_playerTransform2D+0x00,8);
            memcpy(&y,(uint8_t*)g_playerTransform2D+0x08,8);
            jlog(@"vacuum: FOUND playerTF=%p from CF=%p x=0x%llx y=0x%llx", g_playerTransform2D, playerCF, x, y);
        }
    }
    
    if(!g_playerTransform2D) {
        if(g_vacuumTotalTicks % 50 == 1) jlog(@"vacuumTick[%d]: playerCF=%p but no TF found", g_vacuumTotalTicks, playerCF);
        return;
    }
    
    // 读取玩家Position.X和Position.Y
    int64_t playerX_raw=0, playerY_raw=0;
    memcpy(&playerX_raw, (uint8_t*)g_playerTransform2D + 0x00, 8);
    memcpy(&playerY_raw, (uint8_t*)g_playerTransform2D + 0x08, 8);
    
    // Y轴限制: 回写玩家Position.X使其不变
    // (vacuumTick每0.1秒调用一次, 间隔内可能已经有X位移了)
    // 但这不够频繁, 需要更快的频率或在其他hook中做
    
    // 吸怪: 遍历所有敌人, 把Position改成玩家位置
    int pulled=0;
    for(int i=0; i<g_enemyCount; i++) {
        if(!g_enemyTransforms[i]) {
            // 敌人还没有TF, 尝试从CF找
            if(g_enemyCFs[i]) {
                g_enemyTransforms[i] = findTransform2DFromCF(g_enemyCFs[i]);
            }
        ...
        // 读取敌人当前X和Y
        int64_t enemyX_raw=0, enemyY_raw=0;
        memcpy(&enemyX_raw, (uint8_t*)g_enemyTransforms[i] + 0x00, 8);
        memcpy(&enemyY_raw, (uint8_t*)g_enemyTransforms[i] + 0x08, 8);
        
        // 如果X或Y不同, 修改为玩家位置(瞬移到脚下)
        if(enemyX_raw != playerX_raw || enemyY_raw != playerY_raw) {
            memcpy((uint8_t*)g_enemyTransforms[i] + 0x00, &playerX_raw, 8);
            memcpy((uint8_t*)g_enemyTransforms[i] + 0x08, &playerY_raw, 8);
            pulled++;
        }
    }
    
    if(g_vacuumTotalTicks % 50 == 1) {
        jlog(@"vacuumTick[%d]: playerTF=%p playerX=0x%llx playerY=0x%llx pulled=%d enemies=%d", 
             g_vacuumTotalTicks, g_playerTransform2D, playerX_raw, playerY_raw, pulled, g_enemyCount);
    }
}
...
static BOOL hIsReady(void *f,int st,void *cf,void *st2){
    // v123: 只记录playerCF, 不调用trackEnemy(没有entity)
    if(cf&&isPlayerCF(cf)){if(!g_playerCFLearned){g_playerCF=cf;g_playerCFLearned=YES;}}
...
static BOOL hCanBeAttack(void *cf){
    // v123: 不再调用trackEnemy(cf,NULL) — CanBeAttack没有entity参数
    // trackEnemy只从Damage hook调用(有entity)
    if(cf){if(isPlayerCF(cf)&&g_godMode){if(g_canBeAtkLC<20)g_canBeAtkLC++;return NO;}}
...
static int64_t hDamage(void *f,void *atkEnt,void *atkCF,void *tgtEnt,void *tgtCF,
    ...
    // v123: 增加EntityType调试 — 前10次打印
    if(g_dmgLC<10){
        g_dmgLC++;
        int32_t atkET=atkCF?getEntityType(atkCF):-2, tgtET=tgtCF?getEntityType(tgtCF):-2;
        int32_t atkCamp=atkCF?getCamp(atkCF):-2, tgtCamp=tgtCF?getCamp(tgtCF):-2;
        uint64_t atkRaw=0,tgtRaw=0; if(atkEnt)memcpy(&atkRaw,&atkEnt,8); if(tgtEnt)memcpy(&tgtRaw,&tgtEnt,8);
        jlog(@"Dmg[%d]: atkP=%d tgtP=%d atkET=%d tgtET=%d atkCamp=%d tgtCamp=%d atkRaw=0x%llx tgtRaw=0x%llx",
             g_dmgLC,atkP,tgtP,atkET,tgtET,atkCamp,tgtCamp,atkRaw,tgtRaw);
    }
    if(atkP&&atkEnt){g_playerEntity=atkEnt; memcpy(&g_playerEntityRaw,&atkEnt,8);}
    // v124c: 捕获玩家CF, 不依赖Move hook
    if(atkP&&atkCF){g_playerCFFromDmg=atkCF; if(!g_playerCFLearned){g_playerCF=atkCF;g_playerCFLearned=YES;}}
    ...
    if(atkP && g_damageMulti>1 && !tgtP) {
        ...
        if(g_dmgLC<20){g_dmgLC++;jlog(@"Dmg×%d",g_damageMulti);}
    }
    return r;
}
...
static void hChangeCharacterToDead(void *mi,void *f,void *target,void *atk,void *tgtCF,int64_t airImpulse,int32_t deathType,BOOL forceSwing) {
    if(g_godMode && tgtCF && isPlayerCF(tgtCF)) {
        ...
        // v123: Invincible @ dump 0x10 → actual 0x00 (1字节byte)
        uint8_t inv=1; memcpy((uint8_t*)tgtCF,&inv,1);
        // v123: isDead @ dump 0x58 → actual 0x48 (QBoolean=4字节int, 不是1字节!)
...
static void hUseSkill(void *f,void *entity,void *cf,int skillStateType,BOOL isRight,void *states,void *state,void *playerInfo) {
    if(g_skillReplace && skillStateType>=14 && skillStateType<=18) {
        if(cf) {
            // v123: 用isAICF判断 — IsAI @ dump 0x54 → actual 0x44 (QBoolean=4字节int)
            // 玩家IsAI=0, 敌人IsAI!=0
            if(!isAICF(cf)) {
...
static void findIL2CPP(void) {
    ...
    if(class_from_name && get_fields && field_name && field_offset) {
        ...
        // v123: Dump CharacterFiled字段偏移 — 验证EntityType/Camp/isDead等offset
        for(size_t a=0;a<assemCount;a++){
            void *img=get_image(assemblies[a]); if(!img)continue;
            void *cfklass=class_from_name(img,"FrameSync","CharacterFiled");
            if(cfklass) {
                jlog(@"=== v124: Dump CharacterFiled fields (runtime offsets) ===");
                void *fiter=NULL, *field=NULL;
                while((field=get_fields(cfklass,&fiter))!=NULL) {
                    const char *fn=field_name(field);
                    int32_t off=field_offset(field);
                    jlog(@"  CF field: %s @ 0x%x (actual=0x%x)", fn?:"?", off, off>0x10?off-0x10:off);
                }
                break;
            }
        }
    ...
    jlog(@"v124: Move=%p AIAgentUpdate=%p", g_fMove, g_fAIAgentUpdate);
...
static void refreshBtns(void){
    ...
    if(g_fullScreen){[g_btnFullScreen setTitle:@"ON  全屏+吸怪+Y锁" forState:UIControlStateNormal];g_btnFullScreen.backgroundColor=IMGUI_BTN_ON;g_btnFullScreen.layer.borderColor=IMGUI_GREEN.CGColor;}
    else{[g_btnFullScreen setTitle:@"OFF 全屏+吸怪+Y锁" forState:UIControlStateNormal];g_btnFullScreen.backgroundColor=IMGUI_BTN_OFF;g_btnFullScreen.layer.borderColor=IMGUI_RED.CGColor;}
...
-(void)onFullScreen{
    ...
    if(g_fullScreen){findIL2CPP();
        ...
        // v124: 全屏自动hook Move(Y轴限制+吸怪都需要Transform2D*)
        if(g_fMove && !g_hMove) hookOneFunc(g_fMove, hMove, (void**)&g_oMove, &g_hMove, "Move(fullScreen+Y-axis+vacuum)");
        // v124: 全屏自动启动吸怪(把怪瞬移到玩家位置)
        if(!g_hCanBeAttack)hookOneFunc(g_fCanBeAttack,hCanBeAttack,(void**)&g_oCanBeAttack,&g_hCanBeAttack,"CanBeAttack");
        g_enemyCount=0;
        g_playerTransform2D=NULL;
        startVacuumTimer();
        jlog(@"fullScreen: ON - 秒杀+Y轴限制+吸怪自动启动");
    } else {
        // v124: 全屏关闭时停止吸怪
        stopVacuumTimer();
        g_playerTransform2D=NULL;
        jlog(@"fullScreen: OFF - 吸怪停止");
    }
    refreshBtns();
}
-(void)onSkillReplace{
...
// v121: 怪不动 — hook AIAgent.Update只冻结敌方, 同时自动吸怪X轴对齐
-(void)onMonsterFreeze {
    g_monsterFreeze=!g_monsterFreeze;
    if(g_monsterFreeze) {
        findIL2CPP();
        // hook AIAgent.Update — 只冻结敌方AI, 玩家正常(自动战斗不受影响)
        ...
        // 同时也hook Move获取Transform2D*(吸怪X轴对齐用)
        if(g_fMove && !g_hMove) hookOneFunc(g_fMove, hMove, (void**)&g_oMove, &g_hMove, "Move(vacuum)");
        // hook Damage/CanBeAttack追踪敌人entity和Transform
        if(!g_hCanBeAttack)hookOneFunc(g_fCanBeAttack,hCanBeAttack,(void**)&g_oCanBeAttack,&g_hCanBeAttack,"CanBeAttack");
        if(!g_hDamage)hookOneFunc(g_fDamage,hDamage,(void**)&g_oDamage,&g_hDamage,"Damage");
        // 自动启动吸怪定时器 — 把敌人X拉到玩家X(同一垂直线)
        g_enemyCount=0;
        g_playerTransform2D=NULL;
        startVacuumTimer();
        jlog(@"monsterFreeze: ON - AIAgent.Update hooked=%d + vacuum auto-started", g_hAIAgentUpdate);
    } else {
        stopVacuumTimer();
        g_playerTransform2D=NULL;
        jlog(@"monsterFreeze: OFF");
    }
    refreshBtns();
}
...
static void setupUI(void){
    ...
    UILabel *tl=[[UILabel alloc]initWithFrame:CGRectMake(10,6,g_panelW-20,20)];tl.text=@"剑影江湖 v124c";tl.textColor=IMGUI_ACCENT;tl.font=[UIFont boldSystemFontOfSize:15];tl.textAlignment=NSTextAlignmentCenter;[tb addSubview:tl];
...
static void initialize(void){
    ...
    jlog(@"========== JYJH v124c (不依赖Move hook的吸怪) ==========");
